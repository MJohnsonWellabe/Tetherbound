...
**Mandate:** `docs/acceptance/GATE_F_PROTOCOL.md` §2–§12. This document is the executable
...
**The bar is `docs/GAME_VISION.md` plus current owner decisions,
...
1. **Input is synthetic.** All input enters through Godot's input system as
   ...
   **UI focus navigation MUST use `Input.parse_input_event` — action-press
   alone does not move focus** (`docs/AGENT_WORKFLOW.md`, pattern in
...
2. **Rendering runs under xvfb + opengl3 (software rasterizer), never
   ...
   (`docs/AGENT_WORKFLOW.md`). Logic-only segments may run `--headless` (no
...
(`docs/owner/OWNER_PLAYTEST_2026-08-30.md`): from the first frame of a new
...
`docs/acceptance/GATE_F_INSTRUMENTATION_REQUEST.md`. Summary:
