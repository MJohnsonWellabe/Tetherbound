# Archived 2026-09-10 — superseded by docs/DEVELOPMENT_ROADMAP.md

This plan governed the old Meadows-only "Gate 2 / Gate 3" framing. `docs/ROADMAP.md` (which this plan sits under) itself states it is now "subordinate to docs/DEVELOPMENT_ROADMAP.md (the cross-biome sequence)" and that its per-gate status lines "are older than docs/CURRENT_STATE.md and lose to it." The four-biome Stage A-G framing in `docs/DEVELOPMENT_ROADMAP.md` is current; this file is preserved for historical reference only. Preserved verbatim below.
...
acceptance authority for both gates) and `archive/docs/gates/GATE3_EXECUTION_PLAN.md` (who is doing
...
| 2.9 Pond walker | **closed** | RE-MEASURED 2026-09-05 by N10-HARNESS-TESTS-0905, which is what this row was waiting for: the world had been exonerated (`probe_pond_stranding.gd`, 0 of 10 stands wedged) and the walker never re-run. `tools/gate_f/probe_pond_walker_fix.gd` drives `stick_navigator.gd::walk_to()` from the exact coordinate GATE2-EVIDENCE-0903's S05 run froze at — `(-328.7, -14.2, 505.3)`, pinned to the centimetre on three runs — on the same bearing toward the Old Bram detour, with the same 29,250-frame budget the failing run burned 543 s inside, no `S05-32x` workaround waypoint and no teleport. **It arrives, on both arms of an A/B with the walker as the only variable**: `origin/main`'s walker `arrived=true` in 9,725 frames (162.1 play-seconds), 3.9 m short of target; this lane's walker `arrived=true` in 10,257 frames (170.9 play-seconds), same finish. So the fix is FENCE-CORNER-0903 (`c64af25f`, landed in PR #30), already on `main` — the same landing that closed CL-H14 — and this lane's foot-height probe costs the leg 8.9 play-seconds and does not regress it. The signature W03-S08-FREEZE-0904 noticed this row shared with CL-H14 was the right read. One honest caveat, recorded rather than filed off: the controller's own `_recover_if_entombed` failsafe fired once mid-route on this lane's arm, at `(77.31, -2.19, 839.44)`, 340 m past the basin — not the Pond, not this row, and worth a look by whoever owns that stretch. | N10-HARNESS-TESTS-0905 — landed on `ralph/N10-HARNESS-TESTS-0905` |
...
| 2.14 stale trace thresholds | **closed** | all three re-derived by S02-60's own method — 80% of the shortest healthy completion measured from committed telemetry. S02 900→450 (W20-SMALL-FIXES-0904); S04 480→**420** and S05 1200→**970** (N10-HARNESS-TESTS-0905), the two of them having sat at 92% and 98.8% of their own shortest recorded run. Guarded by `tests/test_gate_f_harness_predicates.gd`. See CL-H4. | N10-HARNESS-TESTS-0905 — landed on `ralph/N10-HARNESS-TESTS-0905` |
...
| CL-H13 | **FIXED 2026-09-05 (W02-HARNESS-CONTEXT-0904, D74): the harness was context-blind, not the game.** `input_context` reported the truth at all three sites; the harness pressed `combat_charged` by action name with **no fight running**, injected LT (`JoyAxis:4`), and LT in `world` is `build_shortcut` — the Build catalogue opens by design (HUD-INPUT-0903). Reproduced deterministically per frame by `tools/gate_f/probe_press_context_flip.gd`: at Oreth with a 1 HP lead the blind injection FLIPS at `S08-93` (frame 1665, `world`, arbiter enabled, game reads `build_shortcut+map_zoom_out+build_rotate_left`), at Vance at `S07-57` (frame 2039). The row's own premise was wrong: 'after Captain Riverwatch' is Oreth (`captain_riverwatch` = Captain Oreth) and `S08-93` is a joypad charged hold, so all three sites press LT; BAND3's shared-binding observation was right, its player-facing reading was not (a player knows whether they are in a fight). **Fix at the mechanism:** `press`/`press_until`/`hold` resolve the control against the live `input_context` and `input_contexts.json` before every repetition and refuse — FAIL, naming context, binding and the live action it would fire — what is not live there; unmapped contexts/actions pass through `[unchecked]`. Same probe, guarded: `S08-93` refused by name, final context `world`, CLEAN. `test_gate_f_rig.gd` 53/0 (four new real-behaviour tests, each seen red with the guard neutered); S08's first 30 steps through the fixed harness 26 PASS / 4 DELEGATED / 0 FAIL, never `build_catalogue`. No rebinding, no mouse routing, no segment JSON edit, nothing under `scripts/`. **Consequence:** blind press blocks that outlive their fight now FAIL at the first press after it ends (CL-H1's cure applies). Report: `ralph/reports/W02-HARNESS-CONTEXT-0904/REPORT.md`. | M | **fixed** |
| CL-H14 | ~~**S08's Ironwood-approach leg freezes solid, deterministically — and it is why Band 4 still has no evidence.**~~ **CLOSED, W03-S08-FREEZE-0904, root-caused and reproduced a third time.** **The body walks head-on into a tree, and the walker of the day could not get it off.** At frame 1474 of the reproduction, travelling 5.00 m/s, the capsule contacts `CommonTree_2_Collision` with normal `(0.33, 0.00, -0.95)` — dot **-1.00** against its own travel, a dead-centre hit with no tangential component for `move_and_slide` to turn into a slide; velocity goes to exactly zero on that frame and never changes again. The world is behaving correctly: ground 1 cm under the feet, `on_floor` true every frame, downward rays from +1 m and +3 m both hitting `Terrain` at the body's own height, the camera rig **1.7 m** away all through (so collision is streamed — not a Terrain3D artefact), the deployed ally 10.2 m off on `collision_layer 0`, `unstick_count` **0** because `_entombed_at()` correctly reads 'pressed against something, not sealed in it', and eleven authored tree colliders within 6 m. What pinned it was `stick_navigator.gd` as it stood then: `_detour_stalled()` flips `_side` **and zeroes `_side_detours`**, so the counter never climbs past 1, `DETOURS_PER_SIDE` (3) is never reached, `_back_off()` is never called, and the walk runs a closed **41-frame cycle at exactly zero displacement** to the end of its budget — 41 distinct states, `locomotion_enabled` true, `input_context` `world` throughout, which is precisely the signature the original report could not explain. **The fix was already on `main` and had been sitting in an unmerged pull request while this was being investigated:** FENCE-CORNER-0903 (`c64af25f`, landed as #30 / `65fc6625`) abandons a side on **measured progress** instead of a resettable attempt count. `c64af25f` is an ancestor of neither `44f06cf9` nor `9413488b` — the trees CL-H14 was measured on. **Proven by swapping only the walker on today's tree:** the frozen copy at `tools/gate_f/probe_s08_freeze_legacy_navigator.gd` pins at `(-164.12, -9.13, 4334.56)`, 747.6 m short — the recorded coordinate to the centimetre and its distance to the tenth of a metre — while the live walker **arrives: 839.5 m to (-345, 5060) in 10,907 walking frames, 0 held**, with the segment then running 12 steps past S08-22. **The waypoint was not moved**; the brief's 'fails if' is not met. Also settles why the isolated probe always arrived: the trap is **dynamic**, not a place you can stand in — four teleported starts, the freeze coordinate itself included, all walk out; only a body pressed into that trunk by its own motion is stuck, which is also why the earlier cold-teleport probe read 'inconclusive'. Residual, **not** fixed and recorded for the walker's next owner: `_free_space()`'s lowest probe ray is 45 cm up and this geometry is below it, so the walker still cannot see what stops it and escapes only by noticing it made no progress. Source: `ralph/reports/W03-S08-FREEZE-0904/REPORT.md`. | M–L | **closed — reproduced, root-caused, fix already landed** |
...
| CL-H14 | **S08's Ironwood-approach leg freezes solid — CLOSED, root-caused, and the fix was already on `main`.** Rewritten N10-HARNESS-TESTS-0905 on W03-S08-FREEZE-0904's evidence (`ralph/reports/W03-S08-FREEZE-0904/REPORT.md`). The body did not fall through the world and Terrain3D did not fail to stream: it **walked head-on into a tree**. At frame 1474 of the reproduction the capsule contacts `CommonTree_2_Collision` with a surface normal of (0.33, 0.00, −0.95) — dot **−1.00** against its own direction of travel, no tangential component for `move_and_slide` to convert into a slide — and velocity goes to exactly zero and stays there. What kept it there was the WALKER: the pre-`c64af25f` `stick_navigator.gd` reset its per-side attempt counter on every side flip, so `DETOURS_PER_SIDE` was unreachable and the walk ran a closed 41-frame cycle at zero displacement to the end of its budget. **FENCE-CORNER-0903 (`c64af25f`) already fixed it** — measured progress instead of attempt counting — and reached `main` in **#30 (`65fc6625`)** while CL-H14 was being investigated as an unexplained world defect. Proved by A/B on today's tree with the walker as the only variable: the frozen arm pins at `(-164.12, -9.13, 4334.56)`, 747.6 m short (CL-H14's coordinate to the centimetre, third independent measurement) with `probe_s08_freeze_legacy_navigator.gd`; the live walker **arrives** (11,185 walking frames, 0 held), touching 78 tree colliders in the same 25 m of z and getting off each within 0.65 s. The segment agrees: `S08-22 PASS`, *walked 839.5 m to (-345, 5060) in 10,907 walking frames (0 held)*, plus 12 steps past it — and W21-HARNESS-FIGHTS-0904 crossed the same leg cleanly on its own branch, independently. **The waypoint was not moved**, so this row's own **fails if** is respected. Reproduction: `tools/gate_f/probe_s08_freeze_repro.gd --mode=loaded --deploy --settle=10800 --budget=45000 --stop-on-freeze [--navigator=…probe_s08_freeze_legacy_navigator.gd]`, ~12 min per arm. The deeper cause the freeze exposed — `_free_space()` blind to geometry under 45 cm, so the walker commits detours into flanks the body cannot enter and escapes by noticing no progress rather than by seeing the obstacle — is **fixed separately by N10-HARNESS-TESTS-0905**: a fourth probe height at 0.38 m, three centimetres above `player_controller.gd::STEP_HEIGHT`, guarded both ways by `tests/smoke_stick_navigator_low_geometry.gd` (a 0.40 m root flare must read blocked; a 0.30 m kerb must not). Evidence for the row rewrite lives on `ralph/W03-S08-FREEZE-0904`, which was not yet merged when this row was written; the FIX it credits is on `main`. | G3-HARNESS §4; W03-S08-FREEZE-0904 | — | — | **closed** |
| CL-H13 | **The harness's `input_context` "misresolves" to `build_catalogue` — ROOT-CAUSED, and it was never a misresolution.** Rewritten N10-HARNESS-TESTS-0905 on W02-HARNESS-CONTEXT-0904's evidence (`ralph/reports/W02-HARNESS-CONTEXT-0904/REPORT.md`, branch `ralph/W02-HARNESS-CONTEXT-0904`). At all three sites the segment pressed `combat_charged` **with no fight running**; the harness injected that action's physical binding (LT, `JoyAxis:4`) and the engine marked every action bound to that axis pressed — in `world`, LT is `build_shortcut`, which opens the Build catalogue **by design** (HUD-INPUT-0903). The catalogue opened, the harness wrote `pressed combat_charged x1 … PASS`, and every later step ran behind a menu nothing in the script closes. The game's two shipped guards hold (`input_contexts.json` exclusivity; `_world_input_allowed()` refuses during combat) and **nothing under `scripts/` changed**: the harness was context-blind, not the router. This row's own claim that "two of the three sites involve no charged attack at all" was **wrong** — `S08-93` is a joypad `combat_charged` hold inside the Oreth block, and all three sites press LT; `captain_riverwatch` *is* Captain Oreth. The fix is a press guard in `operator_harness.gd` (`_load_input_contexts`/`_resolve_press`/`_press_guard`, re-resolved before every repetition of `press`, `press_until` and `hold`) that turns a press into `FAIL press guard: …` naming the context, the binding and the colliding live actions, plus `docs/decisions/D74`. **NOT ON `main` AS OF THIS COMMIT** — checked, not assumed: `grep -c '_press_guard\|_resolve_press' tools/gate_f/operator_harness.gd` on `f8a47ee4` returns 0. The row is **fixed on a branch, unlanded**, and `operator_harness.gd` also carries N10-HARNESS-TESTS-0905's own unmerged edits, so the coordinator should expect one merge in that file. | three lane reports (S06–S10 evidence); W02-HARNESS-CONTEXT-0904 | — | — | **fixed on `ralph/W02-HARNESS-CONTEXT-0904`, not yet on `main`** |
...
|---|---|---|---|---|---|
| CL-H1 | **Blind press blocks in every Gate 3 segment** — **re-scripted on a branch, not yet landed.** W21-HARNESS-FIGHTS-0904 (`ralph/reports/W21-HARNESS-FIGHTS-0904/REPORT.md`, branch `ralph/W21-HARNESS-FIGHTS-0904`) converted **43 counted `press combat_quick` blocks into 23 `fight_until_resolved` steps and 4 `chip_to_floor` chips** across `S06–S09` and their capture twins, plus 20 counted challenge conversations to `advance_dialogue_until_closed`, 21 by-identity Revive ladders and 25 post-fight switches, guarded by `tests/test_gate_f_segments.gd` (seen red 6/6 on the pre-change tree). **Nine trainer fights were then played, and every one ended on its own defeat flag with a live party** — including S06-22 Dorn, the exact fight G3-BAND2 watched wipe a party of four inside a 34-press block: 41 presses, 76 of 170 HP off the lead, bench untouched. **Six of the ten fights in those segments are still unplayed**, blocked behind three defects that lane did not own: S06's Warrens chamber and guardian (the S06-50 navigator pin), S08's three captains and the ridgeline patrol (the S08-79 arbiter derail, CL-H12's shape at a new site), S09's checkpoint (the S09-33 gorge, now fixed by N10-HARNESS-TESTS-0905 — see CL-H3). **NOT ON `main` AS OF THIS COMMIT** — checked, not assumed: `S06.json` on `f8a47ee4` holds 0 `fight_until_resolved` steps and `tests/test_gate_f_segments.gd` does not exist there. N10-HARNESS-TESTS-0905 did not need to touch the remaining fights and did not; it fixed the two harness-side defects W21 routed out (`fight_until_resolved` PASSing on an absence, `chip_to_floor` unpriced) and left the segment scripts to that lane's landing. | G3-BAND2 addendum; G3-BAND3 §4b; G3-BAND4 addendum; W21-HARNESS-FIGHTS-0904 | — | — | **re-scripted on `ralph/W21-HARNESS-FIGHTS-0904`, not yet on `main`** |
...
| CL-H4 | **Stale route-row thresholds** (2.14) — **CLOSED, all three re-derived by one method.** S02-60's 900 was fixed to 450 by W20-SMALL-FIXES-0904; N10-HARNESS-TESTS-0905 did S04-61 and S05-60 the same way. The method is the point: the check exists to catch a recorder that stopped, not to grade pacing (each segment's own distance and dead-travel asserts do that), so the threshold is **80% of the shortest healthy completion on record**, counted from every committed `<segment>/telemetry/route.csv` under `ralph/reports/` and filtered to runs whose measured rate really is ~2 rows per play second (the 2026-08-25/27 pair record 1.5–1.6 on a different clock and are excluded rather than averaged in). S04: six healthy completions of 526–658 rows, shortest 526 → **420** (was 480, i.e. 92% of the shortest run this segment has ever recorded). S05: four of 1,214–1,531, shortest 1,214 → **970** (was 1,200, i.e. 98.8% — not headroom at all). Both still fail every broken shape on record, including the 164-row aborted S04 attempt and the 89-row S05 derail. Guarded by `tests/test_gate_f_harness_predicates.gd`, which re-derives from the committed telemetry and fails any threshold above 85% of its own shortest run. | 2.8 §4.7; chain S02; W20-SMALL-FIXES-0904; N10-HARNESS-TESTS-0905 | — | — | **closed** |
...
| CL-H8 | **`S06-30`'s invented workbench beat — DECIDED, and now REMOVED.** W20-SMALL-FIXES-0904 settled the fork with one live probe rather than by argument (`tools/_probe_workbench_context.gd`, driving the real nodes and reading the harness's own resolver): `world → open the catalogue → build_catalogue → close → world → place a real workbench through build_placer.gd → world → interact its CraftInteractable → panel:craft_panel.gd → cancel → world`. The workbench interact **never enters `build_catalogue` at all**, so the beat was not photographing a shared-UI defect; S06's stuck catalogue is CL-H13's, above. N10-HARNESS-TESTS-0905 carried out the deletion W20 handed on: **`S06-31`…`S06-49` are gone**, and section L.3's requirement (*S06 orb_greater + reinforced tools — each paid at real cost*) is rehomed to band 2's own authored crafting site rather than deleted with the beat. The site exists and this row's premise that "band 2 authors no crafting site" was true only of the protocol's text: `data/config/bands/band2_stone_and_root/props.json`'s `ranger_camp` carries `craft_at: [-259.6, 2257.4]` and `craft_label: "Craft at the ranger's anvil"`, stood up as a real `CraftInteractable` by `scripts/world/rest_point.gd` (T5-CADENCE; `tests/smoke_authored_camps.gd` already asserts an authored camp offers craft as well as rest), on the `ranger_camp_spur` loop between the quarry and the Warrens mouth, pinned on the map as `band2_ranger_camp`. New steps `S06-30a`…`S06-30l` walk there and craft on rootstone broken out at the quarry one step earlier. | G3-BAND2 addendum root cause 2; W20-SMALL-FIXES-0904; N10-HARNESS-TESTS-0905 | — | — | **closed** |
...
| CL-D6 | `GATE3_EXECUTION_PLAN.md` §4's open questions, three of five now have answers: the Warden reads harder (measured, then W-1 raised him); the relay's escalation is V-1/V-2 applied but unplayed; Band 5 is not an empty corridor (26/30/22 creatures counted live at three eyes, 63 m worst gap). Record them. | `archive/docs/gates/GATE3_EXECUTION_PLAN.md` §4 | S |
...
| CL-G11 | **Every NPC is drawn with the player's portrait.** Was: `assets/ui/portraits/` held `trainer.png` and `grandpa.png` only and 125 of 138 authored `portrait` entries named `trainer.png`. **Wiring closed** in two steps: W04-PORTRAITS (PR #45) rendered 34 plates from the installed bodies and re-pointed every field outside `stronghold.json`; N04-DIALOGUE-PORTRAITS (2026-09-05, D87) re-pointed the Warden's three conversations to `warden.png`, gave the shared generic trainer refusals the challenged trainer's own name and plate (`dialogue_panel.start(id, identity)` + `trainer_npc.speaker_identity()`), and laid each female-rig villager's hair colour on the painted hair through a baked mask so Mira/Tam/Halda/Rae/Doss no longer share one face. Evidence: `tests/test_dialogue_portraits.gd` opens the REAL panel on Halda then Oskar, and Mira then Tam, and asserts two different plates each matching the speaker's own field; no non-player speaker resolves to `trainer.png` (the only carve-out is three bodiless stronghold narration speakers, by name, per that file's own comment); `tests/smoke_trainer_refusal_portrait.gd` proves the refusal in the booted world; `tests/test_villager_female_painted_hair.gd` pins the mask and the per-colour materials. **Still partial, honestly:** no NPC lacks a plate that loads, but several share one — the male-rig villagers Oskar, Bram, Kell, the Quarry Foreman and bodiless Coll all wear `villager_male.png` (that rig has no separable hair and no mask yet); Sela, Dara, Nan and the Rescued Ranger all wear `villager_ranger.png`; Team Tether speakers wear their rank's plate; the Readout, Duty Board and Chamber Five wear `trainer.png` by the finale lane's choice. Every plate but `trainer.png`/`grandpa.png` is a rig render, not painted art, and the shared-rig cheek-wedge texture artefact (D81) is untouched. | owner #8b | M | wiring closed (N04, 2026-09-05); per-NPC art partial — see item |
