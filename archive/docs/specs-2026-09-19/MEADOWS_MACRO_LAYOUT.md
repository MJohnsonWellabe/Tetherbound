...
`docs/owner/MEADOWS_QUALITY_REBUILD_PLAN.md` §5 asks for before terrain
...
- `archive/ralph/DONE.md:2752` — "The bake is **~5.5 minutes** and everything geometric
  depends on it."
...
| `archive/ralph/DONE.md:16` (`SG44`) | storm_road "a BAKED 11 m carve with **65.6°** walls" | as shipped |
...
4. **Widen before you re-bake, not after.** Raising a `rim` after a 3-hour bake
   ...
   (`archive/ralph/DONE.md:2752`), so every carve can be re-probed at the new origin
...
and `scripts/world/rift_collapse.gd` (`SG44`, `archive/ralph/DONE.md:16`) already
...
**23,452** (`archive/ralph/DONE.md:10002`, the `SA1-lod` note), **23,762** (`:7206`),
...
- **The bake is cheap to redo and cheap to avoid redoing.** `height_at` is
  analytic and reads the JSON live (`archive/ralph/DONE.md:2752`), so any landform
...
`smoke_boss` assert against its walls, and `archive/ralph/DONE.md:16` records it as
