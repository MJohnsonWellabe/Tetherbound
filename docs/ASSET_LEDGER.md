...
| Village props ×10 | Quaternius | [Medieval Village MegaKit](https://quaternius.itch.io/medieval-village-megakit), Standard (free) tier | CC0 1.0, stated in the pack's own `License_Standard.txt` | No | `assets/buildings/*.gltf` + 9 PBR maps | 10 of 176 taken: wagon, crate, log, chimney, support, two bricks, three fence panels. The kit is MODULAR — walls, roofs, corners — so almost none of it is a standalone landmark; these are the pieces that are. `Prop_MetalFence_*` was extracted and then deleted because its textures are Pro-tier only and it would have imported untextured, which is the exact defect the critic named. |
