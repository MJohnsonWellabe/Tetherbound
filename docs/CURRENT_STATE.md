...
| Traversal / South Bridge | **Working; the entombment does not reproduce** | `smoke_traversal` passed on the first attempt (2026-09-03, ~13 min in this container). The documented entombment at (7.9, −3.4, 1319) was re-investigated with four independent methods — an isolated ring probe (`tools/probe_south_bridge_gully.gd`, 8 bearings × 3 radii around the site, settle only), the same ring with a forced walk into the gully at every point, a full replay of the exact prior play history (all nine perimeter stations + the kill-volume respawn) before checking the site, and the real unmodified `smoke_traversal.gd` — and none produced a sealed placement or a `[player] entombed ...` log line. Walking north from the site legitimately drops the player over the gully lip near the gate, and `severed_spokes.gd`'s own carve failsafe recovers it to the road every time, which is the mechanism working as designed. `smoke_traversal.gd` now asserts this directly (`_assert_south_bridge_site_sound`, same `_entombed_at` predicate the production failsafe uses) as a permanent regression guard. Not marked fixed — no geometry changed — only verified sound in this session; a fresh owner or Gate F reproduction reopens it as a new finding |
...
| P1 | South Bridge entombment at (7.9, −3.4, 1319) | terrain/collision near `south_bridge.gd` | **verified sound 2026-09-03** — does not reproduce (see §2 row above and `tools/probe_south_bridge_gully.gd`); regression guard added to `smoke_traversal.gd`; reopen on any fresh reproduction |
...
| SOUTH-BRIDGE-HOLE | no player-facing change: four probes place the capsule around (7.9, −3.4, 1319) with the terrain flat and no depenetration, so the entombment does not reproduce on the current tree. A regression guard now asserts the site stays sound | probes 4/4; `smoke_traversal` passed first attempt earlier in the session. **Watched, not closed** — the owner has never reported it, and it was last reproduced against an older tree |
