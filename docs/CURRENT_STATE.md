...
| Performance on the ROG Ally | Unable to verify | no container can measure it; perf proxy: band1_open 6943 draws / 11.62M prims (GRASS-CULL-0903: was 6947 / 11.79M) under provisional budget |
...
| P0 *(owner, hardware)* | ~10 FPS with grass on | `grass_field.json` on at 75k tufts | needs an Ally measurement; perf proxy is under budget. **GRASS-CULL-0903**: re-checked whether the distance culling `_comment_enabled_ownerplaytest_20260901` called "unbuilt" still was. It mostly wasn't — VP2 (already on `main`) tile-culls every tier and hard-caps stones/flowers/litter at `reach_m`; the one populous tier it missed was bushes (6000 count, no reach cap), because its own lattice plan has `per_cell` 1 in every layer and the grass tier's `_thin_far` mechanism (built to split a dense per-cell count into shorter-reach sub-layers) is a structural no-op at that density — confirmed with a debug probe, not assumed. Gave bushes a `reach_m`/`reach_band_m` cap instead (the mechanism `_cap_reach` already uses for flowers/litter, which works at any density because it shrinks the generation radius rather than splitting a count): band1_open 11,789,608 → 11,619,782 primitives (−1.4%, `tools/perf_render_stats.gd`, real before/after in this run). Real but small next to the 22.5M-primitive game-breaker; every other tier was already treated, so this was the one primitive count left to move without shrinking an unmeasured number or leaving this lane's scope. Ally A/B still the only thing that can say whether 11.6M is affordable — unchanged by this session |
