...
| P0 *(owner)* | Interact works "about half the time" | `scripts/world/harvest_logic.gd::swing_answers_the_prompt()` (suspected) | **REPRODUCED IN-CONTAINER 2026-09-03**, first time — it is not hardware-only. `smoke_gate_b_continuous` run 2 of 3: `physical interact on the node did not start the visible axe swing (arbiter winner=Interactable, equipped=axe, prop=Axe_Bronze2, cooling=false, node=@Node3D@62713)`, and the winner's parent IS that node — so every precondition the assertion checks was correct and the press still produced nothing. Run 1 passed the same step, which is the owner's "about half the time". Leading hypothesis: `swing_answers_the_prompt()` returns true WITHOUT starting a swing when one is already in flight ("that swing resolves on its own"), so a press landing in the tail of a previous swing is silently swallowed — from the player's seat, X does nothing. The harness now samples `is_swinging` and tool_hold's own `equipped()` BEFORE the press, which separates that from a genuinely idle hand. Ruled out: `build_shortcut` stealing the button (interact is joypad 2, build_shortcut is axis 4) |
...
| P1 | Objective chain stalls one rung later than it did | `scripts/world/quest_log.gd`, `data/progression/objectives.json` | **still open, and it MOVED (2026-09-03).** OBJECTIVE-CAMP-0903 fixed the "Gather supplies for your team's camp" stall, and `smoke_gate_b_continuous` now drives 22 minutes of continuous play past it — opening, village, tournament readiness, gathering (wood 69 / stone 42 / fiber 36), tent, campfire and bedroll placed — and stalls at the NEXT transition: `'home_built' is done and the tracked objective reads 'Rest at camp and let a creature recover.'; it should have moved on to the beat that says 'Care for your team'`. Same class, one rung further along (Gate 1.x) |
