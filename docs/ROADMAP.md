...
| 2.13 | **Props, fence, signposts, water and terrain form** — the residual half of the blind-judge gap that no vegetation, creature or night task can reach (see §Acceptance). Connect or remove the orphan fence segments at the bridge approach; give the mill visible sails or stop calling it a mill; signposts as set dressing; the smooth primitive dome hill at the village approach; water shading. | Sonnet slices from a Fable composition contract | blind judge names each addressed item as improved, on the same stands |
