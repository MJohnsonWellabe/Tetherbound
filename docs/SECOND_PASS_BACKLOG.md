...
| Item | Criterion | Why deferred | Evidence that exists |
|---|---|---|---|
| Map drawing repair's code-blind recording verdict is outstanding, not passed | CLAUDE visual workflow; map interface legibility | Isolated real-menu frames are available, but the current agent tree rejected two fresh zero-context critics at its thread limit; retain this required review for the next available fresh critic, not an informed substitute | `ralph/reports/FOUR-BIOME-BUILD/MAP-UI-CAPTURE-20260908.md`; 29 map tests/95 assertions pass; full-world reopen validation remains separate and pending |
