...
- Co-op is required, with invitation joining and no router configuration or manual address entry. LAN/direct-IP alone is insufficient for release; the platform invite/relay implementation remains unbuilt.
...
| Invite admission | `session.gd::_rpc_hello` lacks explicit build/content admission, while registry insertion can replace a claimed duplicate character identity. Transport/admission work must preserve the one Session owner and stable characters before new invite UI. Steam binding/export artifacts and actual platform access remain unresolved. |
...
**Art tooling check:** the repo already has `tools/art_pipeline/meshy.py` for image-to-3D generation and manifests; this process currently has no `MESHY_API_KEY`. That is a CLI credential dependency, not a request for art permission or evidence about the web account. No generation was submitted or credit spent during the plan update. Existing accepted replacements should not be regenerated merely to exercise the pipeline.
