...
| Network | `scripts/net/session.gd`, `peer_registry.gd`, `scripts/mp/join_driver.gd`, `encounter_host.gd`, `world_ledger.gd`, `realm_shells.gd`, `realm_transition.gd`, `realm_replication_scope.gd`, `ledger_rpc.gd` | Session constructs ENet peers; identity/capacity admission and readable refusal now precede snapshots on the Meadows payoff branch. One spare transport handshake slot is not a fifth admitted player. Steam lobby/invite/relay and build/content compatibility remain unbuilt. Preserve stable characters, authority, save receipts and both logical traffic purposes; prove compatible Godot4.7 packaging. MULTIPLAYER. |
