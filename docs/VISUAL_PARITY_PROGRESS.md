...
- **VP10 (WORLD)**: `band1_open` stays at **7659 draws vs the 7500 provisional proxy (+2.1 %)**, primitives 11.76M ≤ 12.0M, hall_approach 3844 ≤ 4000. The deliverable is the breakdown: the grass carpet is 133 draws (a MultiMesh is one draw per surface, not per instance) — the brief's leading candidate disproved by measurement; 45.3 % of draws are unattributed (vegetation's true cost unknown); per-group numbers are surface-count upper bounds, not frustum-aware. A `structure_visibility_range` mechanism (per-group `visibility_range_end`, `scripts/world/structure_visibility_range.gd`, `performance.json`) shipped **default-OFF and unmeasured**. **MERGED** `dfddf3fe` (no bake inputs). **WORLD session archived** (cost $290.31). WORLD closing summary in `WORLD/REPORT.md`: passes = sky rework, true angular sun disc (27.4° halo → 2.0°), dawn near ground, night seam, aerial fade key + boot-order fix, canopy white-blob bugs (`adjust_bsc` typo + runtime ImageTexture dropped through PackedScene), the red-wash root cause; limitations = Terrain3D streams around the player (~30 capture tools still drop the player), night far ground bluer/brighter, dawn far plain, VP10 residual.
- **VP10 one-shot (coordinator, owner rule)**: the shipped-but-unmeasured `structure_visibility_ranges` switch gets exactly one measurement: ON at band1_open/hall_approach/village_high with the survey stands pixel-diffed; kept only if band1_open ≤ 7500 with primitives ≤ 12.0M and no visible loss at the stands, else OFF stays and VP10 is recorded at 7659 as the ceiling of the provisional proxy (on-device FPS remains the owner-side measurement).
- Sentries night: render delivered (`round12-sentries-night/`, WIP b387586c); coordinator's own read — both guards now read as warm-lit figures flanking the door at night; the agent's numeric proof is pending. Warrens knob render queued. `smoke_traversal` returned 1 inside the contended final_bake chain (three Godot processes at once) — a serial re-run is queued with output captured before it is called anything.

### Check-in #28 (17:56 UTC) — Warrens slab gone; night sentry one-shot; bake fresh again; WORLD time-boxed

- **Warrens (clean restart)**: ray-cast named the slab (outer face of the mouth chamber's front wall, `material_override` near-white lerp; three lane rounds had set a surface material the override silently beats). Fix = 0.4 m earth-skin cladding on the exterior faces (`site.exterior_cladding_m`), interior untouched. Proof (`round11-warrens/`): slab regions median 102 / 44 → 2.8 / 2.5, 61 % px changed, den unchanged. Judge `JUDGE-round11-warrens.md`: standing PARTIAL (slab gone, flanks now an unlit black void with no grain), approach PARTIAL (clearly improved; a flat grey lintel remains up close), den PASS; FAIL → PARTIAL; **ONE MORE KNOB**. The same agent applies the one agreed knob after its commit: a dedicated `exterior_cladding_colour` (~#4a3a2a) so Ground030's grain survives the shadow; proof = flank medians 20–70 with std ≥ 12; one render, then ceiling.
- **Sentries night**: one Sonnet shot (per-sentry chest-height shadowless omni, data-driven `night_light` block; WIP `ca5eed89`); proof = each guard's box mean ≥ 25 at gate-face-night; then ceiling.
- **CI**: `b805d9d6` red on exactly the stale-bake pair (`verify-scatter-bake-freshness`, `test_scatter_perf_budget`) after CORRIDOR r9's band4 vegetation merge; every other job that ran was green (unit shards 1–3, veg corridor, harvest, scatter rules, gate-A UI, combat, regions, owner regressions, gate evidence; shards cancelled by the next push). Fresh bake committed `414418b8` (825,837 placements); CI running on it → ship PR when green.
- **WORLD**: silent since 17:04 (VP10 "structure visibility ranges, default OFF, not yet measured"; dawn far-plain rendered 16:47, frames never pushed). Time-boxed delivery trigger at 18:00: dawn frames + numbers (proven or reverted), VP10 at its measured state with the kept candidates, one CI commit, closing summary; then archive.
- Local guards on the merged tree: `smoke_stronghold`, `smoke_warrens`, `smoke_relay` exit 0 (PLACES r9/10 site smokes); perf-budget/veg tests + `smoke_authored_camps` + `smoke_traversal` running on the fresh bake.
