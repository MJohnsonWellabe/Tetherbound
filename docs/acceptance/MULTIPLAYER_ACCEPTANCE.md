...
| Item | Evidence | Owner |
|---|---|---|
| Latency / jitter | **MEASURED and PASSING — 5 consecutive runs of `smoke_net_shared_wild_fight` under 150 ms delay / 30 ms jitter / 1 % loss** (2026-09-06), after the measurement found and closed two defects F7 had been standing in front of. 7.A's original F7 (the friendly-fire refusal MESSAGE never arriving while the safety held) is closed by polling for the host's verdict. Then the run failed on **host 96.698 against guest 104.595** — a gap of 7.9, almost exactly one blow, i.e. the guest's copy of the replicated record was one delta behind — so the smoke now asserts CONVERGENCE rather than instantaneous equality, and still fails with the final gap if the two never agree. Then it failed on **`104.5 -> 104.5`**, the joiner unable to land a blow at all: finding F10's spawn-table lottery in the one smoke that never got F10's arm. All three mechanisms verified ENGAGING in a captured run, not merely passing: `stand-in was moved onto the host` ×3, `health bar … within 1 poll(s) … gap 0.000` ×2, `friendly_target after 1 poll`. The remaining owed item is a re-run of the OTHER two smokes 7.A drove under this profile | ☐ |
