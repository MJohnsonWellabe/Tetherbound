...
Cloudreach now has 178 authored world-pickup rows: exactly 100 single-placement Candy pickups, 75 single-placement recovery pickups, and the three existing TMs. The original 19 canonical rows retain their IDs, counts, items, unlock requirements, and runtime overrides; 159 added rows use explicit authored positions, stable IDs, count `1`, and the same realm-qualified one-time persistence path. Unlock flags control spawning. All five camps reuse the production RestPoint/night-rest/autosave/crafting path and installed bed prop. Loading an earlier save reconstructs previously collected caches and removes later unlocked placements. The exact distribution and evidence are recorded in `PICKUP_DENSITY_0905.md`.
...
- `tests/test_cloudreach_physical_runtime.gd`: guards, data completeness, ordered airborne gates, stacked-elevation landing rejection, dialogue/physical separation, all encounter IDs, circuit requirements, all 178 cache identities, five camp contracts and bounded currents.
- `tests/test_cloudreach_pickup_density.gd`: exact owner totals and region/item/lane matrices, canonical-row immutability, stable one-time IDs, unlock contracts, minimum same-region spacing, and clearance from interactions, camps, and battle entries.
...
- `tests/smoke_cloudreach_physical_placements.gd`: instantiates the production Cloudreach scene and checks intended ground under all 215 physical/NPC/camp/pickup/trigger placements.
- `tests/smoke_cloudreach_pickup_density.gd`: instantiates the production pickup runtime, proves the early and fully unlocked populations, collects independent same-item placements, rebuilds the runtime, and verifies their independent persistent removal.

The density checkpoint's focused unit selection passes 92 tests / 7,754 assertions. Its production geometry probe checks all 215 placements with zero failures, and its focused production density smoke passes. The earlier package validation remains: focused unit suite 6 tests / 248 assertions; isolated real-input smoke 33 assertions; combined cast and physical unit selection 13 tests / 1,034 assertions. `git diff --check` is clean for this package.
