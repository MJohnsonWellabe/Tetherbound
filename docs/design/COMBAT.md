...
**Built:** pool40, regeneration20/s after2s, stagger0.6s, next-hit crit1.5, charged-into-telegraph interrupt (`data/config/combat.json::poise`). **Target correction:** pool remains40 points but each landed damage event removes `100 × actual_HP_damage / uninjured_max_HP` points. This normalizes breaks across levels; current raw-damage interpretation must not be mistaken for this proposed rule. Ignore overkill when deriving poise damage. Skills that do no HP damage do no poise damage unless their row explicitly says so.
...
## 6. Third skill and opponent decisions — not built target
...
Required implementation evidence: focused action/poise/resource tests; real varied-size world fights;24-seed team matrix; manual controller/mouse samples; host/client simultaneous-hit and reconnect; two-player and four-player boss; tight-room camera motion; exported build error scan. Main's tests are regression foundations, not proof of new targets. Keep the unmerged combat branch's verdicts explicitly tied to its hash until changes land.

Recovery carried: FINDINGS S05,S16,S27–28, decision records D07/D08/D31/D32/D68/D77 and original COMBAT_DEPTH_PLAN. Explicitly replace raw-damage poise scaling, restore unbuilt skill/AI, retain burst Option B, defer loud types and skill-TM replacement. No automated-play green substitutes for owner feel acceptance. Acceptance must fail if mashing stays equally effective or a move's visible shape lies.
