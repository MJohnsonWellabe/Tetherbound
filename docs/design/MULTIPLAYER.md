...
**Admission implementation update (`ralph/meadows-payoffs`):** `peer_registry.gd::admission_verdict` and `session.gd::_rpc_hello` reject malformed/empty identities, identities owned by a live peer and full registries before insertion, realm preparation or snapshot. Direct registry insertion can no longer evict another live character. A disconnected character can rejoin after its old peer is removed. `join_driver.gd::terminal_admission_reason` preserves the host's specific refusal while leaving transient connection retries intact. ENet reserves one additional transport handshake seat, not an additional admitted player, so a full-session refusal can reach the caller.

**Compatibility and delivery update (`ralph/x05-reconnect-reservation`):**
- **Version check.** Before identity, capacity, realm preparation or snapshot, `session.gd::_rpc_hello` compares the joiner's `build_fingerprint.gd` fingerprint: wire protocol, engine version, and SHA-256 over `res://data/**/*.json`. On a mismatch it refuses with `incompatible_version` and a reason naming what differs. The Steam lobby publishes a short token so a joiner refuses a mismatched friend before dialling.
- **Delivering refusals.** After a refusal, kick, snapshot abort or host exit, the host keeps the link open until the other side closes it (clients close on receipt), for at most 10 s per peer or 5 s for a host close. This replaces the six-frame flush, which ENet's disconnect could discard under loss.
- **Harness evidence.** Measured on the harness loss proxy at 30% loss, 150±30 ms: 4/4 valid runs delivered the reason in 3–6 s, where 2/2 were lost before. This is not a delivery guarantee.
- **Still open:** platform identity authentication.
...
**Implementation branch update (`ralph/invite-coop`): partial, not release accepted.** `scripts/net/steam_lobby.gd` implements optional native initialization, friends-only four-member lobby creation, pending running/cold-start invitations, original-host metadata, deferred peer connection and lobby cleanup. `session.gd` accepts a `MultiplayerPeer` while preserving its registry/snapshot path; `join_driver.gd` closes authority/input/snapshot gates during world preparation. Client leave and fallback autosave cannot write an unadmitted character. The title selects a portable character explicitly for a Steam join; the Players tab opens the host's invite dialog. Stock Godot retains solo/ENet. Snapshot transfer now uses192KiB chunks, a64MiB cap, a60s handshake/snapshot deadline and boundary delta replay after baseline admission, with focused proof and a clean default two-peer ENet host/join/leave smoke at source SHA89f486c652ab. The checksum-pinned Windows development runtime is GodotSteam4.20/Godot4.7/Steamworks1.64, installed by `tools/setup_steam_runtime.py`; a real local native initialization/host/private-lobby probe passed with explicitly configured development AppID480. AppID480 is never the product default or a release entitlement. The native identity-to-claimed-lobby membership and protocol gate is implemented and has focused unit/probe evidence; two-network remote joining/relay proof, four-account play, exact build/content fingerprints, overlay/device behavior and exported templates remain open. Lobby product/protocol strings alone do not establish compatible content or authorize a gameplay peer. `server_relay=true` routes clients through the game host; it is not itself proof that Valve's internet relay was used. Evidence: `ralph/reports/INVITE-COOP/REPORT.md`.
...
Rejoin uses the same invite or Steam `Join Game` route and rebinds the same stable `character_id` to the new peer before reconciling the host snapshot.

- **Reconnect seat.** When an admitted joiner's link drops, the host holds a seat for that `character_id` for `session.reconnect_window_s` (120 s). Another character is refused `session_full` with a held-seat reason, and the same character reclaims the seat under its new peer.
- **Deliberate leave.** A client that leaves deliberately sends a goodbye and the host closes its link, so no seat is held.
- **When the seat starts.** The seat is held only once the host detects the drop. A clean close is detected at once. A crash or cable pull is detected only after ENet's 135–180 s peer timeout, and until then the returning character is refused as `character_in_use`.
- **No countdown.** Do not show a countdown until the UI shows the real remaining time.

On orderly host exit, save the host-owned world and committed portable characters, close the lobby and return peers to title with a host-ended reason. On a host crash or abrupt connection loss, preserve the last durable acknowledged state; do not promise a final host save, acknowledgement or lobby-close callback from a dead process. Clients time out safely, discard uncommitted predictions and reconcile on rejoin. There is no host migration.
...
|---|---|---|
| World state | Host commits flags, time/day, gates, pickups, harvest, placed buildings and their contents, storage, satchels, reward-delivery journals, spawn/defeat state and legendary offer state. | `WorldState`, `WorldLedger`, `ledger_rpc.gd`; D99/D103. |
...
| Shared inventory verbs | Pickup, harvest, storage, building, dismantle, satchel, drop and item trade are versioned host transactions. A stale revision or losing claim is refused explicitly. Stormwood Stormglass arches are judged where they will stand: the host snaps a request inside a footing's 5 m seat to that footing's centre and facing, then checks occupancy, route rules and cost there. Refusals name their cause: `arch_footing` (a non-arch building on an arch footing), `arch_occupied` (another arch already holds the footing or stands within 5 m of its centre, including legacy records without a footing), `arch_locked` (`stormwood_arch_build_rules.placement()` route and progress rules) and `arch_materials`. | `world_ledger.gd`, `ledger_rpc.gd`; D103. |
...
**Shared-wild presentation repair (`ralph/shared-opponent-presentation`): partial.** Previously `join_encounter` fought the guest's nearest ambient wild while the host decided hits against a different body. The new wild-only path waits for admission before constructing `shared_opponent_proxy.gd` from the host's canonical creature card, body scale and alpha appearance. The guest follows host foot position/facing at10Hz with0.05s interpolation half-life; telegraphs and strikes use reliable serial-numbered cues. The proxy never runs wild AI, including after a failed catch, and cleanup bypasses ambient faint/respawn/once-only state. A5s unanswered join cancels and disengages; late canceled records cannot create a fight. Steam protocol is now `tetherbound-invite-v4`; ENet still requires matching builds without a compatibility gate. Source: `encounter_director.gd`, `shared_opponent_proxy.gd`, `water_capture_codec.gd`, `multiplayer.json::encounter.shared_opponent_*`. Evidence and current validation belong in `ralph/reports/INVITE-COOP/REPORT.md`.

**Same-realm lifetime repair (`ralph/shared-wild-lifetime`): partial.** Each host-started wild now has a `shared_wild_host_fight.gd` engine independent of the host's local combat manager. Withdrawal removes one participant; remaining participants retain the body, HP, AI and independent pose/cue counters. The host may rejoin that body or fight another wild. Last withdrawal restores the wounded ambient body; victory/capture retire ecology once. Stored host catch decisions and current claimant/membership/expiry checks control terminal retirement. Local victory XP is guarded once per combat presentation, including participants who did not land the final blow; this is not a new durable reward receipt protocol. Existing network proof and its exact limits belong in `ralph/reports/INVITE-COOP/REPORT.md`.

**Ordinary catch confirmation (`ralph/shared-catch-confirmation`): partial.** Accepted attempts carry a host claim ID; completion revalidates the exact claimant, encounter, membership and expiry. The manager waits for the finish reply before granting the host's canonical creature card. A5s unanswered finish exits without a grant; a30s/64-entry session-local result cache answers exact retries after runtime retirement. Steam protocolv5 supersedesv4 for this wire change. Water Alpha keeps its separate journaled confirmation. Ordinary catches still use the existing party/pending-release path: this is not durable catch delivery across host retirement, disconnect and character-save failure. The full-party target remains unaccepted rather than silently certified by this repair. No new save field or autoload. Source and focused evidence: `encounter_director.gd`, `combat_manager.gd`, `catch_arbiter.gd`, `multiplayer.json::encounter.catch_finish_*`, `ralph/reports/INVITE-COOP/REPORT.md`.

This does not replicate ambient ecology or arbitrate guest-initiated local wild fights. Legacy trainer/boss joins still use their existing presentation path. Ordinary encounter engines still belong to their world director: changing the host's realm destroys that scene without migrating its active ordinary fights. Realm transfer, reconnect and complete catch/save delivery remain release blockers; same-realm continuation alone does not accept shared combat.
...
**Identity contract:** a newly created character receives `character-` plus16cryptographically random bytes encoded as32hexadecimal digits before session registration. Host, client, autosave and manual save retain that identity; save-slot numbers locate worlds and never identify a trainer. Several world slots may reference the same current portable character, so loading another world slot does not fork a second team or restore an earlier character copy. Creation/preservation lives in `character_identity.gd`, `title_screen.gd`, `session.gd` and `save_game.gd`. The existing saved-character two-peer reconnect passes on8379ab1a6; internet/four-peer and reported runtime diagnostics remain open.

**Return-home placement:** a file locator such as `slot-0` cannot identify which host's world a saved pose belongs to. The `ralph/world-return-provenance` correction uses the existing persisted random `reward_delivery_namespace` as world-instance identity and adds character-format4 `last_world_instance_id`. The host creates/retains the world identity; a guest carries the host snapshot's value. Exact saved placement requires matching locator and nonempty instance. Different or unknown provenance uses the selected home slot's authored regional spawn and clears pending foreign entry, while retaining the character's team, items, equipment, rewards and maps. Old character files without provenance stay readable and may be repositioned safely once. This explicitly replaces locator equality as placement proof. **Owner ruling 2026-09-26:** a rejoin to the same host world (matching instance from the host snapshot) restores the character's exact saved pose, subject only to sealed-placement enforcement; the provenance check is made against the host snapshot's instance, not the joiner's local slot. Out of scope: account authentication, remote ownership aliases, legacy character renaming, world cloning and transferring world progress between hosts.

**Death-satchel transaction scope (`ralph/satchel-world-scope`): partially validated implementation.** A new pending row and request carry the same typed nonempty `world_instance_id` before inventory changes. Retry requires matching world instance, locator and character; the host rejects foreign/missing identity before any duplicate handling or mutation. Refusals and recovery snapshots must identify that same world; a wrong-world refusal never refunds. Character5/merged26 prevent older readers from ignoring this boundary. Legacy pending rows stay intact without replay/refund unless the loaded authoritative world holds an exact transaction receipt with the same nonempty owner. A known personal grant/refund can settle once outside its originating world, without another host mutation. An unresolved older move receives a readable notice. Source: `satchel_escrow.gd`, `world_ledger.gd`, `ledger_rpc.gd`; evidence in the portable-identity report. This deliberately replaces locator-only retry and preserves uncertain property instead of guessing. Out of scope: inferred ownership, automatic legacy renaming, world cloning, manual recovery tooling and internet acceptance from local tests.

**Legacy migration remains partial:** existing nonempty valid IDs are preserved, including `slot-N` and `legacy-slot-N`. This prevents a silent rename from orphaning satchels, capture claims and reward receipts, but it does not resolve collisions between old characters from different computers. Those records require an explicit migration with proven local ownership and failure recovery. A remote claim to an old ID alone cannot establish ownership; no alias, inventory recovery or duplicate reward may be inferred from it. This deliberately preserves D100's original-save protection while replacing slot-derived identity for new characters. Out of scope: cloud identity, account recovery, character cloning and implicit ownership transfer.
...
A behind-progress character may join an ahead world and act immediately against that world's state. Personal onboarding or rewards cannot relock a bridge the world has opened. Taking the same character to a second world carries the right column only and changes none of the new world's left column. World-scoped rewards happen once; mandatory personal `reward_grant` items and flags pay once per eligible participant through a `(world-instance namespace, source, character_id)` receipt and are never divided by party size. The persisted random namespace distinguishes two hosts whose save locator is `slot-0`. The host saves its pending journal before publication; the recipient saves an idempotent portable escrow row before ACK. Full capacity keeps the finite authored reward pending until the whole grant fits, and snapshot replay defers character writes until admission.
...
The chapter legendary follows the settled per-participant rule: the freeing and gate are one world fact, while each recorded fight participant has one once-only offer bound to their stable character ID. A character who has answered a freeing — accepted or refused — is never offered that legendary again, in this world or any other. Each accepter receives one distinct creature instance bound to that character; non-participants and refusers receive none. The answer's world receipt (`legendary_resolution:<accepted|refused>:<character>`) is committed only in the world instance where it was given; the personal answer carries that world's identity, so one world's answer never decides another world's disposition, such as the full-refusal herd display. While a fight's participants are unknown (no participant journal) in a multi-peer session, nobody is offered. Reconnect or joining another world must not duplicate an accepted offer. With five creatures, that character completes the permanent release/refusal ceremony before any grant; a sixth slot never exists. Older single-world-recipient wording in D84/D96/FINDINGS is provenance, not the current contract.
...
**Current:** `session.gd::_on_peer_disconnected()` removes the peer's registry row.
- **Seat reservation.** Unless the client said goodbye, the host reserves that character's seat for `reconnect_window_s` in `peer_registry.gd`. Reservations are host bookkeeping: they are not replicated and not in the registry fingerprint.
- **What never reserves.** Kicks, admission refusals and snapshot aborts remove the row first and never reserve. Hellos are refused with `host_closing` while the host is closing.
- **Rejoin.** A rejoin is admitted under a new transport peer ID using the same portable character ID. It loads the character's own durable file and receives the current host world snapshot.
- **Evidence.** `smoke_net_reconnect_reservation` and `test_session_reconnect_reservation`. Reward reconciliation and safe playable restoration still require their scoped evidence.
...
World consequences commit once. Every eligible participant receives authored XP, items and personal flags in full. Items and personal flags use the durable character delivery described above; this implementation does not change the existing XP announcement/receipt path. XP and required items are not divided. A catch is different: one wild becomes one creature owned by the single winning catcher; everyone else receives the identity of the winner and the encounter resolution. Full party still refuses the catch.
...
**Current implementation branch (`ralph/revive-first-tap`):** the multiplayer downed window remains **45s**; one interact tap requests **3s** progress within **2.5m**; success restores35% health/stamina. Solo has no window. `downed_state.gd` now delegates authorization to `scripts/net/revive_authority.gd`: the host checks admitted peers, its current collision-resolved body positions, matching registry/body realms, unchanged body identities and a0.3m horizontal movement deadzone. One active attempt per reviver and per target; monotonic down-window/attempt identities reject stale starts, cancellation replay and duplicate completion. Only host peer1 can deliver a grant for the target's exact current window; the old direct `_rpc_revive` is inert. The reviver displays host progress notices at up to10Hz and retains immediate local cancellation guards. `multiplayer.json::downed` owns timing/range/recovery values. Steam admission uses `tetherbound-invite-v3`; ENet still requires matching builds and has no equivalent version gate. Evidence: `ralph/reports/REVIVE-AUTHORITY/REPORT.md` and the existing two-peer smoke.

**Authority still partial:** remote human health/damage is not independently replicated for host validation; its local guard sends reliable cancellation. Movement remains owner-replicated before host collision resolution. The target still owns45s expiry and the existing death/satchel pipeline; durable disconnected death, same-realm interior residency, encounter-exit authority and complete downed-player progress presentation remain open. This implements host revive authorization, not the complete host-owned death contract.
...
| Water dock actions | **Partial:** `ledger_rpc.gd` saves the validated world change before player cost or delta publication; failed writes roll back without charge and allow retry. `test_reward_delivery_rpc.gd` and `smoke_water_dock_actions.gd` cover this boundary. Paid repair costs still need portable receipts to reconcile a crash between world save and character debit. No distributed atomicity claim. |
...
### Tournament registration boundary

**Partial, current source:** the ordered three entrants are personal durable creature UIDs (`Party.tournament_selection`, merged save27/character6); active fights and their entry-care snapshots remain transient. `encounter_director.gd` checks the sticky five-owned/trained milestone and current selected-three care before local entry or a guest join. A guest waits for the correlated host verdict before creating its combat presentation. The host freezes three announced UIDs for that round, rejects replacement/outside deployment and checks membership before strike/burst; terminal/disengage cleanup releases the round constraint. A loss restores only that player's selected three to full HP and their entry care, without awards or bracket advancement.

Remote portable roster ownership and care are still owner-held metadata. The host enforces announced-roster consistency, not independent possession/care verification. The existing guest trainer presentation still uses a nearby stand-in; this slice does not accept shared tournament play, remote opponent fidelity, reconnect mid-round or router-free invitation delivery. Out of scope for this slice: a new trainer transport, portable-character anti-cheat or resumable active fights. The registered choice survives an ordinary save/load; an interrupted fight does not become a saved victory.
