...
**Current implementation branch:** multiplayer downed window **45s**; one interact tap starts **3s** progress within **2.5m**; success restores35% health/stamina. Solo has no downed window. Commitab9df5fb7 replaced the baseline's held input with tap/cancel and a0.3m movement deadzone. Evidence: `multiplayer.json::downed`, `downed_state.gd`, SYSTEMS§10 and the existing revive smoke. Authority remains partial: `_tick_revive` advances on the reviver, and the target's `any_peer _rpc_revive` checks only local downed state. It does not validate host permission, sender, range or elapsed time; a peer can bypass the local progress checks. This source correction supersedes the stale claim that input is still held, without claiming the host-owned target below is implemented.
