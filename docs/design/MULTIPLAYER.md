...
**Admission implementation update (`ralph/meadows-payoffs`):** `peer_registry.gd::admission_verdict` and `session.gd::_rpc_hello` reject malformed/empty identities, identities owned by a live peer and full registries before insertion, realm preparation or snapshot. Direct registry insertion can no longer evict another live character. A disconnected character can rejoin after its old peer is removed. `join_driver.gd::terminal_admission_reason` preserves the host's specific refusal while leaving transient connection retries intact. ENet reserves one additional transport handshake seat, not an additional admitted player, so a full-session refusal can reach the caller. This does not implement build/content compatibility, platform identity authentication, a120s reconnect reservation, Steam lobbies or invitation transport. The reliable refusal uses the existing six-frame close-flush lifecycle; clean loopback evidence cannot promise delivery under packet loss.
...
**Current:** `session.gd::_on_peer_disconnected()` immediately removes the peer's registry row. A rejoin is admitted under a new transport peer ID using the same portable character ID. `multiplayer.json` declares a **120s** reconnect window, but no120-second reservation is implemented; the old process-lifetime-row description was inaccurate. Reconnect loads the character's own durable file and receives the current host world snapshot. Reward reconciliation and safe playable restoration require their scoped evidence; source presence does not certify every transaction or guarantee a reserved place in a full session.
