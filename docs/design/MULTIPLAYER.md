...
This does not replicate ambient ecology or arbitrate guest-initiated local wild fights. Legacy trainer/boss joins still use their existing presentation path. Host-character withdrawal while guests remain is a separate authority-lifetime defect: preserving an encounter record does not preserve its manager/body simulation. These remain release blockers; synchronized guest presentation alone does not accept shared combat.
