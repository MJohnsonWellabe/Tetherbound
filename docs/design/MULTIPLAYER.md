...
**Identity contract:** a newly created character receives `character-` plus16cryptographically random bytes encoded as32hexadecimal digits before session registration. Host, client, autosave and manual save retain that identity; save-slot numbers locate worlds and never identify a trainer. Several world slots may reference the same current portable character, so loading another world slot does not fork a second team or restore an earlier character copy. The `ralph/portable-character-identity` implementation owns creation/preservation in `character_identity.gd`, `title_screen.gd`, `session.gd` and `save_game.gd`; runtime acceptance remains open until its recorded checks pass.
