...
**Built:** level/stat arithmetic in `scripts/creatures/progression.gd`, instance persistence in `scripts/creatures/creature_instance.gd`, level and XP values in `data/config/progression.json`. **Partial:** the four source ladders and flags exist, but an earned clear and balanced five have not been accepted. **Partial branch slice:** Grandpa's current-party acknowledgement after Water restoration, with personal saved completion (`scripts/story/regional_homecoming.gd`). The full Tidewake return/credits/ending and economy solvency remain unbuilt or unproved. The universal L4 Y skill and revised bond credit are parked candidate designs, excluded by ACCEPTANCE §3.

**Pacing and scope decision:** there is no mandatory 12–16-hour campaign floor or 3–4-hour chapter floor. An earned clear around eight good hours is acceptable. Do not add fights, errands, travel, resource scarcity or XP requirements to fill time. First prove the existing expedition loop in one 15–30 minute agent-piloted check. ACCEPTANCE §3 excludes the universal L4/Y skill, revised bond, normalized poise and Strain from this completion pass. Fix observed pacing and reward failures with current systems before considering a documented spec amendment.
...
|---|---|---|---|
| Meadows | L3 → L21 lead; other retained members within3 levels | `data/config/chapter_curve.json`, five `data/config/bands/*/{spawns,trainers}.json`, Warden18/18/19/19/20 | Named starter and retained team; camp; first TMs; saddle; Mudsnout evolution; Meadowstride. No universal L4 skill gate. |
...
Use observed play to move, combine or remove beats. Preserve the chapter identities and four-biome ending, but cut repetition and compulsory detours before adding content. The opening check is one 15–30 minute agent-piloted expedition through existing systems. Record concrete friction and repair it under the current mechanic scope; it is not the start of a repeated cohort or harness programme.
...
Each chapter retains 6–10 meaningful optional activities, at least one per principal region, as release content rather than a prerequisite for the initial brief loop check or a duration floor. WORLD specifies their identity. A reward is committed with objective completion once. One activity may pay a move plus an acknowledgement; it still counts once. A trainer rematch is not assumed: all31 Meadows trainer entries currently set rechallenge false. Wild spawn count is neither encounter-design variety nor authored runtime. Later catches remain optional; primary rewards should deepen the five the player already loves rather than pressure replacement.
...
Later gates use earned story flags, machinery and physical passages, not an unseen party-average calculation. WORLD §2.5 fixes ordinary wild-site return after two world days and a full-party regional exit. XP, Orb and recovery solvency must pass an earned route with zero repeat wild encounters; respawn is optional ecology and catching access, never mandatory grind.
