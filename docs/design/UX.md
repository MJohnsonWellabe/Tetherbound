...
| Menus/rebinding/glyphs | **Built foundation.** Data menu, controls UI, last-device tracking and UI tokens exist. Settings → Accessibility holds reduced motion, look sensitivity (25–200%) and per-axis look inversion (`look_prefs.gd`), all pad-reachable and saved. | Make every shown glyph rebound-aware; complete 720p and accessibility validation. Subtitle size/background, camera-shake level and aim-assist strength remain unbuilt settings. |
...
| Onboarding/readiness | **Broad sequence built, experience unaccepted.** First camp uses one bed; the journal and Halda then require three for first tournament entry. | Preserve exact one-time kit; prove earned three-bed preparation, shelter, level/feed/rest guidance and bracket retry/recovery at720p/1080p. |
