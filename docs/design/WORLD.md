...
| Lower Meadows: cart / `band1_broken_cart` | Coll and damaged cart visible off bridge road; deliver the source item-gate cost once. | Target visibly repaired cart/usable approach. Current `cart_repair.gd::_on_tried` spends materials and sets completion but grants no item/currency reward and does not change the cart mesh. The earlier "existing repair reward" wording was inaccurate; no new currency payout is authorized by this row. |
