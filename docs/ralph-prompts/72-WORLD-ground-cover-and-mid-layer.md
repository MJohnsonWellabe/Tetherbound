...
Provenance and the limits on using these are in `docs/ASSET_LEDGER.md`. They are
...
5. **A landmark in the middle distance** where a band lacks one — the rock hill
   in frames 02/03 is what gives the eye somewhere to go. `ralph/BACKLOG.md`
...
- `PERF-ROG-GPU` in `ralph/BACKLOG.md` says plainly that this container cannot
...
Per `ralph/conventions.md`, this is visual-affecting work and the blind pass is
law, not a formality:
...
- Re-bake scatter and `--headless --import` after it. A bake that does not reach
  a capture is a documented trap in `ralph/conventions.md`.
