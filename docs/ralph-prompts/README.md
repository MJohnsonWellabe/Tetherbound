...
`ralph/ACTIVE_GAME_PLAN.md` contains the complete primary mapping of the existing prompt library into gameplay gates, and `ralph/PROMPT_COMPATIBILITY_MAP.md` resolves overlapping historical prompt files.
...
Their current primary placement is recorded in `ralph/ACTIVE_GAME_PLAN.md` rather than duplicated here.
...
Use `ralph/PROMPT_COMPATIBILITY_MAP.md` to merge their unique requirements into the current canonical prompts/packages and implement each issue once.
