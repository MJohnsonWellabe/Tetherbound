...
**Orientation was wrong on three of the four and fixed post-generation, not in the prompt.** Meshy's raw exports do not reliably land upright; per-object rotation was determined empirically (render six axis-aligned views, apply the rotation that makes the object stand as intended, bake it into the exported mesh — never left as a magic number for the instancing site to carry) and every object was then re-centred on X/Z with its base sitting at Y=0, matching how a ground pickup is normally authored:

- `candy_pickup`: generated already lying naturally on its side (correct for a rounded wrapped-candy pickup resting in grass — matches the board's own in-world context shots). No rotation needed.
- `mushroom_pickup`: generated on its side; `rot_x(+90°)` stands it up (verified by rendering all six axis views first — the "top"/"bottom" views showed a readable cluster, the four `elev0` side views did not).
- `revive_flower` / `potion_plant`: generated lying on their side **and** upside-down; `rot_z(-90°)` then `rot_x(180°)` (a proper rotation, not a mirror, so normals/winding survive) stands each one up with its bloom/buds pointing away from the soil rather than into it.
