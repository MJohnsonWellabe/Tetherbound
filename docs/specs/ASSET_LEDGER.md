...
|---|---|---|---|---|---|---|
| Ironwood ancient-tree Meshy reference, selected 2026-09-11 | OpenAI built-in ImageGen, directed by Codex for this private project | Three generated candidates; independent code-blind selection report `ralph/reports/BROAD-VISUAL-0910/IRONWOOD-ANCIENT-TREE-REFERENCE-BLIND-0911.md` ranks A > C > B | Proprietary project art; generated in the owner's OpenAI workspace | No Meshy credit in this row | `docs/art/reference/49_Ironwood_Ancient_Tree.png`; Meshy primary crop `docs/art/reference/49_Ironwood_Ancient_Tree_Meshy_Front.png` | Owner renewed the Meshy budget and authorized new art on 2026-09-11. Candidate A won for colossal mass, grounded buttress roots, coherent non-pom-pom crown and gameplay-distance readability. The full three-panel board is retained for provenance; because its panels are related takes rather than a consistent turntable, only the clean left/front panel is cropped as the reconstruction input. No generated mesh is shipped by this row; preview/refine task IDs and measured credit spend must be added when a mesh is selected. |
| Ironwood ancient-tree production landmark | Meshy multi-image-to-3D, directed from the selected agent-generated reference above | `tools/art_pipeline/meshy.py`; preview tasks `01a09257-9aaa-7242-8962-127d600e5ae2`, `01a09257-a1fe-7636-b39a-de0732411da8`, `01a09257-a936-7214-b710-9cc28aa2bcf4`; refine tasks `01a09265-f5f1-7432-a481-98234f7f0275` (selected A), `01a09266-025e-747f-a972-3b2255e1c346` | Meshy's standard generation terms; derived from proprietary project reference art | Yes — 120 credits measured, balance 13,925 -> 13,805 | `assets/environment/meadows/ironwood/ironwood_ancient_tree.glb` | Selected through independent blind review at both preview and refine stages. Installed only as a collisionless static landmark: 23,425 rendered triangles, one material, no armature/clips. The raw topology remains disconnected/non-manifold and is explicitly unsuitable for deformation or collision; production R7 shows no catastrophic visible import corruption. The procedural hero remains a load-failure fallback. In-world blind verdict is POLISH, a clear R6 FAIL -> POLISH promotion; remaining blockers are workyard storytelling, arrival clutter and night/material polish. |
| **CANDIDATE, not shipped** — Kenney Nature Kit v2.1, 10 `cliff_*_rock.glb` modules (block, large, top, corner, cornerLarge, half, diagonal, blockSlope, blockCave, large_stone) | Kenney | [Nature Kit](https://kenney.nl/assets/nature-kit) (zip verified HTTP 200, 10.5 MB, 2026-09-06) | CC0 1.0 (pack `License.txt`, copied alongside as `LICENSE_kenney_nature_kit.txt`) | No | `assets/environment/kenney_nature_candidates/` | None. Staged only for the CLIFF-ART-0906 option C render (`ralph/reports/CLIFF-ART-0906/`); referenced by `visual.geology.crag_kit` when that option is active, unused otherwise. Delete the folder if the owner picks A or B. |
| **CANDIDATE, not shipped** — Poly Haven `mossy_rock` 1k colour + GL normal | Poly Haven | [mossy_rock](https://polyhaven.com/a/mossy_rock) via `dl.polyhaven.org` (fetched 2026-09-06) | CC0 1.0 (Poly Haven blanket licence) | No | `assets/environment/terrain/candidates/mossy_rock_*_1k.jpg` | None. Staged only for the CLIFF-ART-0906 option B render; referenced by `visual.geology.albedo/normal` when that option is active. Delete if the owner picks A or C. |
...
| Legendary Tether Machine — production hero object | Generated (Meshy multi-image-to-3D: preview x3 rejected, preview x3 accepted, then refine x2) from four crops of the owner's board `15_Legendary_Tether_Machine.png` — **and the front and side crops had the bound legendary programmatically LIFTED OUT of them first** (`crop_prop_views.py::lift_occupant`) | `tools/art_pipeline/`, task ids in `assets_raw/tether_machine/*/provenance.json`; winner `01a00ae7-0325-7f01-b8db-07097a7929e1` | Meshy ToS at generation date (paid credits of owner's account); derived from owner's own concept art. All Rights Reserved / proprietary, owner-licensed | Paid credits (200: 6 preview + 2 refine) | `assets/environment/team_tether/tether_machine.glb` | R8.2. D24's third and last reserved hero. **THE BOARD LICENSES THE MACHINE, NOT ITS PRISONER** — D24, and D23 §20 forbids a new creature mesh at any balance. The occupant was removed from the INPUT IMAGES rather than only banned in the negative prompt, because image-to-3D follows its pictures. Every candidate was checked by eye and none contains a creature. Installed at `machine.model` in `data/config/stronghold.json`, fitted to `machine.height` (15m) — `smoke_stronghold` measures it at 16.6 x 15.0 x 12.0 m. In-engine renders: `shots/_hero_machine_*.png`. **RESTYLED 2026-09-06 (TETHER-MACHINE-0906) — albedo and roughness only, geometry, UVs and vertex count untouched; NO Meshy generation, no credits spent, no emissive added.** Three blind-judge rounds, verdicts at `ralph/reports/TETHER-MACHINE-0906/JUDGE-machine-round{1,2,3}.md`. `tools/art_pipeline/regrade_tether_machine.py` maps the albedo through a value-keyed ramp (warm dark stone -> `stronghold.json::site.stone` #6a6157 -> board 15's own BRASS/GOLD on lit trim) with the baked micro-detail flattened by a median pass and soft quantisation; `roughnessFactor` 0.80 -> 0.93 to match the Hall masonry (`stronghold.gd::_material`). The GLB carries `extras.tetherbound_albedo_regrade` because the ramp is NOT idempotent. **Outcome: the style-mismatch half of the H6 gap is closed, the silhouette half is not, and the evidence says it is not closeable from this asset's textures.** Round 5's "grey-green mass in an orange room ... as if it were lit by a different scene" is gone — two independent judges now open with "not a palette mismatch. The opposite" (machine median hue 35.5 deg vs wall 27.7). Surface churn down ~30% (gradient energy 13.32 -> 9.37 against the wall's 3.98). But three grades bracket and exhaust the lever: albedo medY 80.7 rendered brighter than its own room, 45.6 rendered statistically identical to the wall (Cohen's d 0.025-0.101), and the shipped 44.3 holds the board's dark mass (78.9% below luma 70 vs the board's 64.3%) with the highlight end restored (p90/p95 93.3/134.9). Round 3 proved the remaining silhouette failure is LIGHTING, inside one frame set: same mesh, same albedo, T-03 scores Michelson 0.652 against C-02/C-03's 0.234/0.303 purely because the machine is silhouetted against a TORCH-LIT wall — "the failure in the other two is a lighting/staging failure, not only a mesh failure". A roughness change was also measured and did NOT move the surface read (top-5% sat 0.426 -> 0.425): under gl_compatibility the specular lobe is negligible and the blown highlights are baked in the Meshy albedo. **Note the handoff's H4/H6 premise of "cap the installed machine GLB's emissive" does not apply to this asset**: it carries ONE material, `metallicFactor` 0, a base-colour texture, and no `emissiveFactor` and no emissive texture at all — the chamber's teal key is `_machine_shell`'s CoreLight omni and the room's conduits, so the albedo-carried read of ledger line 62 is already what this mesh does. Nor is the board missing: it is `docs/art/reference/15_Legendary_Tether_Machine.png`, in the repo, and the judges measure the built object against it as carrying three of its features and NONE of its six key materials (0.00% teal against the board's 18.06%; zero chains, brass banding, runes or clamps; the containment ring is a zero-thickness white quad). `machine.facing_deg` (-101.2, OP-0905-16), `machine.height` (15.0) and `core_light` unchanged; `smoke_stronghold` green throughout at 16.6 x 15.0 x 12.0 m with the doorway sightline clear. |

> **ROAD-VISUAL 2026-09-07 machine-fit amendment.** Veridian's owner-directed
> 5.8m live scale makes its 1.22x bound presentation about 7.08m tall. The
> smaller licensed machine therefore grows from 15.0m to 19.5m and the chamber
> from 22m to 23m; `smoke_stronghold` measures the fitted machine at
> 21.5 x 19.5 x 15.6m, its cage void at 7.67m, and the creature with 0.59m
> internal headroom. This supersedes the old 15m/16.6 x 15.0 x 12.0 runtime
> measurement in the production-machine row above; no asset, source, licence,
> or Meshy spend changed.
>
...
the blind critic has said so in two consecutive reviews (`archive/reports/docs-reviews-full/MA-01`,
...
## Restored core environment provenance (2026-09-04)

These rows restore the authoritative provenance that was present in commit
`87be93f20:docs/ASSET_LEDGER.md` but was lost before the ledger moved to
`docs/specs/`. Counts and use notes are reconciled to the files installed now;
newer task-specific entries elsewhere in this ledger remain authoritative for
their own later additions and modifications. This restoration was prompted by
the Cloudreach reuse audit. It documents reusable assets only: the supplied
Cloudreach concept board is not present in this worktree, and these rows make no
claim that Cloudreach visual parity has been established.

| Asset | Creator | Source | Licence | Paid | Local path | Modifications |
|---|---|---|---|---|---|---|
| Stylized Nature MegaKit — 46 installed models | Quaternius | [Stylized Nature MegaKit](https://quaternius.com/packs/stylizednaturemegakit.html) | CC0 1.0 | No | `assets/environment/stylized_nature/*.gltf` | Trees, bushes, ferns, grass, flowers, mushrooms, rocks and path stones, with shared textures and normal maps. The historical row recorded 40 initially curated models; the current installed set contains 46. `Bush_Common` ships with the crimson autumn leaf texture and is re-pointed at the green one per layer by `data/config/vegetation.json`; later derived leaf variants under `derived/` are local texture transforms, not newly sourced assets. |
| Kloppenheim 05 Pure Sky (HDRI) | Greg Zaal / Poly Haven | [Poly Haven](https://polyhaven.com/a/kloppenheim_05_puresky) | CC0 1.0 | No | `assets/environment/sky/day.hdr` | 2K Radiance HDR; midday, partly cloudy, sky-only. Still installed, but retired from the live world-look configuration because its photographed sun cannot track the runtime directional light. |
| Table Mountain 1 Pure Sky (HDRI) | Greg Zaal / Poly Haven | [Poly Haven](https://polyhaven.com/a/table_mountain_1_puresky) | CC0 1.0 | No | `assets/environment/sky/golden.hdr` | 2K Radiance HDR; sunset, partly cloudy. Still installed, but retired from the live world-look configuration for the same sun-tracking reason as `day.hdr`. |
| Nature Kit — 27 installed models | Kenney | [Nature Kit](https://kenney.nl/assets/nature-kit) | CC0 1.0 | No | `assets/environment/nature/*.glb` | Superseded by the Quaternius nature kit and retained mainly for flower/log shapes. The current material defect and safe-use limitation are documented in the earlier `assets/environment/nature` defect-note row; this provenance row does not override that warning. |
| Ultimate Furniture — 13 of 20 models | Quaternius | [Ultimate Furniture](https://quaternius.com/packs/ultimatefurniture.html) | CC0 1.0 | No | `assets/props/quaternius_furniture/*.obj` | OBJ+MTL beds, tables, chairs, bookcases, closets, desk, nightstand and door used for interiors/camps. Modern sofas and office chair were not curated. `Bookcase_Books.obj`'s `mtllib` reference was corrected to the sibling MTL's actual filename. The source pack is authored at roughly twice real-world scale; placement code carries the correction. |
| Survival Pack — 5 of 25 models | Quaternius | [Survival Pack](https://quaternius.com/packs/survival.html) | CC0 1.0 | No | `assets/props/quaternius_survival/*.obj` | OBJ+MTL `Axe`, `Backpack`, `Bonfire`, `Bonfire_Fire` and `Knife`. Modern batteries, flare gun and first-aid pieces were not curated. This is distinct from the newer Kenney Survival Kit 2.0 row above. The same approximate 2x source-scale correction applies. |
| Grass005 — colour, normal and roughness | ambientCG | [ambientCG Grass005](https://ambientcg.com/view?id=Grass005) | CC0 1.0 | No | `assets/environment/terrain/Grass005_*.jpg` | 2K source downsampled to 1024×1024 so Terrain3D can include it in the shared `Texture2DArray`. Base ground layer. |
| Grass004 — colour, normal and roughness | ambientCG | [ambientCG Grass004](https://ambientcg.com/view?id=Grass004) | CC0 1.0 | No | `assets/environment/terrain/Grass004_*.jpg` | 2K JPG set. Former base layer, retained as a dry-grass/highland option. |
| Ground003 — colour, normal and roughness | ambientCG | [ambientCG Ground003](https://ambientcg.com/view?id=Ground003) | CC0 1.0 | No | `assets/environment/terrain/Ground003_*.jpg` | Soil/slope set. `Ground003_Color.jpg` was locally desaturated by `tools/art_pipeline/desaturate_soil_texture.py` to reduce the source photo's pervasive green saturation; normal and roughness are unmodified. CC0 permits the derivative. |
| Rock030 — colour, normal and roughness | ambientCG | [ambientCG Rock030](https://ambientcg.com/view?id=Rock030) | CC0 1.0 | No | `assets/environment/terrain/Rock030_*.jpg` | Slope-rock set. `Rock030_Color.jpg` received a local screen-blend brightness lift via `tools/art_pipeline/brighten_rock_texture.py`; normal and roughness are unmodified. CC0 permits the derivative. |
| Ground030 — colour and normal | ambientCG | [ambientCG Ground030](https://ambientcg.com/view?id=Ground030) | CC0 1.0 | No | `assets/environment/terrain/Ground030_*.jpg` | 1K dirt/pebble pathway set. The colour was locally desaturated in moss regions and its largest repeated moss blobs reshaped by the committed art-pipeline scripts; the normal is unmodified. CC0 permits the derivatives. |
| Ground037 — colour and normal | ambientCG | [ambientCG Ground037](https://ambientcg.com/view?id=Ground037) | CC0 1.0 | No | `assets/environment/terrain/Ground037_*.jpg` | 1K damp, mossy forest-floor/highland-grass set; installed as an available Terrain3D layer. |
| Medieval Village MegaKit — 176-model source pack | Quaternius | [Medieval Village MegaKit](https://quaternius.itch.io/medieval-village-megakit) | CC0 1.0 | No | `assets_raw/vendor/quaternius_medieval-village-megakit/` | Owner-supplied download, 2026-08-11. glTF export retained as the source set; redundant FBX/OBJ exports were not staged. The directory is raw/vendor input, not a claim that all 176 models ship. |
| Medieval Village MegaKit — 64 installed models | Quaternius | [Medieval Village MegaKit](https://quaternius.itch.io/medieval-village-megakit) | CC0 1.0 | No | `assets/buildings/quaternius_medieval/*.gltf` | Curated plaster/uneven-brick walls, roofs, gables, timber corners, doors, windows, shutters, chimneys, fences, floors, exterior borders, vines, rubble and wagon modules plus shared trim textures. `data/config/building_prefabs.json` composes the settlement/build prefabs. The later T1-HALL-ART row remains the specific addendum for the four Hall ruin modules and its 2026-08-30 licence re-verification. |
| Fantasy Props MegaKit — 94-model source pack | Quaternius | [Fantasy Props MegaKit](https://quaternius.itch.io/fantasy-props-megakit) | CC0 1.0 | No | `assets_raw/vendor/quaternius_fantasy-props-megakit/` | Owner-supplied raw glTF source pack; redundant export formats were not retained. The raw directory is not a claim that every source model ships. |
| Fantasy Props MegaKit — 16 installed models | Quaternius | [Fantasy Props MegaKit](https://quaternius.itch.io/fantasy-props-megakit) | CC0 1.0 | No | `assets/props/quaternius_fantasy/*.gltf` | Current curated set of work, farm, furniture and route-dressing props, including the installed `Rope_1` bridge-dressing option. Models retain the pack's shared trim textures and are placed/configured through the repo's prop/build systems. The historical row recorded the initial ten-model slice; this reconciles the present installed count without replacing later placement records. |
| **HALL-ART-0906** Fantasy Props MegaKit — 28 further installed models (`Torch_Metal`, `Cauldron`, `CandleStick_Stand`, `Barrel_Holder`, `Crate_Metal`, `Chest_Wood`, `Peg_Rack`, `Shelf_Simple`, `WeaponStand`, `Chain_Coil`, `Cage_Small`, `Pouch_Large`, `Rope_2`, `Rope_3`, `Table_Large`, `Stool`, `BookStand`, `BookGroup_Medium_1`, `Vase_Rubble_Medium`, `Bucket_Metal`, `Dummy`, `Pot_1`, `Lantern_Wall`, `Stall_Cart_Empty`, `Shield_Wooden`, `Sword_Bronze`, `Anvil`, `Bookcase_2`) | Quaternius | [Fantasy Props MegaKit](https://quaternius.itch.io/fantasy-props-megakit) — installed from the already-vendored source pack at `assets_raw/vendor/quaternius_fantasy-props-megakit/glTF/` (the row above), **not downloaded**: nothing left this machine for these files | CC0 1.0 (per the pack row above; `License_Standard.txt` ships in the vendored directory) | No | `assets/props/quaternius_fantasy/*.gltf` | None — `.gltf` + `.bin` copied verbatim out of the vendored pack. No textures were copied: all 28 reference only the pack's shared `T_Trim_Metal/Furniture/Props/Cloth_*` images, which this folder already carried for the first 16. Installed to close HALL-ART-0906's H1 (a real iron sconce for `_wall_torch`), H2 (per-room interior dressing through `hall_occupation.dressing`) and X1 (a brazier built from `Cauldron` on a `CandleStick_Stand`, rather than modelling or downloading one). Placed through `stronghold.gd::_build_hall_dressing()` from `data/config/stronghold.json`. |
| Fantasy Props MegaKit — CLOUDREACH-DRESS-0906 install (+16 models) | Quaternius | [Fantasy Props MegaKit](https://quaternius.itch.io/fantasy-props-megakit) | CC0 1.0 | No | `assets/props/quaternius_fantasy/{Torch_Metal,CandleStick_Stand,Barrel_Holder,Cage_Small,Chain_Coil,Lantern_Wall,Banner_1,Banner_1_Cloth,Banner_2,Banner_2_Cloth,Rope_2,Rope_3,WeaponStand,Cauldron,Shield_Wooden,Crate_Metal}.gltf` | Installed from the already-vendored 94-model pack row above (`assets_raw/vendor/quaternius_fantasy-props-megakit/glTF/`) — copy of the `.gltf` + `.bin` only; every material resolves against the shared `T_Trim_{Metal,Furniture,Cloth}_*` textures already installed alongside the previous 16 models, so no new texture files were added and nothing was downloaded. Installed to dress the Cloudreach summit approach and final arena (C6), the aviary interior (C7) and the cliffside cottages (C8). Audited on install: no material relies on the absent-`metallicFactor` glTF default (each carries an ORM `metallicRoughnessTexture`), so none imports as a black full-metal silhouette. Brings the installed count from 16 to 32. |
| Medieval Village MegaKit — CLOUDREACH-DRESS-0906 install (+14 models) | Quaternius | [Medieval Village MegaKit](https://quaternius.itch.io/medieval-village-megakit) | CC0 1.0 | No | `assets/buildings/quaternius_medieval/{Balcony_Simple_Straight,Balcony_Simple_Corner,Balcony_Cross_Straight,Balcony_Cross_Corner,Prop_MetalFence_Simple,Prop_MetalFence_Ornament,Prop_Vine4,Prop_Vine5,Prop_Vine6,Prop_Vine9,Roof_Log,Prop_Crate,Prop_Brick3,Prop_Brick4}.gltf` | Installed from the already-vendored 176-model pack row above (`assets_raw/vendor/quaternius_medieval-village-megakit/glTF/`) — `.gltf` + `.bin` only; all materials resolve against the shared medieval trim textures already installed with the previous 64 models, so no new texture files and no download. Installed so the Cloudreach cliffside cottages read as a windswept clifftop settlement rather than the Meadows village moved uphill (C8): balconies and vines on the exposed faces, buttress/brick stubs at the footings, metal fence rails along the terrace edges. Same metallic audit as the row above: clean. Brings the installed count from 64 to 78. |
| Modular Medieval Buildings — 30-model source pack | Quaternius | [Modular Medieval Buildings](https://quaternius.com/packs/modularmedievalbuildings.html) | CC0 1.0 | No | `assets_raw/vendor/quaternius_modular-medieval-buildings/` | OBJ+MTL source pack fetched through Quaternius's public download. The FBX/Blend duplicates were not retained. Fortress vocabulary includes towers, walls, entrances, bridge, tunnel, banner and related props. |
| Modular Medieval Buildings — 21 installed models | Quaternius | [Modular Medieval Buildings](https://quaternius.com/packs/modularmedievalbuildings.html) | CC0 1.0 | No | `assets/buildings/quaternius_castle/*.obj` | Curated wall, gate and tower geometry used by `data/config/building_prefabs.json` and the stronghold. In-engine scale was confirmed. Every source MTL colour is the same placeholder grey, so production use depends on the existing retint/material-treatment path; this provenance row does not classify the raw material appearance as finished. Nine source-pack models, including `Bridge` and `Tunnel`, were never installed. |
...
the trap `docs/AGENT_WORKFLOW.md`'s art-pipeline section already documents.
...
**Provenance gap this lane found and did not create.** At the time this row was
written, neither the full `quaternius_medieval/` installation nor
`quaternius_castle/` had a surviving ledger row. The 2026-09-04
`Restored core environment provenance` section above closes that historical
gap from commit `87be93f20`; this T1-HALL-ART row remains the narrower addendum
for the four modules newly used by the Hall and its later licence re-check.
...
## Meadows pickup props — candy, revive flower, potion plant, mushroom (2026-09-04)

| | |
|---|---|
| Source | Meshy.ai multi-image-to-3D, preview then refine tiers, plus one installed-asset decision (mushroom) |
| Reference | `docs/art/reference/17_Candy_Revive_Potion_Mushroom_Pickups.png`, owner-supplied 2026-09-04 (a ChatGPT-produced production board, uploaded in-conversation as part of a six-board handoff pack; the other five — `18_Signpost_Bridge_Modular_Props.png`, `19_Meadows_Asset_Boards_Visual_Direction.png` plus a bridge board, a signpost board and a broken-wall/campsite board not carried into this repo — are recorded where they were used or deferred, below) |
| Licence | Meshy paid generation on the owner's own account, from the owner's own reference board. All Rights Reserved / proprietary, owner-licensed, same footing as every other Meshy row above |
| Owner authorisation | Direct instruction, 2026-09-04: *"we should use meshy to design the candies, mushrooms, revive flower and potion plants."* Satisfies `CLAUDE.md`'s reference-art gate the same way the TM Orb and camp-set rows above do — the owner directing a generation with a supplied board, not an autonomous spend. |

**One mesh per family, not one per tier** — owner correction, same session: *"I don't think the candy's need three props... one and color it different and add a different badge... flowers and mushrooms similarly."* `candy_pickup` covers Good/Great/Rare by material tint plus a medallion-decal swap (Rare's wings are real geometry the board draws, not a texture; add them as separate small child-node meshes in Godot rather than a second generation). `mushroom_pickup` covers Speed/Stamina by tint; Wild Shroom's broader cap is a non-uniform scale, not a second mesh. This is the same economy the TM Orb row above already established for this pipeline ("ONE MESH, TEN MATERIALS").

| Subject | Path | Task id(s) | Credits |
|---|---|---|---|
| `candy_pickup` | `assets/props/candy_pickup/candy_pickup.glb` | preview `01a06d37-b18b…` / `…-b577…` / `…-bad2…` (3 candidates, rejected), refine `01a06d40-906b-7742-b2dd-37790d7cc851` (installed) | 60 preview + 30 refine = 90 |
| `revive_flower` | `assets/props/revive_flower/revive_flower.glb` | preview `01a06d37-c09c…` / `…-c52f…` / `…-c975…` (3 candidates, rejected), refine `01a06d40-95d7-70ae-914d-a2c625affe05` (installed) | 60 preview + 30 refine = 90 |
| `potion_plant` | `assets/props/potion_plant/potion_plant.glb` | preview `01a06d37-d01b…` / `…-d483…` / `…-d98c…` (3 candidates, rejected), refine `01a06d40-9bbb-7176-b385-f2d24ff4e1aa` (installed) | 60 preview + 30 refine = 90 |
| `mushroom_pickup` | `assets/props/mushroom_pickup/mushroom_pickup.glb` | preview `01a06d37-dfda…`/`…-e421…`/`…-e967…` (3, rejected: fused with base-grass clutter into a spiky rock); retry `01a06d3b-ee58…`/`…-f5b1…` (2, tighter crop, still rejected: mixing two different mushroom photos as "one object, two angles" produced a rounded blob); `01a06d3e-1952-74b1-8e26-a3982aa90d4f` (1, single reference duplicated as both views — **installed**, owner confirmed the raw grey mesh "looks fine, just needs to be rotated and colored") | 60 + 40 + 20 = 120 |

Total: 390 of 515 credits available when this batch started (owner-stated balance ≈500; `meshy.py check` measured 515 at the first call). Balance after: 125.

**Every candidate here is untextured-preview-tier geometry review followed by exactly one refine call on the winner** — no candidate hand-picking between preview and refine (refine is an independent generation against the same references/prompt, not an upgrade of a specific preview task, `tools/art_pipeline/meshy.py::cmd_generate`), matching this file's own "preview first, spend only on the winner" rule from the TM Orb row.

**CORRECTED, same day.** This section originally claimed three of the four needed a rotation fix, determined by rendering six axis-aligned views in `matplotlib`'s `mplot3d` and picking the one that looked upright. **That diagnosis was wrong and the "fix" broke three good meshes.** `mplot3d` treats its own plot-Z as "up" by default, which is not glTF's Y-up convention; eyeballing "looks vertical in matplotlib" was not the same question as "is this Y-up correct", and the rotations baked in on that basis (`rot_x(+90°)` for the mushroom, `rot_z(-90°)` then `rot_x(180°)` for the flower and potion plant) tipped all three onto their sides in every renderer that follows the real convention — Blender (which both `turntable.py` and Godot's glTF importer treat the same way) and, unverified until this correction, Godot itself.

The bug was found by finally rendering through `tools/art_pipeline/blender/turntable.py` — real Blender lighting, the same renderer this whole pipeline already trusted for creature candidates — rather than the matplotlib preview. All three raw sources (`assets_raw/{mushroom_pickup,revive_flower,potion_plant}/{build/clean,refine_a/model}.glb`) were, in fact, **already correctly Y-up on generation**, confirmed with a new small probe tool, `tools/art_pipeline/blender/probe_rotation.py` (imports a glb, applies a candidate rotation, renders one still — for exactly this kind of check, in the renderer that matters, not a plotting library's default camera). Re-exported from the untouched raw sources with **no rotation, only the base-anchor** (recentre X/Z, base at Y=0) — the part of the original transform that was always correct. `candy_pickup` was right all along, as originally recorded.

**The lesson, stated so it does not recur:** never judge 3D orientation, shape or colour through `matplotlib`/`mplot3d` on this project. It does not share glTF's up-axis convention and a render that looks right there can be wrong everywhere else. Use `turntable.py` (full four-angle, lit) or `probe_rotation.py` (quick single-angle check) — both drive the real Blender import path.

**`assets/creatures/tetherbound/{candy_pickup,revive_flower,potion_plant,mushroom_pickup}/reference/`** hold the generator-input crops (`front`/`three_quarter` or `front`/`side`), per `tools/art_pipeline/prop_views.json`'s new entries — kept as the pipeline's standing convention (every generated species/prop keeps its crops there, `.gdignore`'d from the shipped build) even though these are props, not creatures, because that is the directory `meshy.py::reference_views()` reads from for every subject in `PROPS` alike.

**Mushroom used Meshy only after checking for an installed asset second, not first** — a process gap worth naming plainly: `assets/environment/nature/mushroom_red.glb` and `assets/environment/stylized_nature/Mushroom_Common.gltf`/`Mushroom_Laetiporus.gltf` already existed in the installed nature pack and were only discovered *after* three failed preview rounds (120 credits) chasing a generated mesh. The owner reviewed the final generated candidate directly and confirmed it over switching to the installed asset (*"do not use the already included mushrooms. finish these"*), so the generated mesh shipped — but the art-source order (installed asset → free pack → Meshy) should be checked **before** a generation is queued, not after it starts failing, and the next session that needs a small nature prop should search `assets/environment/` and `assets/props/` first.

**Deferred, not generated: `meadows_signpost` and `meadows_bridge_section`.** Reference crops from `docs/art/reference/18_Signpost_Bridge_Modular_Props.png` are staged in `tools/art_pipeline/prop_views.json` and `SPECIES_PROMPTS` (`meshy.py`), and both names are registered in the `PROPS` set, so a future `generate` call needs no further setup — but per owner instruction the same session (*"I wonder if the signs and bridge could actually be created without meshy by editing what we have to look more like the reference art"*), no credit has been spent on them. See `docs/prompts/74-ART-REFERENCE-owner-boards-for-meshy.md` for the procedural-first task.

**Not carried into the repository: the bridge board, the signpost board, and the broken-wall/campsite board** from the same six-board upload. Board 18 (`Signpost_Bridge_Modular_Props.png`) already covers signpost and bridge in one image at production-board quality, superseding the two separate single-object boards from the same handoff; the broken-wall/campsite board was out of scope for this session's instruction (candy, mushrooms, revive flower, potion plants, "then... the signs and bridge") and is not referenced anywhere in the codebase — recorded here so a later session does not go looking for files that were never committed.

**`mushroom_pickup` received a vertex-color material pass, same session, no Meshy spend.** The raw preview-tier mesh is untextured (a single flat colour), and the owner's own review of the render called it out: *"it just needs to be rotated and colored... give the top polka dot type design."* Painted directly onto the mesh's existing vertices (`ColorVisuals`, `vertex_color_use_as_albedo` reads it in Godot with no texture/UV dependency) rather than a texture, because the topology is one voxel-remeshed manifold with no UV unwrap worth trusting: cap-top vertices (upward-facing normal, upper 58% of the object's height, so the low base/fin geometry is excluded) get a saturated orange-red; downward-facing (gill) and the remaining near-vertical (stem) vertices get two different creams; a deterministic multi-frequency sine hash over cap-vertex position (`np.percentile` at the 78th, so results are reproducible from the same seed rather than `random`) paints the top ~22% of cap vertices a pale near-white for the spotted-cap read the board asks for. This is a coloring pass on the geometry already installed above, not a new generation — the mesh, task id and credit spend are unchanged from the table above.

**Wiring into gameplay (spawn placement, per-tier material/medallion swap, the Rare Candy wing meshes, save-safe one-time pickup identity) is not done by this entry.** That is `docs/prompts/74-ART-REFERENCE-owner-boards-for-meshy.md` §4's install route and the addendum's §B/§C placement contract — this row only covers "the mesh exists, is textured, is upright, and is on disk."

## Follow-up — blind review and a mesh/material fix pass (same day, no Meshy spend)

Once the four were installed with the orientation correction above, a code-blind reviewer (no context on what changed, following the spirit of `.claude/skills/visual-judge/SKILL.md`: score nothing, name specific defects, name the frame) compared real `turntable.py` renders against board 17. Verdict: **Revive Flower passed outright.** Candy, Potion Plant and Mushroom were each **CLOSE BUT NEEDS WORK**, with concrete, addressable defects — the candy body read as a flattened lozenge from the side and the wrapper texture was a mottled camouflage pattern rather than the board's clean glossy gradient; the potion plant's leaves rendered muddy olive rather than vivid green, its buds carried pale blotches that read as mould, and a hard-edged dirt disc sat under it; the mushroom's vertex colour was not rendering at all (every frame came back pure white) and its front-centre mushroom had no stem or cap definition, reading as a rock.

A second pass (mesh/material edits, not a regeneration — no Meshy credit spent) fixed all three:

- **Mushroom.** Root-caused the invisible colour: `trimesh.Scene(mesh).export()` on a `ColorVisuals` mesh writes the `COLOR_0` vertex attribute but **no material at all**, so any renderer that does not invent one (Blender, and presumably Godot) falls back to flat grey. Confirmed by injecting a bare material into the same file and re-rendering — colour appeared immediately. Fixed properly rather than patched: UV-unwrapped the mesh and **baked the vertex-colour layout into a real `baseColorTexture`** with a proper `PBRMaterial`, the portable fix rather than relying on every downstream renderer to honour vertex colour. The rock-like mushroom was isolated by height-watershed segmentation, pinched into a stem, and given a flared cap rim. The grass-shard tufts were identified by per-vertex inward ray-cast (local thickness) rather than guessed boxes, deleted, and the resulting boundary loops hole-filled.
- **Potion plant.** Palette measured directly (leaves were hue 70°/sat 0.41/val 0.40, genuinely muddy) and remapped in HSV to a vivid green; bud vertices classified by sampling the baked albedo at each vertex's UV and repainted with a real base-to-tip red gradient plus a proper roughness map so the highlight is a specular response, not paint; the dirt disc identified and removed as two passes of near-horizontal low faces (350 faces total, including a fringe of ten sliver islands the first pass missed).
- **Candy.** The flattened axis (Z, at a 1.9:0.44 ratio) was scaled back to round (1.06:1 afterward) with a smooth falloff through the neck, closing the "reads fine from the front, fails from the side" defect exactly. Texture hue was collapsed from a 104–171° spread (the camouflage read) to a tight 114°±3° band with a noise-coring filter that kills patch-scale mottling while preserving the leaf medallion. The wrapper ends were reshaped geometrically (a neck cinch, a 65° helicoidal twist, a flare, a crinkle) — a real improvement over the original flat fan/fin plates, but **an honest, disclosed limitation**: vertex deformation of one continuous membrane cannot produce the board's separate pointed foil lobes, which need real topology. If this is ever revisited with a fresh generation, the prompt should say explicitly: *wrapper ends gathered into a tight twist at the neck, then opening into three or four separate pointed leaf-shaped foil lobes with sharp fold creases; NOT a continuous fan, NOT a bow tie.*
...
## Placing the generated assets in the world (2026-09-04)

The pickup props, the saddle and the checkpoint gate above all shipped as installed `.glb` files with no gameplay wiring. This entry closes that gap for all six.

**Pickups (candy, revive flower, potion plant, mushroom).** `data/items/items.json` gained `world_model`/`world_model_scale` on `potion_small`, `potion_large` and `revive` (pointing at `potion_plant.glb`/`revive_flower.glb`), and six new items: `good_candy`/`great_candy`/`rare_candy` (a new `level_up` effect — 1/2/3 levels, no fight required, applied through `creature_instance.gd`'s existing `set_level()`) and `speed_mushroom`/`stamina_mushroom`/`wild_mushroom` (reusing the existing tonic `creature_buff` mechanism outright — speed/defence/attack, 90s, no new code needed in the apply path). All six candy/mushroom items carry `world_model`/`world_model_scale` pointing at `candy_pickup.glb`/`mushroom_pickup.glb`.

`scripts/world/playground_world.gd::_spawn_item_cache` read only one shared barrel model before this (`CACHE_MODEL`); it now calls a new `_item_cache_model(item_id)` that reads `world_model`/`world_model_scale` off the item's own definition first and falls back to the barrel for anything that has neither (the elixir, the TM disc — unchanged). Four of the new items were given a first placement, 2-3m from the two already-proven-valid cache points, to prove the seam end to end: `good_candy`, `potion_small`, `revive`, `stamina_mushroom`. **This is not the regional density pass** — `docs/FINISH_THE_MEADOWS_ADDENDUM_2026-09-04.md` sections B/C ask for roughly a hundred candies and a hundred-odd findables total, authored in regional batches; this is one of each family in Band 1, and the four new coordinates are UNVERIFIED (no running Godot instance this session to probe ground height at a new site — `ground_height_at()` fails safely rather than crashing if any land off-terrain, but they need a real probe before being trusted as final).

**Riding saddle.** `scripts/world/riding_controller.gd` gained `_attach_saddle()`/`_detach_saddle()`, called from `mount()`/`dismount()`. The saddle appears only while a creature is actually being ridden, seated at the species' own `mount_offset` with a small downward nudge. This satisfies the hard rule (never worn at spawn) but is scoped narrower than "worn permanently once earned" — an un-ridden, un-followed creature standing in the field does not show it yet, since that needs a per-creature-instance flag threaded through `creature_body.gd`'s own build path, which this pass did not attempt. Flagged as further work, not a design change.

**South Bridge checkpoint gate.** The hero scan bakes its own closed double leaf into the same fused object as its posts, lintel and lanterns — but the crossing already has a separate, independently-animated leaf (`_mesh` in `gated_crossing.gd`) that swings permanently open on the real unlock event. Standing both up at once would either double the leaf while shut or leave a "ghost" closed leaf standing once the player actually opens it.
