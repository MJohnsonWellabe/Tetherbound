...
|---|---|---|---|---|---|---|
| **CANDIDATE, REMOVED (owner picked A, 2026-09-06)** — Kenney Nature Kit v2.1, 10 `cliff_*_rock.glb` modules (block, large, top, corner, cornerLarge, half, diagonal, blockSlope, blockCave, large_stone) | Kenney | [Nature Kit](https://kenney.nl/assets/nature-kit) (zip verified HTTP 200, 10.5 MB, 2026-09-06) | CC0 1.0 (pack `License.txt`, copied alongside as `LICENSE_kenney_nature_kit.txt`) | No | `assets/environment/kenney_nature_candidates/` | None. Staged only for the CLIFF-ART-0906 option C render (`ralph/reports/CLIFF-ART-0906/`); referenced by `visual.geology.crag_kit` when that option is active, unused otherwise. Deleted: the owner picked option A, which is material-only and needs no modules. `assets/environment/kenney_nature_candidates/` is gone from the tree. If ledge STRUCTURE is ever wanted, the nature family already vendored at `assets_raw/vendor/quaternius_stylized-nature-megakit/` carries `Rock_Big_1/2` and `Rock_Medium_1-4` and is the SAME family the rest of this world uses, so a second pack is not the answer. |
| **CANDIDATE, REMOVED (owner picked A, 2026-09-06)** — Poly Haven `mossy_rock` 1k colour + GL normal | Poly Haven | [mossy_rock](https://polyhaven.com/a/mossy_rock) via `dl.polyhaven.org` (fetched 2026-09-06) | CC0 1.0 (Poly Haven blanket licence) | No | `assets/environment/terrain/candidates/mossy_rock_*_1k.jpg` | None. Staged only for the CLIFF-ART-0906 option B render; referenced by `visual.geology.albedo/normal` when that option is active. Deleted: the owner picked option A, which keeps the shipped `Rock030` textures and only retints them. `assets/environment/terrain/candidates/` is gone from the tree. |
...
## Burrow Warrens mushroom species (WARRENS-ART-0906, 2026-09-06)

| Asset | Creator | Source | Licence | Paid | Local path | Modifications |
|---|---|---|---|---|---|---|
| Stylized Nature MegaKit — `Mushroom_Oyster` and `Mushroom_RedCap` (2 models, +2 on the 46 in the pack row above, so 48 installed) | Quaternius | [Stylized Nature MegaKit](https://quaternius.com/packs/stylizednaturemegakit.html) — **already vendored in this repository** at `assets_raw/vendor/quaternius_stylized-nature-megakit/` before this lane; **not re-fetched, nothing downloaded** | CC0 1.0, per the pack row above (line ~113 of this file), which this row is an addendum to | No | `assets/environment/stylized_nature/Mushroom_Oyster.{gltf,bin}`, `assets/environment/stylized_nature/Mushroom_RedCap.{gltf,bin}` | **No modification to either file.** Copied verbatim out of the vendored pack and imported. Both reference `Mushrooms.png`, which was **already installed** in the same folder for `Mushroom_Common`/`Mushroom_Laetiporus`, so the install is two `.gltf` + two `.bin` and no new texture at all. Recoloured at runtime only, per species, by `burrow_warrens.gd::_glow_fungus()` from `data/config/burrow_warrens.json::fungus.species` — pale cave variants (sage / bone / cool blue-grey) over a material duplicate; the source files and the shared atlas are untouched. |

**Why this row exists, and what was NOT done.** `docs/HANDOFF_2026-09-06.md` §4.3 row W1 proposed downloading the AssetQuest Free Mushroom Asset Kit (54.8 MB, 5 species) to fix the judge's "mushrooms are plain domes with no gills or cap profile". The owner's standing correction of 2026-09-06 — *"There is art in the repo for most of these things already, like mushrooms and team tether stuff. Find it and use it. Recolor it if you need to. Don't use Meshy."* — supersedes that row. The repository already held **six** mushroom silhouettes: `Mushroom_Common` and `Mushroom_Laetiporus` (installed), `Mushroom_Oyster` and `Mushroom_RedCap` (vendored, one copy away — installed by this row), `assets/environment/nature/mushroom_red.glb`, and `assets/props/mushroom_pickup/mushroom_pickup.glb` (the harvestable item, deliberately left alone so its silhouette and its warm saturated colour stay the ones a player reads as pickable). **No pack was downloaded and no Meshy generation was spent.** This is the same correction the ledger already records against the 2026-09-04 mushroom work ("Mushroom used Meshy only after checking for an installed asset second, not first"), applied first this time.

**Also used by this lane, all already installed, no new rows needed:** `DeadTree_1/2/3`, `TwistedTree_1/2/4` (nature megakit, pack row above) as the burrow arch's and dug face's root masses; `assets/environment/nature/stump_round.glb` (Kenney Nature Kit row above); `assets/props/kenney_survival/tree-log.glb` (Kenney survival row). Nothing in `bank.root_masses` or `bank.brow_root_meshes` is new to the repository.

## Burrow Warrens threshold lantern (WARRENS-ART-0906, 2026-09-06)

| Asset | Creator | Source | Licence | Paid | Local path | Modifications |
|---|---|---|---|---|---|---|
| Fantasy Props MegaKit — `Lantern_Wall` (1 model, +1 on the 16 in the pack row above, so 17 installed) | Quaternius | [Fantasy Props MegaKit](https://quaternius.itch.io/fantasy-props-megakit) — **already vendored in this repository** at `assets_raw/vendor/quaternius_fantasy-props-megakit/`; **not re-fetched, nothing downloaded** | CC0 1.0, per the pack row above, which this row is an addendum to | No | `assets/props/quaternius_fantasy/Lantern_Wall.{gltf,bin}` | **No modification to the file.** Copied verbatim out of the vendored pack and imported. Its three textures (`T_Trim_Metal_BaseColor/Normal/ORM.png`) were **already installed** in the same folder for the other fantasy props, so the install is one `.gltf` + one `.bin` and no new texture. Hung on the existing Team Tether lamp post by `burrow_warrens.gd::_build_bank_lamp_and_cable()` through `bank.lamp_head_model`; the post, its light and its oxblood collar are unchanged. |

**Why.** The blind visual judge, round 2, on `03-mouth`: *"a modern mushroom-shade park lamp on a smooth grey aluminium pole with an unshaded emissive yellow sphere and a painted red base, standing outside a prehistoric earth burrow in a wildflower meadow … It reads as a municipal streetlight."* It put the fix in its own **not fixable by scene work** list: *"A modern park lamp with a red base does not belong in this world at any brightness. It needs to be a different object — a lantern, a torch, a marked cairn."* The judge was right that a brightness change would not fix it and wrong that it needed art the build does not have: `Lantern_Wall` and `Torch_Metal` were both already vendored. Per the owner's standing correction of 2026-09-06, the repository was searched before anything external was considered. **No download and no Meshy generation.**
