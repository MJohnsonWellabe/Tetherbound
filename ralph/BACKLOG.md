...
- **Grass — investigated and landed, needs an owner call.** `ralph/OWNER-0902-GRASS-RENDER`: confirmed `grass_field.enabled` is `false` on `main` on purpose — the intended effect of yesterday's ~10fps-game-breaker fix, not a new defect. Went further: cut every cost lever in `grass_field.json` at once and re-measured — a config that's ~5x cheaper than the one that broke the Ally (13.7M vs. 31.8M primitives) while a player-eye-level shot still shows legible grass. `enabled` stays `false` — turning it on for a real handheld A/B is still the owner's call, this just means that A/B starts 5x lighter than the one that already failed.
- **Tent/campfire placement — landed.** `ralph/OWNER-0902-TENT-CAMPFIRE-PLACEMENT`: the placement mechanism itself was never broken (reproduced the real menu-pick→arm→ghost→build_place path headlessly, places cleanly everywhere tested). Real bug: the "camp" buildable has always bundled a tent+campfire+bedroll into one buy, but the Build menu never surfaced that — a player looking for a "tent" tile found nothing named one and reasonably concluded it couldn't be built. Menu now shows the piece's own blurb plus an "Includes: ..." list. Separately flags the owner's stated preference (three independently-placeable pieces, not one bundle) as a real, larger follow-up, correctly not attempted in this pass.
