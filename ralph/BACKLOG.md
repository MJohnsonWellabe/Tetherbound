...
- **Village layout — partially landed.** Same merge: the Grandpa's-house path endpoint, which sat a metre inside the house's own east wall (a kit rebuild widened the house and nobody re-derived the path), is fixed; Mira's shop got an exterior sign so a player knows a merchant is behind that door before entering. Village-gate-on-every-exit is still open, separately, under active re-investigation (`session_015saSq6rvUFKPhZzULvsbhT`) — real bug found (a jump-plus-edge-catch escape at boundary corners, not the road-gate props the original finding named), a fix landed for ~45 flat panels but one corner's re-test is still in flight as of the last commit; not landing until that comes back confirmed clean, not on a "should be fixed now" guess.
...
- **Grass — owner decision made: turn it on for real.** `ralph/OWNER-0902-GRASS-RENDER` confirmed off-by-design and prepared a ~5x cheaper config; the owner then directly overrode the "still the owner's call" hedge — "grass needs to be on." In flight: `session_015tK6hMPqfoE96e36KjfNWs` (`ralph/OWNER-0902-GRASS-ON`), flipping `enabled: true` on the already-measured cheaper config, with real render + test verification before landing.
