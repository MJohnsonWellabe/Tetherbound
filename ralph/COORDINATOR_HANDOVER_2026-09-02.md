...
| 7 | "Still unclear how to train a team." | `OWNER-0901-TRAIN-CLARITY` | **Confirmed fixed by real headless execution** (`ralph/OWNER-0901-TRAIN-CLARITY-V2`, 2026-09-02) — see `ralph/BACKLOG.md` §2 for the full evidence (real Godot 4.7 run, not a code read: `test_tournament.gd` 85/85 green, `smoke_tournament_bracket.gd` fought and won the real tournament through Halda's whole ladder, `smoke_objective_hint_card.gd` proved the concrete "how" guidance actually reaches the HUD). No code change needed. |
...
| 12 | Tournament `min_level` 6 → 5, Halda's guidance made concrete. | `OWNER-0901-TOURNAMENT-LEVEL5` | **Confirmed fixed by real headless execution** (`ralph/OWNER-0901-TRAIN-CLARITY-V2`, 2026-09-02) — `min_level` is 5 with no lingering 6 anywhere real (fallback-only), and the real simulated tournament run entered/lost/retried/won all three rounds against that threshold. See `ralph/BACKLOG.md` §2. No code change needed. |
