...
protocol is `ralph/GATE_F_MASTER_PROTOCOL.md` (§C schema, §I summary) if
context is wanted, but nothing here requires reading it.

**Deadline semantics:** per `ralph/GATE_F_PROTOCOL.md` §1.5, everything
...
suite before pushing. Branch/CI per `ralph/conventions.md`.
...
3. **Input injection contract:** every step sends the physical event via
   ...
   navigation and poll readers both see it (`ralph/conventions.md` trap:
...
- Before and after: kill orphan Godot processes whose cwd is deleted
  (pgrep recipe in `ralph/conventions.md`).
