...
func _run() -> void:
	...
	output_dir = "user://f07_cadence_probe"
	if accelerated:
	...
	if "--without-cadence" in OS.get_cmdline_user_args():
		var kept: Array = []
		for site: Dictionary in director.get("encounter_config").wild_sites:
			if str(site.get("_why_cadence_f07", "")).is_empty(): kept.append(site)
		director.get("encounter_config").wild_sites = kept
	print("F07 CADENCE SITES configured=%d" % director.get("encounter_config").wild_sites.size())
	...
	# F07 CADENCE PROBE (scratch). Declared fixture: post-shrine chapter flags and
	# one relocation to the top of the counterweight stair (the Windscar floor loop
	# is not walkable in this build, see report). Then the harness's own
	# _navigate/_walk and first-offer logging cover west anchor -> east anchor ->
	# Voss road entry, with anchor flags granted where the harness activates them.
	for flag: String in ["cloudreach_chapter_started","cloudreach_crisis_learned","storm_anchor_lower_west_mapped","cloudreach_lower_anchors_investigated","tether_lieutenant_senn_defeated","causeway_survivors_reconnected","windscar_aerie_prepared","keeper_maela_trial_defeated","cloudreach_act_i_complete","fly_traversal_unlocked","sky_shrine_reached","storm_anchor_engine_truth_learned","cloudreach_upper_route_unlocked","cloudreach_act_ii_complete"]:
		game.progression.set_flag(flag)
	stage = "counterweight_top"
	player.global_position = Vector3(-400.0, 781.0, 3890.0)
	await _frames(60)
	# Walk with the active companion deployed, as smoke_cloudreach_deployed_cadence
	# does: without an ally body the director offers no wild Engage at all.
	await _tap("creature_recall")
	await _frames(20)
	print("F07 CADENCE ALLY " + str(director.ally_body() != null))
	previous_activity = {}
	_log("flight_waypoint", {"target":"fixture: counterweight stair top"})
	stage = "upper_anchor_west"
	var west: Node3D = physical.get_node("upper_anchor_west/Interactable")
	if not await _navigate(west.global_position-Vector3.UP*0.8): return _finish()
	_log("physical_interaction", {"id":"upper_anchor_west (arrival)"})
	game.progression.set_flag("storm_anchor_upper_west_disabled")
	stage = "upper_anchor_east"
	var east: Node3D = physical.get_node("upper_anchor_east/Interactable")
	if not await _navigate(east.global_position-Vector3.UP*0.8): return _finish()
	_log("physical_interaction", {"id":"upper_anchor_east (arrival)"})
	game.progression.set_flag("storm_anchor_upper_east_disabled")
	await _frames(30)
	stage = "voss_approach"
	if not await _navigate(Vector3(300,1080,5100)): return _finish()
	_log("physical_interaction", {"id":"voss yard road entry (arrival)"})
	var worst: Dictionary = {}
	for interval: Dictionary in activity_intervals:
		if worst.is_empty() or float(interval.gap_seconds) > float(worst.gap_seconds): worst = interval
	print("F07 CADENCE WORST " + JSON.stringify(worst))
	for interval: Dictionary in activity_intervals:
		if float(interval.gap_seconds) > 30.0: print("F07 CADENCE INTERVAL " + JSON.stringify(interval))
	# Admission: every F07 pair spawns completely on real supported floor.
	if not "--without-cadence" in OS.get_cmdline_user_args():
		for flag: String in ["cloudreach_upper_anchors_disabled","captain_veyra_defeated","storm_anchor_network_disabled","cloudreach_winds_restored"]:
			game.progression.set_flag(flag)
		for site: Dictionary in director.get("encounter_config").wild_sites:
			if str(site.get("_why_cadence_f07", "")).is_empty(): continue
			var raw: Array = site.position
			player.global_position = Vector3(raw[0], raw[1], raw[2]) + Vector3(0, 0.6, 0)
			player.velocity = Vector3.ZERO
			await _frames(40)
			var members: Array = director.get("_site_members").get(str(site.id), [])
			var bodies: Array = []
			var ok: bool = director.get("_site_spawned").has(str(site.id)) and not director.get("_site_failures").has(str(site.id)) and members.size() == int(site.count)
			for wild: Variant in members:
				if not is_instance_valid(wild): ok = false; continue
				var support: Vector3 = director.call("_wild_support", wild.global_position, float(wild.call("body_radius")), wild)
				var supported: bool = support.is_finite() and absf(support.y - wild.global_position.y) < 0.8
				ok = ok and supported
				bodies.append({"name":str(wild.name),"species":wild.get("species_id"),"at":str(wild.global_position),"supported":supported})
			print("F07 CADENCE SITE " + JSON.stringify({"id":site.id,"ok":ok,"bodies":bodies}))
