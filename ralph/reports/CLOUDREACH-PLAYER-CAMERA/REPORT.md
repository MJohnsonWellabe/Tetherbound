...
| Night reads as a desaturated day | **Fixed.** Two causes, both in how `cloudreach_world.gd` layered the realm's look over the shared time-of-day presets. First, the sky profile's daytime `energy: 1.3` overwrote night's own authored 0.75; a preset's own values now win. Second, the `atmosphere` block went in whole as a weather delta, and a weather delta's colours replace every hour's, so the pale `#a2bdc8` haze stood behind the night sky too. Those colours are now the realm's base palette, which day inherits and night, dawn and golden override. Only the additive fog density stays weather. Before/after: `_sheet_night_before_after.png`; the day frame is unchanged. Tests: `test_cloudreach_world_presets.gd` (+2). |
