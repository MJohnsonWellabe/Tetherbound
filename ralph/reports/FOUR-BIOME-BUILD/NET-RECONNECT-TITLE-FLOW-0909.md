...
Final native result: **80 PASS, 0 FAIL, exit 0, `ALL CHECKS PASSED`**.
...
trainer in their current world. The restored client then moved **11.85 m**
...
The successful native run included two pure guard assertions:

- an in-window production build is not called silent;
- a production join still silent after the 90-second allowance is called
  silent.

After that run, review added a third pure assertion: an ordinary peer with no
allowance is silent after 15 seconds. That assertion is present in the final
source and the helper is a direct pure comparison, but it was added **after**
the 80-check native run and is therefore **not counted as native-verified
evidence here**. No further world run was authorized solely for that assertion.
...
```text
C:/Users/mattj/.cache/tetherbound-tools/godot-4.7/Godot_v4.7-stable_win64_console.exe --headless --path C:/Projects/Tetherbound --script res://tests/smoke_net_reconnect_keeps_character.gd --log-file C:/Projects/Tetherbound/.artifacts/net-reconnect-production-20260909-c/engine.log
```

Environment:

```text
TB_NET_RUN_ID=reconnect-production-20260909-c
TB_NET_OUT_DIR=.artifacts/net-reconnect-production-20260909-c/net
APPDATA=.artifacts/net-reconnect-production-20260909-c/coordinator-profile
LOCALAPPDATA=.artifacts/net-reconnect-production-20260909-c/coordinator-profile
```

Peer 0 PID `2200` and peer 1 PID `4112` both terminated. The terminal Godot
...
`TB_NET_CONDITIONS` is configured. Neither refinement was part of the 80-check
native run, so this report does not claim latency or a post-refinement native
rerun.
...
- `.artifacts/net-reconnect-production-20260909-c/console.log`
- `.artifacts/net-reconnect-production-20260909-c/engine.log`
- `.artifacts/net-reconnect-production-20260909-c/net/NET_RUN.json`
- `.artifacts/net-reconnect-production-20260909-c/net/SUMMARY.md`
- `.artifacts/net-reconnect-production-20260909-c/net/peer-0.log`
- `.artifacts/net-reconnect-production-20260909-c/net/peer-1.log`
- `.artifacts/net-reconnect-production-20260909-c/net/home-1/Godot/app_userdata/Tetherbound/characters/reconnect-smoke-character/character.json`
