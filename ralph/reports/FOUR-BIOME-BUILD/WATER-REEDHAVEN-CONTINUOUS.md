...
caller must also carry the campaign-earned axe. The helper does not reset the
...
1. fail closed unless the lesson is earned, the departure is unrepaired and a
   real axe is present on a controller hotbar action;
...
- `tests/test_water_reedhaven_segment.gd`: 1 test, 4 assertions, 0 failures.
