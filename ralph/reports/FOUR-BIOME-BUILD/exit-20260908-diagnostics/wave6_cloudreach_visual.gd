## CLIFF-ART-0906 scratch copy of capture_cloudreach_environment_correction.gd
## with `--out=` and `--only=` user args. Same stands, same production camera.
##   xvfb-run -a -s "-screen 0 1280x800x24" godot --path . --rendering-driver opengl3 \
##     --resolution 1280x800 --script tools/_capture_cloudreach_cliff_options.gd -- \
##     --out=res://shots/opt_a --only=01-arrival,03-windscar,05-upper
extends SceneTree
...
var OUT_DIR := "res://shots/cloudreach_cliff_options"
var PERF_PATH := "res://shots/cloudreach_cliff_options/performance.json"
...
func _run() -> void:
	...
	if game != null and game.has_method("reset_for_new_game"):
		game.set("save_system", SAVE.new("user://cloudreach_visual/"))
	...
	# CLIFF-ART-0906 scratch tool: `--out=res://...` and `--only=a,b,c` (stand
	# name substrings) so three material/geometry options render the same
	# stands into separate folders without touching the committed report tool.
	var only: Array = []
	...
	for raw_view: Variant in VIEWS:
		...
		if not only.is_empty():
			var wanted := false
			for needle in only:
				if str(view["name"]).contains(str(needle)):
					wanted = true
			if not wanted:
				continue
	...
	var perf_file := FileAccess.open(PERF_PATH, FileAccess.WRITE)
	if perf_file == null:
		failures.append("could not write %s" % PERF_PATH)
	else:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("CLOUDREACH CAPTURE OK %d frames; performance -> %s" % [rows.size(), PERF_PATH])
	quit(0)

func _record_cover(world: Node3D, at: Vector3) -> void:
	var counts={}
	for raw in world.find_children("*", "MultiMeshInstance3D", true, false):
		var node=raw as MultiMeshInstance3D
		if not (str(node.name).begins_with("Cover") or str(node.name).begins_with("Grass")):continue
		var mm=node.multimesh
		if mm==null:continue
		var count=0
		for index in mm.instance_count:
			var p=node.global_transform*mm.get_instance_transform(index).origin
			if Vector2(p.x-at.x,p.z-at.z).length()<=20 and absf(p.y-at.y)<3:count+=1
		if count>0:counts[str(node.name)]=int(counts.get(str(node.name),0))+count
	print("ACTUAL COVER within20m surface at=",at," counts=",counts)

func _timeout() -> void:
	await create_timer(900.0, true, false, true).timeout
	push_error("Cloudreach visual diagnostic fifteen-minute setup deadline")
	quit(1)
