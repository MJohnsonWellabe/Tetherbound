...
An additional post-fix `smoke_net_host_join_leave` attempt was started with isolated APPDATA and two real peer processes, but produced no peer logs or `SUMMARY.md` and was terminated after the coordinator made no progress. The committed host/join evidence in `tetherbound-invite-host-join.log` predates the final save/retry fixes; this smoke is therefore pending rather than claimed as final post-fix evidence.
