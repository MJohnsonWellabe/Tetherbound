...
Base: `ralph/x05-rejoin-exact-pose` `5e130ba05`. Headless. One run on the committed code:
...
- **`scripts/ui/title_screen.gd` `_begin_join`:**
  ...
  - It clears the live pose before the world builds, so no world ever places a pose saved somewhere else.
  - It mounts `RejoinPose`.
- **`scripts/mp/rejoin_pose.gd`:**
  - It waits until the host snapshot is applied, then calls `decide()`.
...
1. **Setup in world A:**
   ...
   - It saves. Its character file names A's instance (`72ddc653…`) and that pose.
   - It leaves.
...
3. **Join B as the same character:**
   - B's instance (`2b66af6d…`) differs from A's, so the decision is `regional`.
   - The guest stands at B's regional spawn (0.00 m from where B's world placed it), 82.8 m from A's saved spot.

## Unit coverage

`tests/test_rejoin_pose.gd` has 4 tests and 14 assertions. It covers:
- the same instance;
...
- **Steam joins.** They go through `_start_pending_steam_join`, not `_begin_join`, so the Steam path is unchanged.

Logs: `coordinator.log.gz`, `peer-0.log.gz` (host A), `peer-1.log.gz` (host B), `peer-2.log.gz` (the guest), `SUMMARY.md` (net run `local-3969392`).
