...
**Count: 3 qualify outright and 3 qualify conditionally** (Bram, herd and Juno outright; the vault Elder, Doss and the Hall alpha conditionally). The vault Elder is conditional because its current-main headless rerun fails on the Heartstone pickup and on reload (see below). WORLD §11 (`docs/design/WORLD.md:361`) counts "existing branch rewards/evolution catalyst access" as the vault's payoff, and the Heartstone is one of those branch rewards (`WORLD.md:92`). The Elder's two-potion receipt alone is therefore not the whole row. That failed rerun predates the witness fix in 744aa37ff and has not been rerun since. Doss was made conditional for a comparable runtime failure, and the vault is treated the same way. Doss's failure is order-dependent. Run alone on current main 47774c350 (`smoke_local_requests.gd --only=doss`, headless), the whole Doss flow passes, including the repeat and the disk round trip. Swapping in the pre-#215 `dialogue_panel.gd` changes nothing, so #215 is not the cause. Run after herd, Bram and Juno in the same session, its repeat greeting fails because `DialoguePanel` keeps input ownership. That is either a real cross-activity dialogue leak or a harness artefact, and it is open. The Hall alpha is conditional because its optionality is unproven: it sits 18 m from the spine in the chapter's largest aggressive cluster, against WORLD's off-road detour rule. Together they cover all five principal regions, three of them only conditionally: Lower Meadows 2 and Upper Meadows 1 outright; Stone & Root 1 (vault Elder), River Lock 1 (Doss) and Hall approach 1 (Hall alpha) conditionally. The unit record has not caught up. `tests/test_meadows_activity_qualification.gd` still lists the vault without a `conditional` reason (`:171`) and asserts 4 outright (`:307`). This report edit does not change that file, so the test and the report disagree until it is updated (see Open). None ends in a generic chest. `test_no_qualified_activity_gates_or_is_credited_by_the_main_route` checks that no main objective reads any counted completion flag, and a grep finds no other gate on those flags. None of the flags' other readers is a gate: Rae's `unless_flag` branch (`village_npcs.json:496`), the world ledger's Doss handler, Juno's `dialogue_after`, `lost_companion_reunion.json` and `tools/gate_f/segments/S05*.json`.

**What "qualify" rests on.** Every file-checkable column above is asserted from shipping data, and every runtime column has a **staged** single-player witness. None of the six has an unstaged ordinary-approach witness, a two-peer reward witness or a code-blind presentation pass. Sol's earlier batch verdict was "limited activity evidence", with follower crowding and a cropped Doss platform. F03 is therefore **not closed**. On file, 3 activities qualify outright and 3 conditionally (vault Elder, Doss, Hall alpha), all with staged runtime evidence only. F03's six-count is **not** claimed met while any of the six is conditional. Ordinary discoverability and Hall-alpha optionality remain open.
...
- Current-main headless rerun failures for the vault (Heartstone settle and reload) and the Hall (road engage); a rendered rerun is needed.
- Rerun `smoke_warrens.gd --vault-activity` on a tree containing 744aa37ff. The recorded failure predates that witness fix, and the vault stays conditional until the run passes.
- Shared defect: `vegetation.gd::clear_area` removes gather points only for collidable layers, so an invisible fibre Gather point stays in the vault and wins the interaction arbiter over the Heartstone. The fix needs a grant on the vegetation `clear_area` code.
- `tests/test_meadows_activity_qualification.gd` still records the vault Elder as outright (`:171` has no `conditional` reason; `:307` asserts 4 outright). It should record 3 outright and 3 conditional.
