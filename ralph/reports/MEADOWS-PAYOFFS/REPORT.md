...
- `smoke_local_requests.gd` (all activities in one session, Doss last): `local requests smoke test passed`. Doss's order-dependent repeat failure recorded at 47774c350 has not recurred in five runs since (835744b35, 2049ff283, fe07d0d28, c8935da00 twice).
...
- `smoke_alpha_pins.gd --hall-decline` (optionality): real road input walked the spine past the pack without a fight; closest approach 17.9 m to the alpha and its pack. One run. The Stronghold approach also has table-rolled aggressor clusters on the road (5011 engaged an earlier run); those are route danger, not this activity, and the witness fights through them.
