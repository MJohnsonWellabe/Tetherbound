...
- `smoke_alpha_pins.gd --hall-decline` (optionality, receipt `b5-hall-decline-a329d57ae.log` (17.9 m) and `b5-hall-decline-24fb8c6ed.log` (tightened witness, 20.4 m`): real road input walked the spine past the pack without a fight; closest approach 17.9 m and 20.4 m on two runs, outside the pack's 14 m notice range, so the road stays outside its notice envelope by geometry. Two runs. The Stronghold approach also has table-rolled aggressor clusters on the road (5011 engaged an earlier run); those are route danger, not this activity, and the witness fights through them.
...
### Current-main runtime rerun (staged)
