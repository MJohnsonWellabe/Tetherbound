...
### Two fresh peers finish the Meadows opening

`coop-opening/two-peer-pass08.log` passes 117 assertions with both fresh identities entering through production title host/join, each walking the real house stairs, greeting Grandpa, choosing and naming one starter, and receiving catch supplies. Both screens carry the chosen body/name identity and the full map draws the other player. The openings run sequentially inside the same shared session; this is not simultaneous modal interaction or proof of internet invitation transport.

Both owner-appropriate production save/load paths retain the starter UID, starter receipt and opening supplies; the guest does not replace the hosted world. This is same-process load, not cold reconnect. Exact peer logs contain no ERROR, SCRIPT ERROR or entombed recovery. The prior roof collision fix is on main; this witness adds only passive opening observations and an opt-in three-frame physical tap for directional menu polling. Default gameplay presses remain one frame. Luna and root reviewed the selected changes.

`one-frame-rejected07.log` retains the rejected naming attempt: one-frame directional injection left the cursor at A. The accepted test reads the actual cursor after each bounded physical tap and requires the newline Done cell with nonempty text; it does not write UI state, inventory or opening completion flags. No full Meadows co-op, tournament, Warrens, river or Hall acceptance follows from this opening evidence.

### Shared guardian admission — preserved at usage stop

Reviewed by root and Luna; implementation by Sol and Terra. Shared encounters now deploy the local ally into the arena without moving the authoritative opponent or trainer. Warrens guest aggression joins an already announced matching guardian encounter instead of creating a local duplicate. Client-first aggression without an announcement remains unresolved.

Focused tests: 22 tests, 105 assertions, zero failures (`guardian-admission/units.log`). Required Playground exited 0 with the same eight known engine-error categories and no SCRIPT ERROR (`guardian-admission/playground.log`). Two-peer attempt 05 demonstrated host victory (8 hits) and guest contribution (10 hits), but was rejected: a freed-opponent cast in combat_pilot's final diagnostic aborted the guest verdict before reward, vault and save assertions. This is not full Warrens or chapter co-op acceptance. The witness and diagnostic correction remain uncommitted in the preserved checkout. Automatic admission has focused unit coverage; attempt 05 used explicit joining.

Stopping at 10% weekly remaining. Existing opening, bridge and Doss proofs are landed; tournament, Warrens end-to-end payoff and river witnesses remain incomplete. Main CI run 35538103362 also retains a failed default shared-wild geometry/receipt witness and the known export failure; neither is claimed fixed here. No planning documents changed for this stop.

### PR175 resumed integration checks

Owner resumed integration after the usage stop and requested every authored unfinished change be pushed. Recovery branch `ralph/meadows-recovery-pr175` preserves all authored WIP; its older ROADMAP edits are not a new approved plan. Main ROADMAP and design remain authoritative. Generated import/cache/UID churn is not authored work.

`guardian-admission/default-wild-pass.log`: default shared-wild witness passes 95 checks, both peer logs have no ERROR/SCRIPT ERROR. The fixture now avoids overlapping bodies and derives the friendly-target swing at the same live origin as submission. Local first attempt expired only at the 180-second boot limit; the passing external wrapper changed hello allowance to 360 seconds, with gameplay deadlines/assertions unchanged. CI retains its existing budgets.

`guardian-admission/rejected-08.log`: both players won the real guardian (host 6 hits, guest 10), ordinary guardian aggression automatically joined the exact host record, both received full rewards, five UIDs survived owner-appropriate save/load, and both delivery journals matched. Guest clear flag/live vault failed. `_mark_once_cleared` was writing only local progression; the correction routes multiplayer `warrens_cleared` through the existing ledger without local fallback. Other once flags and solo remain unchanged. The corrected two-peer rerun is pending.

`guardian-admission/final-units.log`: 39 tests, 176 assertions, zero failures, including the updated realm-owned placement expectation and ledger-routing regression. `remote-teardown-units.log`: 7 tests, 22 assertions, zero failures. `final-playground.log`: smoke OK, exit 0, no SCRIPT ERROR, eight existing engine-error categories. Remote trainer teardown now releases actual reciprocal exceptions without removing an already-cleared side. The export verifier gives the completed world a physics/process turn before the existing measurement and native quit; export exit/crash checks remain intact. This is not yet proof that the existing GLES3 heap abort (`export-existing-crash.log`) is fixed.

Reverse-revive CI diagnostics establish a valid host-observed 0.98m gap within 2.5m, but the original one-frame input produced no authorization/progress. Confirmation now requires the actual RevivePrompt winner and uses the already-landed three-frame tap option. No revive authority or distance rules changed; runtime confirmation and full CI are pending.

Guardian rerun `guardian-admission/two-peer-pass09.log` now passes completely on PR175 code `4a96589f0`: guest aggression automatically joins the host record; host wins with 11 hits and guest with 3; both receive `warrens_cleared`, both live vault colliders open, each full authored payout arrives, all five UIDs survive owner-appropriate reload, and exact addressed reward journals match. Both peer logs have no ERROR/SCRIPT ERROR. This proves the bounded two-peer guardian segment, not an earned chapter, cold reconnect, catch path or presentation acceptance. The entrance warp, prepared level-16 five and muted unrelated residents remain explicit fixture staging.

### Reverse revive fixture seat

The original client-to-host reverse revive was not an authority or range failure. The host observed the client 0.98 m from the downed host, within the 2.5 m radius, but the client arbiter had no winner: it was enabled, in `world` input, and had the local player as viewer, while the attached `RevivePrompt` returned an empty offer because its line-of-sight query was false. The prompt and remote host body existed at `(-23.60, 5.25, -15.70)` and `(-23.60, 4.65, -15.70)`; the fixed `+X` client seat was `(-21.70, 4.54, -15.70)`. No ray-hit identity was recorded, so this finding is limited to authored geometry blocking that line.

`stand_by_downed` now accepts a signed side for fixture seating. The reverse leg uses `-X`, returning the client to the clear counterpart of the successful first revive seat. `revive-fixed-20260920` exits 0 with all checks passed: the host sees 1.82 m, the active winner is `RevivePrompt`, its direct offer is actionable at priority 10 with LOS true, the client receives host-owned progress, the host holds one authority attempt, and the host revives exactly once. The smoke still verifies cancellation, movement deadzone, no satchel, locomotion, and both directions; it adds no revive authority or distance-rule change. The rich rejected logs and accepted run are retained under the isolated `C:\Users\mattj\AppData\Local\Temp\tetherbound-revive-{rich,fixed}-appdata` coordinator roots.
...
The actual headless `--verbose --verify-export` path at `9cf5f5cf5` exits0 but identifies four surviving MeshInstance3D/ArrayMesh/StandardMaterial3D triples, Rock030 textures and orphan names WorkedFaceWest/MidWest/MidEast/East. Complete raw output is preserved in `guardian-admission/export-retirement-headless.zip`. These exactly match `_textured_wedge`: it allocated a natural-rock node before returning a separate worked-face panel. The correction selects WorkedFace panels before allocating that unused node; returned geometry/materials and all other branches are unchanged.

Focused regression (actual terminal results, not reconstructed raw logs): `tests/run_tests.gd -- --only=test_old_quarry_visual_identity.gd` fails on the old source, orphan count2→6, 11 tests/447 assertions/1 failure plus the matching four-mesh leak footer. The corrected source passes11 tests/447 assertions/0 failures with no leak footer. Full exported-runtime confirmation remains required.

Required ordinary Playground smoke after both corrections: exit0, smoke OK, no SCRIPT ERROR. The former mesh/material/texture/instance allocator and remaining-resource errors are gone; only the existing null-material error remains. Raw evidence: `guardian-admission/export-quarry-playground.log`. Full CI and exported native shutdown are pending.

### Merged integration checkpoint

PR176 merged as `4217ad376` after all26 active jobs in PR CI35593173597 passed on source `a7c0471e5`; the existing two explicitly known-red campaign jobs remained skipped. Release verification35593227583 on that same source also passed: `EXPORT-CHECK terrain=yes ground_at_spawn=1.20 player_y=5.00 props=383315`, followed by `export: OK — extension loaded, data present, ground found.` The verifier still requires a zero native exit code; the teardown implementation changed. Publishing was disabled. Scene retirement alone had failed134 in35591889974; its original artifact is retained as `guardian-admission/export-retirement-only-rejected.zip`. The combined correction is the accepted tested candidate.

The earlier main PR175 run35591444461 also exposed a shared-wild rapid-action witness failure (job106308051365); other shared-wild checks passed, and inspection found no demonstrated production authority regression. Its exact peer logs, summary and run metadata are in `guardian-admission/main175-shared-wild-rejected.zip`. The unchanged smoke subsequently passed in final PR176 CI; no timing assertion was relaxed. Main's independent runs35595222911 (CI) and35595222905 (Release verification) are pending at this checkpoint.

### Shared-wild authority fixture correction

Main Release35595222905 passed on4217ad376, confirming the exported native shutdown fix on main. Main CI35595222911 repeated the shared-wild cooldown failure. The diagnostic reproduced action9001 being correctly refused as friendly_target: the charged cone connected with the host trainer at3.947m before the opponent at4.262m. With no accepted charged action, the later cooldown assertions had no charged lock to measure. This was fixture geometry, not demonstrated production authority failure.

PR178/c678bfd4b turns only the authority action outward for a legal charged whiff. Host validation commits the same deadline for accepted misses. Immediate rapid refusal, replay refusal, unchanged authority, the1200ms minimum, real normal hits and separate friendly-target/zero-damage checks remain. Sol independently reviewed without a blocker. Corrected local two-peer smoke exits0 with all checks passing; both peer logs have no ERROR/SCRIPT ERROR. The external wrapper extends only peer boot allowance to360s; CI budgets and gameplay deadlines are unchanged. A temporary post-rapid diagnostic print was removed before commit; no assertion changed. Complete rejected diagnostic and accepted coordinator/peer logs are in guardian-admission/shared-wild-authority-fixture.zip. PR and subsequent main CI remain pending.

PR178 first CI35597605079 passed the corrected authority/cooldown/replay sequence but failed final A runtime retirement. Both flee injections returned PASS, which did not prove departure; the old failure lacked a participant payload, so its exact remaining peer was not established. Source review identified a0.25s physics input guard after rejoin, while the join helper returns on the first bound frame. Follow-up91b66ac77 waits30 host physics frames, then requires exact host-only participation after the guest's single flee press before issuing the final host press. There are no input retries or production changes. The corrected focused two-peer run passes all checks with no peer ERROR/SCRIPT ERROR. Sol independently reviewed the preserved assertions. Original CI peer evidence and the complete local passing run are in shared-wild-withdrawal-rejected.zip and shared-wild-withdrawal-pass.zip under guardian-admission. Full CI remains pending.

PR178 merged as e74a0783d after CI35599877792 passed all26 active jobs on91b66ac77, including all seven multiplayer shards. Both known-red campaign jobs and the PR-only export exclusion remained explicit skips. Independent main CI35602172224 and Release35602172291 are running; no full-main pass is claimed yet.
...
| 5 | Stone & Root | Warrens vault Elder Trailpup. `burrow_warrens.json:1105` (spawn `nickname`, `completion_reward` at `:1106-1112`); once flag from `burrow_warrens.gd:8066`; paid by `encounter_director.gd:4876`; Heartstone `prize` at `burrow_warrens.json:1440` | Glowing named Elder (`alpha.aura_light`) and the Heartstone light in the side vault. The vault sits behind the gated den-to-vault door (`burrow_warrens.json:797`), which opens on `warrens_cleared` | Step into the vault and choose the manual Engage (`aggressive:false`). The fight is optional; the required guardian is not counted | 2 `potion_large` via receipt `trainer:warrens_once_elder_trailpup:item:potion_large`, independent of species. The Heartstone keeps its own `warrens_heartstone_taken` claim | Receipt line "The Elder Trailpup yields two large potions for the road ahead." (`burrow_warrens.json:1108`) | `warrens_once_elder_trailpup` (world prefix `warrens_once_`, `flag_scopes.json:160`) | **Yes (staged)** | Unit: same file. Runtime (**staged**: `warrens_cleared` pre-set, five seeded, non-Elder aggression disabled): `smoke_warrens.gd --vault-activity`, `six-activities/meadows-vault-prompt.log` "warrens vault activity passed". It walks from the entrance, admits the Elder manually, wins by input, pays exactly 2 and survives a disk reload without repayment. WORLD §11 counts the branch rewards and catalyst access as part of this payoff, so the Heartstone pickup and its reload are part of the row. The `local` objective row has landed on this branch: `band2_warrens_vault_elder` (`data/progression/objectives.json:338`, `flag_id` `warrens_once_elder_trailpup`, `revealed_by` `warrens_cleared`), with its audit row at `data/config/chapter_rewards.json:340` **Batch-5 rerun (receipt `six-activities/b5-vault-f8845554b.log`): passed, Heartstone pickup and save/load included, after the `clear_area` fix; the pre-fix failure in the superseded section below is history.** |
| 6 | River Lock | Doss's bank perch. `scripts/world/river_nest_clear.gd:17-25`; objective `band3_river_nest` (`objectives.json:338`); placed at `playground_world.gd:468` (72,4187.4) | Ranger beside visibly buckled boards on the river loop. Greeting sets `river_nest_doss_met` | Hand over 1 wood and 1 fiber | 45 coins and 1 `potion_large`. Inventory preflight covers the whole reward; paid once by the world flag | `river_nest_doss_defeated` (`band3_the_river_lock.json:13`) names the exact 45 coin, plus the repaired perch | `river_nest_doss_cleared` (world, `flag_scopes.json:77`) | **Yes (staged)** | Unit: same file (also asserts that the thanks names `REWARD_COINS`). Runtime (**staged** seat): `doss-coop-repair/runtime-pass02.log` shows the parsed interaction, the reward paid, save/load and no repayment. Solo only. **Batch-5 rerun (`six-activities/b5-local-f8845554b.log`, all activities in one session, Doss last): passed, repeat step included. One logged pass; the earlier order-dependent failure is recorded in the superseded section below.** |
| 7 | Upper Meadows | Juno's lost companion. `bands/band4_upper_meadows_ironwood/trainers.json:242` `lost_creature_rue` and `:197` `pasture_drover_juno` (`dialogue_after`); `scripts/world/lost_companion_reunion.gd`; `lost_companion_reunion.json:3`; objective `band4_lost_creature` (`objectives.json:354`) | Juno's challenge (`dialogue/trainers.json:245`) names the stolen Meadowhart and sets `lost_creature_rue_met` | Beat the patrol, then lead the stray Meadowhart back to Juno | 50 coins and 1 `revive`, paid once by the patrol win; `rechallenge:false`. The return pays nothing further. No creature joins the party | On arrival Juno's `lost_creature_rue_returned` thanks plus a world toast; the display creature stays beside Juno, and `pasture_drover_juno_reunited_challenge`/`_defeated` (`trainers.json:269,281`). The friendly bout can be declined | `lost_creature_rue_returned` (world; set on arrival within 6 m of Juno, `lost_companion_reunion.json`) | **Yes** | Unit: same file, including `test_the_lost_companion_is_acknowledged_by_juno_on_the_counted_fact`. Runtime (**staged** seat, 6HP opponents): `--only=juno`, `six-activities/meadows-juno-qualified.log` "selected local activities smoke test passed: juno". It covers the reunion, the parsed decline and a disk round trip |
| 8 | Hall approach | Alpha Galecrest. Spawn order 5001 (`bands/band5_stronghold_approach/spawns.json:23`, `completion_reward` `:36-43`); map pin `alpha_pins.gd:187`; once flag `alpha_pins.gd:224`; paid by `encounter_director.gd:4876` | 3.15m alpha leading a pack of three on the west shoulder at (-58,7255), before the Sigil gate (7400). The map pin appears within 300m | Engage and defeat or catch it | 2 `potion_large` and 1 `revive` via receipts `trainer:wild_once_5001:*` | "The west shoulder has gone quiet." (`spawns.json:42`); the pin clears | `wild_once_5001` (world prefix `wild_once_`, `flag_scopes.json:159`) | **Yes (staged)** | Unit: same file. Runtime (**staged**: teleport to the road): `smoke_alpha_pins.gd --hall-activity`, `six-activities/meadows-hall-retained-final.log` "road input defeated Alpha Galecrest, paid ... once, and preserved it across reload". Caveat: this is the chapter's largest *aggressive* cluster, 18m from the spine. Declining it from the road is witnessed by `--hall-decline` (appended note). The `local` row has landed on this branch: `band5_hall_alpha_galecrest` (`objectives.json:373`). Its log reveal is **interim**. `revealed_by` is `hall_approach_open`, which is set at the Sigil gate (the `defeat_the_captains` main objective, `objectives.json:216-221`, beacon z 7400). The alpha stands before that gate, at z≈7255, so the log row may reveal only after the player has already done it **Optionality: `smoke_alpha_pins.gd --hall-decline` (`six-activities/b5-hall-decline-a329d57ae.log` (17.9 m) and `b5-hall-decline-24fb8c6ed.log` (tightened witness, 20.4 m) walked the spine past the pack with real input, no fight; closest approach 17.9 m and 20.4 m on two runs, outside the pack's 14 m `notice_range` — measured, not derived. Two runs.** |
...
**Count: all 6 qualify outright on the batch-5 rerun** (Bram, herd, Juno, vault Elder, Doss, Hall alpha), covering all five principal regions: Lower Meadows 2 (Bram, herd), Upper Meadows 1 (Juno), Stone & Root 1 (vault Elder), River Lock 1 (Doss), Hall approach 1 (Hall alpha). Tree: `ralph/f03-activity-rows` at main c8935da00 plus these rows and the `vegetation.gd::clear_area` fix (every layer's gather point is now removed from cleared ground; failing-first `test_vegetation_clear_area_gather_points`). All runs headless, on the staged fixtures named in the table; receipts in `six-activities/b5-*.log`:
- `smoke_local_requests.gd` default mode (Night Watch, Doss, Juno, herd, cart in one session; it does **not** run Bram): `local requests smoke test passed`. Bram separately: `smoke_local_requests.gd --only=bram`, `selected local activities smoke test passed: bram` (receipt `b5-bram-93ac38045.log`). Doss's order-dependent repeat failure recorded at 47774c350 did not recur in this logged run (earlier unlogged passes on 835744b35, 2049ff283 and fe07d0d28 are not receipts).
- `smoke_warrens.gd --vault-activity`: `warrens vault activity passed`, including the Heartstone pickup and the save/load of the Elder and Heartstone once flags. It failed before the `clear_area` fix because a fibre "Gather" point won the arbiter.
- `smoke_alpha_pins.gd --hall-activity`: road input defeated Alpha Galecrest, paid the receipt once and kept it across reload.
- `smoke_alpha_pins.gd --hall-decline` (optionality, receipt `b5-hall-decline-a329d57ae.log` (17.9 m) and `b5-hall-decline-24fb8c6ed.log` (tightened witness, 20.4 m): real road input walked the spine past the pack without a fight; closest approach 17.9 m and 20.4 m on two runs, outside the pack's 14 m notice range, measured on both runs. Two runs. The Stronghold approach also has table-rolled aggressor clusters on the road (5011 engaged an earlier run); those are route danger, not this activity, and the witness fights through them.
Still not claimed: an **unstaged** ordinary-discovery witness from a real save showing all six lures, and a code-blind verdict on distinctness and usefulness (next work item). The Hall row's log reveal remains interim (see its row).

**What "qualify" rests on.** Every file-checkable column above is asserted from shipping data, and every runtime column has a **staged** single-player witness (fixture seats, seeded teams). All six now pass those staged witnesses on the batch-5 tree. None of the six has an unstaged ordinary-discovery witness from a real save, a two-peer reward witness or a code-blind presentation verdict. F03 is therefore **not closed**: the six-count and region coverage hold on staged evidence only.

### Unit evidence

`godot --headless --path . --script tests/run_tests.gd -- --only=meadows_activity_qualification`: **14 tests, 208 assertions, 0 failed**. The six original tests are unchanged, and eight new ones cover the record. A mutation check was run on a scratch copy and reverted: wrong once flags, a wrong acknowledgement id and a moved region produced 4 failing tests. (The moved region changed only a test constant, so that mutation exercises record consistency, not shipping data.)

### Current-main runtime rerun (staged) — SUPERSEDED by the batch-5 rerun above

Kept as history. Every failure below was rerun on the batch-5 tree (count paragraph above): the vault passes after the `clear_area` fix, which has since landed on this branch under the vegetation grant; the Hall activity passes; Doss passes after the other activities in one logged run. The batch-5 reruns are headless like these; the original vault and Hall passes were rendered. A headless pass exercises the same interaction, arbiter, reward and save/load code, so it is taken as runtime qualification; presentation is judged separately by the open code-blind verdict.

`flock <writer lock> timeout 900 godot --headless --path . --script tests/smoke_local_requests.gd -- --only=herd,bram,juno,doss` ran on 47774c350 plus this branch's test and report changes. It is a headless run of the same staged harness, and it **exited 1**. It printed no failure lines for herd, Bram or Juno. Doss's first interaction passed every check before the repeat step: the perch cleared, exactly one wood and one fiber were consumed, 45 coins and one Large Potion were paid once, and the repaired collider was installed. The run then failed at the repeat greeting: `smoke FAIL: river_nest repeat: exact prompt never became actionable at player=(69.6, -2.835917, 4187.4) winner=<null> input_owner=/root/MeadowsPlayground/DialoguePanel`. On current main, then, the no-repay repeat and the disk round trip for Doss were **not reached**. Doss's reload evidence rests on the earlier rendered `doss-coop-repair/runtime-pass02.log`. It is not yet established whether this is a headless timing artefact or a regression in dialogue input ownership after the cleared acknowledgement. The fix would belong in `river_nest_clear.gd` or in the smoke harness, and it is **open**. The queued headless reruns later completed under the same lock:
- The default-mode `smoke_local_requests.gd` (Night Watch, Doss, Juno, herd, cart) **exited 0** with "local requests smoke test passed".
- `smoke_warrens.gd --vault-activity` **exited 1** with three failures: the Heartstone interaction did not settle its item and flag, save/load lost the Elder or Heartstone once flag, and save/load recreated the consumed Heartstone. This run **predates** the vault-witness fix in commit 744aa37ff. Before that fix, `tests/smoke_warrens.gd` pressed `interact` once without checking the arbiter. It now waits until the Heartstone's own Interactable is the actionable winner of the interaction arbiter, and it reports a prompt that never wins as exactly that. The vault activity **has not been rerun since**. The press was found to lose the arbiter to an invisible fibre Gather point. `vegetation.gd::clear_area` (`scripts/world/vegetation.gd:2476`) leaves that point behind: it removes rendered instances on every layer, but it removes gather points only inside its collidable-layer loop. `_spawn_harvest_point` stands a gather point for every harvestable placement, collidable or not. This is filed as a shared defect, and the fix needs a grant on the vegetation `clear_area` code.
- `smoke_alpha_pins.gd --hall-activity` **exited 1**: "real road input did not reach and engage Alpha Galecrest (gap 3.15)".

At the time (superseded above): the original passing vault and Hall logs were rendered (OpenGL) runs, and these reruns were headless, so the difference may be a headless artefact. It could also be a regression, and neither is established. Until a rendered rerun passes on current main, vault and Hall runtime qualification rests only on the retained `six-activities/` logs, and this is **open**.

### Shared change (landed under SHARED-FILE GRANT on branch ralph/f03-activity-rows)

The coordinator granted `objectives.json` and `chapter_rewards.json` for these rows. As landed: the two `local` rows below, `counts_as_activity: false` on first Ironwood, the two audit rows, and a correction to the Doss audit text (it had called Doss's Large Potion the only one before the Hall). The activity-floor test now excludes `counts_as_activity: false` rows, and main-route credit must carry that mark. The original proposal follows.

`data/progression/objectives.json` is shared and was not edited on the first branch. The proposal adds `local` rows for the two counted activities that have none, and marks the Ironwood row as story-only so no `local`-row count treats it as an activity. Both new flags already exist and are declared (`warrens_once_`/`wild_once_` world prefixes). No new boolean is added.
...
- Landed under the grant (both rows and both audit rows are on this branch). `test_meadows_activity_qualification`, `test_flag_scopes` and `test_chapter_rewards` pass together (38 tests, 726 assertions, 0 failed). The record test requires both landed rows to use the counted flag, scope and reveal. `counts_as_activity` is read only by that test's six-floor count (Ironwood is `false`); no runtime code reads it. `smoke_local_requests.gd` still needs one run under the writer lock on this tree.
...
- **Unstaged** ordinary-discovery witness from a real save showing all six lures, and one code-blind verdict on distinctness and usefulness (F03 item 3).
- Two-peer payout witness for the vault and Hall receipts.
- The Hall row's interim reveal: `hall_approach_open` is set at the Sigil gate, after the alpha at z≈7255, so the row may reveal only after the activity is done. An earlier reveal needs a new `flag_scopes.json` entry.
- Doss: one logged batch-5 pass after the other activities; the earlier order-dependent repeat failure (47774c350) has not been reproduced, so it stays on watch rather than closed.
- The cart owner question; Farro's night lure and missing witness columns.
- Closed on this branch (kept for history): the vault's current-main Heartstone/reload failure (fixed by the `vegetation.gd::clear_area` all-layer gather-point sweep, failing-first `test_vegetation_clear_area_gather_points`), the Hall road-engage failure (batch-5 `--hall-activity` passes) and the Hall optionality question (`--hall-decline`).
