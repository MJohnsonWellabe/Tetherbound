...
- `smoke_alpha_pins.gd --hall-decline` (optionality, receipt `b5-hall-decline-a329d57ae.log`): real road input walked the spine past the pack without a fight; closest approach 17.9 m, outside the pack's 14 m notice range, so the road stays outside its notice envelope by geometry. One run. The Stronghold approach also has table-rolled aggressor clusters on the road (5011 engaged an earlier run); those are route danger, not this activity, and the witness fights through them.
