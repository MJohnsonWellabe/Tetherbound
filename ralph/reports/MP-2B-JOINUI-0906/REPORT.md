...
- **`tests/smoke_net_sleep_vote.gd.uid` was missing from the tree** (lane 5.D
  shipped the script without it; every other `.gd` under `tests/` has a tracked
  one). Regenerated by this lane's import and committed here, in its own commit
  so it is obvious it is not part of the join work. It is one deterministic line
  derived from the script, so if lane 5.D adds it too the content is identical.
