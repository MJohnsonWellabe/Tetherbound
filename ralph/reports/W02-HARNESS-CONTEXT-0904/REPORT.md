...
Untracked and deliberately **not** committed (all generated by Godot's import of files OTHER
lanes committed without their sidecars, and outside this lane's ownership — the coordinator
should route them to the owning lanes, since this repo normally tracks them): `.uid` files for
eight `tests/` scripts and nine `tools/` scripts (e.g. `tests/smoke_trainer_no_ally_deployed.gd.uid`,
`tools/gate_f/probe_ironwood_approach.gd.uid`); `.import` files for the reference PNGs under
`assets/creatures/tetherbound/{candy_pickup,meadows_bridge_section,meadows_signpost,mushroom_pickup,potion_plant,revive_flower,riding_saddle,south_bridge_gate}/reference/`;
and the `.glb.import` files plus GLB-extracted textures (`candy_pickup_0.png`,
`potion_plant_0/1.png`, `riding_saddle_0.jpg`, `south_bridge_gate_0.jpg`, …) under
`assets/props/{candy_pickup,mushroom_pickup,potion_plant,revive_flower,riding_saddle}/` and
`assets/environment/team_tether/`. `git status` on this branch lists them all.
