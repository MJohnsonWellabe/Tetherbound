...
- predicted cost 1219 s over 205928 frames at 0.0059 s/frame (re-priced after each boot)
...
- actual: wiped 2 files from /tmp/w21-userdata/S07/godot/app_userdata/Tetherbound/saves (kept slots [])
- events: t=0.23
- verdict: PASS
...
- actual: booted title in 486 ms (30 settle frames); re-priced at boot:title: 0.0065 s/frame (was 0.0059), 205895 frames left + 0 s boot = 1344 s against 14399 s of budget left
- events: t=0.87
- verdict: PASS
...
- actual: inventory opened the shell: context world -> menu_backpack, focus on '' (@Button@67531)
- events: t=288.65
- verdict: PASS
...
- actual: fought 1468 frames: 43 quick, 0 handover(s), 0 refused switch(es); ended because flag 'defeated_band2_outrider_kest' set, SET 'defeated_band2_outrider_kest'
- events: t=321.67
- verdict: PASS
...
- actual: SKIPPED press: not needed (active creature at 121.4 HP)
- events: t=329.67
- verdict: SKIP
...
- actual: walked 531.7 m to (150, 3500) in 6749 walking frames (0 held)
- events: t=443.17
- verdict: PASS
...
- expected: a note event carries this observation into events.jsonl
- actual: G3-BAND3-0903, harness defect found and fixed. This was a hard `region_is == the_long_water` assert and it cannot pass at this waypoint: `map_landmarks.json`'s own `the_long_water` is a 52m-radius circle centred near (-150,4200) (its own comment records being cut down from 72m specifically so it would not swallow the Tether Relay's centre), and S07-23's walk target for 'the river first visible' is (150,3500) -- roughly 700m away, on the corridor's own approach to the river, not inside the named landmark. Reproduced in a real run: region read 'corridor' (the harness's own fallback label for open corridor ground, `gate_f_probe.gd::CORRIDOR`), not a game defect -- the river genuinely is visible from open corridor ground well before the player is within 52m of the crossing. Converted to a note recording the real region rather than asserting a location this waypoint was never going to be inside.
- events: t=443.17
- verdict: PASS
...
- actual: walked 204.7 m to (241, 3680) in 2493 walking frames (0 held)
- events: t=484.73
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.
...
- expected: game_menu.gd::select() grabs tab_backpack.gd::first_focus() on open, so the item grid owns focus
- actual: inventory opened the shell: context world -> menu_backpack, focus on '' (@Button@69401)
- events: t=484.90
- verdict: PASS
...
- expected: revive-then-cycle, the order ralph/reports/gate-f-run-20260901T220548Z-s03fix measured over three attempts. SKIPPED when the lead is already standing; a real pilot handoff when it is not.
- actual: SKIPPED press: not needed (active creature at 156.4 HP)
- events: t=486.12
- verdict: SKIP
...
- expected: CL-H1 (W21-HARNESS-FIGHTS-0904). the party-health assert CL-H1 asks for before each challenge. `active_creature_alive` fails the segment honestly at the moment the party stopped being fight-ready, instead of letting encounter_director.gd::can_challenge()'s silent refusal (or a scripted press landing on the no-usable-creature conversation instead of a fight) burn the rest of the budget unexplained.
- actual: active creature at 156.4 HP
- events: t=487.12
- verdict: PASS
...
### S07-32 — fight Hess
- expected: Section E.1 per-fight record
- actual: fought 1275 frames: 34 quick, 0 handover(s), 0 refused switch(es); ended because flag 'defeated_relay_hess' set, SET 'defeated_relay_hess'
- events: t=513.55
- verdict: PASS
- observation: G3-BAND3-0903, harness defect found and fixed. This step was a fixed `combat_quick x32` press block with no swap-safety net, unlike Vance's own multi-block sequence below. Traced in a real run's telemetry (ralph/reports/G3-BAND3-0903/gate-f-s07/): the 32-press budget ran out with Hess's second creature (Mudsnout) still alive at roughly 11 HP, combat never formally ended, `input_context` never returned to `world`, and every step from S07-34 (walking to Orrin) onward executed against a player still frozen in that same stuck fight for the rest of the run -- the whole five-creature party was ground down to 0 HP with no further attack input ever sent. `operator_harness.gd::_step_fight` already exists for exactly this (`until_flag` + an HP-gated `party_cycle` handover) and was simply never adopted for this trainer; switched to it rather than guessing a larger fixed count that could still run out against an unlucky roll.
...
- expected: CL-H1 (W21-HARNESS-FIGHTS-0904). the post-faint switch. SKIPPED when the fight left the lead standing; a real pilot handoff when it fainted. Bag-free, so it still recovers the belt with an empty satchel.
- actual: SKIPPED press: not needed (active creature at 103.4 HP)
- events: t=521.55
- verdict: SKIP
...
- actual: walked 54.2 m to (284, 3710) in 945 walking frames (0 held)
- events: t=538.32
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.
...
- expected: game_menu.gd::select() grabs tab_backpack.gd::first_focus() on open, so the item grid owns focus
- actual: inventory opened the shell: context world -> menu_backpack, focus on '' (@Button@70154)
- events: t=538.85
- verdict: PASS
...
- expected: revive-then-cycle, the order ralph/reports/gate-f-run-20260901T220548Z-s03fix measured over three attempts. SKIPPED when the lead is already standing; a real pilot handoff when it is not.
- actual: SKIPPED press: not needed (active creature at 148.0 HP)
- events: t=540.03
- verdict: SKIP
...
- expected: CL-H1 (W21-HARNESS-FIGHTS-0904). the party-health assert CL-H1 asks for before each challenge. `active_creature_alive` fails the segment honestly at the moment the party stopped being fight-ready, instead of letting encounter_director.gd::can_challenge()'s silent refusal (or a scripted press landing on the no-usable-creature conversation instead of a fight) burn the rest of the budget unexplained.
- actual: active creature at 148.0 HP
- events: t=541.03
- verdict: PASS
...
### S07-38 — fight Orrin
- expected: Section E.1 per-fight record
- actual: fought 1047 frames: 32 quick, 0 handover(s), 0 refused switch(es); ended because flag 'defeated_relay_orrin' set, SET 'defeated_relay_orrin'
- events: t=562.87
- verdict: PASS
- observation: G3-BAND3-0903, same fix and same reason as S07-32's own observation: a fixed press count with no swap-safety net is what stranded Hess's fight in this segment's own run history. Switched to `operator_harness.gd`'s existing `fight` action (`until_flag`, HP-gated `party_cycle` handover) before this trainer could reproduce the same class of defect.
...
- expected: CL-H1 (W21-HARNESS-FIGHTS-0904). the post-faint switch. SKIPPED when the fight left the lead standing; a real pilot handoff when it fainted. Bag-free, so it still recovers the belt with an empty satchel.
- actual: SKIPPED press: not needed (active creature at 106.1 HP)
- events: t=570.87
- verdict: SKIP
...
- actual: walked 79.4 m to (342, 3772) in 1214 walking frames (0 held)
- events: t=592.12
- verdict: PASS
...
- actual: walked 2.2 m to (343, 3771) in 25 walking frames (0 held)
- events: t=592.55
- verdict: PASS
- observation: G3-BAND3-0903: this waypoint was still the pre-V-1 coordinate, which is EXACTLY the failure S07-40g's own comment warns a future re-siting would cause -- 'a future re-siting of either the road or the gate shows up as one waypoint disagreeing with the map instead of a silent detour failure', quoted here because that is precisely what happened. Reproduced in a real run: the walk itself 'succeeded' (the old coordinate is still an ordinary walkable yard spot), so S07-44's challenge press found nobody there, no fight ever started, S07-47's own `fight_until_resolved` correctly gave up after 240 frames of no fight running, and the segment never reached Dell, the captain, the captive, the console or the crossing. Updated to Dell's real position; S07-40g's own gate waypoint (342.1,3771.6) is 1.4m from this one and left as-is since it still routes cleanly around the flank wall. answer_prompts note carried over unchanged from before this edit -- see below.
...
- expected: game_menu.gd::select() grabs tab_backpack.gd::first_focus() on open, so the item grid owns focus
- actual: inventory opened the shell: context world -> menu_backpack, focus on '' (@Button@70701)
- events: t=593.22
- verdict: PASS
...
- expected: revive-then-cycle, the order ralph/reports/gate-f-run-20260901T220548Z-s03fix measured over three attempts. SKIPPED when the lead is already standing; a real pilot handoff when it is not.
- actual: SKIPPED press: not needed (active creature at 169.4 HP)
- events: t=594.40
- verdict: SKIP
...
- expected: CL-H1 (W21-HARNESS-FIGHTS-0904). the party-health assert CL-H1 asks for before each challenge. `active_creature_alive` fails the segment honestly at the moment the party stopped being fight-ready, instead of letting encounter_director.gd::can_challenge()'s silent refusal (or a scripted press landing on the no-usable-creature conversation instead of a fight) burn the rest of the budget unexplained.
- actual: active creature at 169.4 HP
- events: t=595.40
- verdict: PASS
...
### S07-47 — fight Dell
- expected: Section E.1 per-fight record
- actual: fought 2857 frames: 74 quick, 1 handover(s), 0 refused switch(es); ended because flag 'defeated_relay_dell' set, SET 'defeated_relay_dell'
- events: t=647.63
- verdict: PASS
- observation: G3-BAND3-0903, same fix and same reason as S07-32's own observation. Dell fields three creatures against Hess's/Orrin's two, so this budget is larger than theirs rather than shared blindly.
...
- expected: CL-H1 (W21-HARNESS-FIGHTS-0904). the post-faint switch. SKIPPED when the fight left the lead standing; a real pilot handoff when it fainted. Bag-free, so it still recovers the belt with an empty satchel.
- actual: SKIPPED press: not needed (active creature at 171.5 HP)
- events: t=655.63
- verdict: SKIP
...
- actual: walked 16.0 m to (352, 3757) in 337 walking frames (0 held)
- events: t=662.27
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.
...
- expected: game_menu.gd::select() grabs tab_backpack.gd::first_focus() on open, so the item grid owns focus
- actual: inventory opened the shell: context world -> menu_backpack, focus on '' (@Button@71521)
- events: t=662.80
- verdict: PASS
...
- expected: CL-H1 (W21-HARNESS-FIGHTS-0904). the party-health assert CL-H1 asks for before each challenge. `active_creature_alive` fails the segment honestly at the moment the party stopped being fight-ready, instead of letting encounter_director.gd::can_challenge()'s silent refusal (or a scripted press landing on the no-usable-creature conversation instead of a fight) burn the rest of the budget unexplained.
- actual: active creature at 106.1 HP
- events: t=664.97
- verdict: PASS
...
- actual: pressed combat_charged x1 (long, 60 frames each) on mouse, resolved to MouseBtn:2
- events: t=670.60
- verdict: PASS
- observation: G3-BAND3-0903, REAL SHIPPED BUG FOUND (project.godot InputMap, not this segment, not this band's data): `combat_charged` and `build_shortcut` are BOTH bound to the identical physical input, JoyAxis:4 at axis_value 1.0 -- a player holding the left trigger for a charged attack on a controller also fires the Build shortcut. Reproduced in two full real runs of this exact step at the default (joypad) device: the instant the press landed, `input_context` flipped to `build_catalogue` and never returned, stranding every step for the rest of the segment (rescue the captive, disable the relay, restore the crossing, save) behind a menu nothing in the script ever closes. `device: "mouse"` routes this ONE step through `combat_charged`'s own separate mouse-button binding (button_index 2, which `build_shortcut` does not share) so this segment's own evidence is not lost to a bug outside Band 3's file ownership -- it does not fix the underlying collision, which needs an owner/input decision (rebind one of the two actions off JoyAxis:4) and is reported, not silently patched, in ralph/reports/G3-BAND3-0903/REPORT.md. Any OTHER segment that presses `combat_charged` on the default joypad device is still exposed and is not this edit's to chase.

### S07-54 — fight the Relay Captain
- expected: Section E.1 per-fight record; the HP margin at the end is band 3's difficulty read
- actual: fought 1853 frames: 52 quick, 1 handover(s), 0 refused switch(es); ended because flag 'relay_captain_defeated' set, SET 'relay_captain_defeated'
- events: t=701.90
- verdict: PASS
- observation: CL-H1 (W21-HARNESS-FIGHTS-0904). was `press combat_quick, times: N` -- the failure mode SEGMENT_SCHEMA.md names by name. A counted press block is right for exactly one matchup, and G3-BAND2's own telemetry watched all four party members faint in sequence inside one such block while every step reported PASS. `fight_until_resolved` presses combat_quick only while the action machine reads READY, presses party_cycle once when the pilot drops below switch_below of max HP (which is CB-09's switching-under-pressure, driven by the creature's real HP instead of a scripted beat), and stops only when both is_fighting() and trainer_battle_active() have been false for quiet_frames. The charged attack keeps its `device: "mouse"` override and now opens the fight; GATE-D3-SWAP's hand-written cycle-between-short-blocks ladder is replaced by fight_until_resolved's own HP-triggered switch, which is what that ladder was approximating with a press count.
...
- expected: CL-H1 (W21-HARNESS-FIGHTS-0904). the post-faint switch. SKIPPED when the fight left the lead standing; a real pilot handoff when it fainted. Bag-free, so it still recovers the belt with an empty satchel.
- actual: SKIPPED press: not needed (active creature at 138.9 HP)
- events: t=711.90
- verdict: SKIP
...
- actual: walked 8.6 m to (356, 3753) in 101 walking frames (0 held)
- events: t=714.60
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.
...
- actual: walked 5.9 m to (350, 3760) in 80 walking frames (0 held)
- events: t=722.82
- verdict: PASS
- observation: GATE-D3-DECK, KNOWN LIMIT, not chased further. `disable_the_relay` is NOT part of section B's own S07 span (river arrival -> relay pickets -> officer -> relay captain -> captive -> Old Mill Crossing restored); it is objective 20/27 in the full chain and this segment's own S07-69 note already records that the protocol gives no site for it. Three straight attempts (a flat walk, a move_to_entity('Console'), then a four-waypoint ramp/gantry/pad chain -- see this run directory's own S07-superseded-6/7/8) all closed the horizontal gap to the Console node but stayed 4.7-4.9m short vertically: the console sits on a raised pad (`deck_y: 10.0`) reached by a real ramp, and `tests/smoke_relay_station.gd` (independently run and PASSING on this same SHA) proves the ramp itself works -- 'walked up the ramp... reached y=9.98 against a deck top of y=10.00' -- but ALSO proves approaching it from the wrong angle legitimately does not climb ('walked at the pad's flank from the yard; reached y=5.22'), which is exactly what this segment's own general-purpose, wall-following move_to/move_to_entity kept doing despite increasingly precise waypoints. This is a harness-navigator precision limit on one narrow ramp, not a Band3 game defect -- verified, not assumed. Reverted to the flat ground-level walk so this segment does not strand the player on the ramp/pad and block the in-scope Old Mill Crossing walk after it (S07-superseded-9's own S07-76 failed as exactly that knock-on). The console interact below is expected to refuse (relay_disabled will not be set) until a future pass gives this its own precision-walked waypoint chain or a dedicated capture-lane probe.
...
- actual: walked 664.1 m to (-152, 4196) in 13871 walking frames (40 held)
- events: t=959.37
- verdict: PASS
...
- actual: walked 2433.1 m this segment (wanted >= 2200.0)
- events: t=965.32
- verdict: PASS

### S07-82 — the 2 Hz trace ran throughout
- expected: Section C.2
- actual: route.csv has 1915 rows (wanted >= 3000)
- events: t=965.32
- verdict: FAIL
...
- actual: dead_travel peaked at 431.3 m this segment (wanted >= 150.0); 2433.1 m walked in total
- events: t=965.32
- verdict: PASS
...
- actual: map opened the shell: context world -> menu_map, focus on '' (@Control@74496)
- events: t=965.48
- verdict: PASS
...
- actual: 4 x ui_down moved focus 'Save' (@Button@74549) -> 'Save' (@Button@74569)
- events: t=966.60
- verdict: PASS
...
- actual: slot 4 copied to saves/S07-exit.json (1420570 bytes)
- events: t=968.87
- verdict: PASS
