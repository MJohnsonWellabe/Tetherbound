...
**Additional P0 save-integrity finding:** `world_save.gd::write` and `character_save.gd::write` call `store_string`, close, then return true without testing the write result or replacing a temporary file atomically. The disk-full Guardian test demonstrated a truncated JSON read despite the writer's success result. Reproduce only in an isolated bounded test volume, not by filling a user's drive. Strongest next hypothesis is unchecked partial writes in the existing shared save layer. Fix and test temporary-write/flush/error/replace behavior before claiming durable transactions under storage exhaustion. The current rollback proofs inject a correctly reported failure; they do not cover an underlying writer falsely reporting success. This finding precedes release acceptance and belongs with bounded task3's save-integrity work.
...
Preservation push `14fd4e46d9ffa161f081e6988bc6faf445725233` is verified with `git ls-remote`. GitHub confirms PR69 remains draft/unmerged and reports `mergeable_state=dirty`; no final-head CI or merge-readiness claim is made. The fresh-session integration task must resolve those conflicts.

The redirected full suite found `test_gate_f_rig.gd::test_the_runner_declares_its_own_logic_lane_so_a_run_can_start` failing because `OS.execute("bash", ...)` returned-1. A single targeted retry with installed Git Bash in PATH launched bash but returned138: `child_copy: cygheap read copy failed`, Win32 error299, `fork: Resource temporarily unavailable`. Log `D:\water-gatef-path-regression.log`. This is an unresolved environment/harness blocker; do not weaken its assertion or run a third near-identical attempt. Retest in a healthy shell environment/CI after current-main integration. No full-suite green claim is valid while this remains failing.

The same run also found `test_scatter_fingerprint_covers_bands.gd::test_the_fingerprint_ignores_crlf_line_endings` failing because this Windows checkout's `data/config/vegetation.json` already contained CR bytes. The test assumes LF input before manufacturing CRLF, so it creates doubled carriage returns from the checkout. The adjacent LF/CRLF band and JSON-round-trip tests pass. Preserve this as a concrete checkout/harness reproduction; do not declare a Water terrain failure or a clean full suite without resolving it.
...
At12:38 UTC the redirected full run was deliberately stopped during the existing scatter suite under the owner's stop policy, after its two recorded failures. Its partial log contains2,128 methods marked`ok` and2 marked`FAIL`, plus existing off-tree fixture script errors. These counts are not a clean pass or the runner's final totals; later test files were not reached. The exact log is preserved as `evidence-final/full-suite-partial.log`. No third full-suite attempt or Bash retry was made. The Water test process was specifically identified by command line before termination; other project processes were left alone. Final acceptance and a complete green regression remain outstanding. The final documentation commit following14fd4e46d preserves this result and the additional save-integrity blocker; `git log -1` identifies it.
