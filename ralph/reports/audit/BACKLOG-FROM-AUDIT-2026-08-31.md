...
| `BACKLOG-VISUAL-CREATURE-ALBEDO-CLIPPING` | 31.9% pure-white on duskhush-shiny, 41.9% pure-black on burrowback-shiny, against 0.00%/≤0.01% on every Palworld creature sampled. **Investigated 2026-09-01, does not reproduce** — a fresh render off the same tool/textures on unmodified `main` measures 0.02–1.87% on all four cited frames, an order of magnitude under the census's numbers, and matches what the census's own committed contact sheet already shows. See `ralph/reports/audit/creature-albedo-clipping/FOLLOWUP-2026-09-01.md`. | Census 63 | ~~albedo/exposure/fill~~ verified, no change needed |
...
| `BACKLOG-VISUAL-CONTROLLER-GLYPHS` | the ability tray and interact prompt show `E`/`F`/`C` and mouse-button icons. **`CLAUDE.md`'s hard rules say controller first**, so this is a rule violation visible in a still. **Investigated 2026-09-01: capture-harness artefact — `tools/survey_combat.gd` never simulated a connected gamepad, so it rendered the game's correct no-controller default rather than the ROG Ally's always-connected-pad default; the live glyph resolver was already controller-aware. Tool patched, no gameplay/UI code changed — see `ralph/reports/audit/controller-glyphs/FOLLOWUP-2026-09-01.md`.** | Census 129 | glyph set |
...
| id | item | source | closing cost |
|---|---|---|---|
| `BACKLOG-VISUAL-INPUT-GLYPH-LANGUAGE` | five input languages at once; the exploration HUD shows **no gamepad glyph at all** (`M`/`I`/`R`/`C` keycaps, quickbar slots `1`–`5`), the tooltip shows a **mouse-click icon with no controller equivalent**, and two on-screen instructions for the same action contradict each other. Controller-first is a hard rule. **Investigated 2026-09-01: the "no gamepad glyph at all" half is a capture-harness artefact — `tools/capture_exploration_hud.gd` and `tools/capture_ui_suite.gd` never simulated a connected gamepad, unlike `tools/_capture_ui_survey.gd`'s already-fixed `_pin_owner_device()`; both tools patched, no gameplay/UI code changed. The "mouse-click icon with no controller equivalent" claim does not reproduce in the live `GLYPHS` table. The "contradiction" is real but narrower: two different verbs (pick-up vs. quick-bar-assign) shown via two different, already-existing display conventions (`icon()` auto-detect vs. `_verb_glyph()`'s explicit dual "key / [pad]"), which is the language-unification work this row's own closing-cost note already separates out — still open. See `ralph/reports/audit/controller-glyphs/FOLLOWUP-2026-09-01.md`.** | Census 148, 129 | data/config for the language; redrawing the two unreadable glyphs is small art |
...
- **D6-SEAM-PAINT-BOUNDARY** — `BACKLOG-D6-SEAM-PAINT-BOUNDARY` tested the one untested lead above directly: a gated `EMISSION` diagnostic (`diag_paint_boundary_mode`, mode 1 = hashed colour per base texture id, mode 2 = white where the base or over id changes across a fragment's own screen footprint) at the same reference camera stand. Result: **ruled out, cleanly**. The idmap reads uniform solid black (a single texture id, zero variation) across the entire seam crop box, and the idedge mask is entirely blank there — no control-map paint-boundary discontinuity anywhere near the dash, confirmed by direct per-pixel measurement rather than inference (the same diagnostic does show real boundaries elsewhere in frame, e.g. the dirt-path edge, confirming it fires correctly when a boundary genuinely exists). This closes the "maybe it's paint, not tiling" branch entirely — the standing diagnosis (same-texture per-tile detiling, both `id_detile` components together, trigger condition still unidentified) is now the only one left standing. One reusable finding en route: a new shader uniform meant to be toggled live via `set_shader_param` must not use Terrain3D's `_`-private-uniform naming convention, or the wrapper silently never wires the write through (no error either direction) — cost a full wasted render pass before being caught via the shader's own header comment. Branch again reverts `shaders/terrain_ground.gdshader` to a verified 0-line diff from `main`. Full writeup: `ralph/reports/audit/D6-seam-probe/PAINT-BOUNDARY-2026-08-31.md`.
