...
| `BACKLOG-VISUAL-BED-FITS-CREATURE` | the creature still does not fit its bed — hindquarters and rear paws outside the ring, front paws on the rim. **This is `BACKLOG-BED-SCALE-POSE`'s own subject, still failing after that lane's change**. **Investigated 2026-09-01, does not reproduce as a spatial defect** — the census frame came from a stale capture tool calling `play_faint()`, not the `play_rest()` every real occupant plays; top-down measurement puts terrapup fully inside the current rim in both poses. The real `play_rest()` pose has a worse, contained-but-broken defect instead (flips onto its back) — fixed for terrapup (`rest_roll_deg: 0`) and for the stale capture tool; trailpup/bramblebun's copy of the flip is tracked separately as `BACKLOG-VISUAL-BED-REST-ROLL-FLIP`. See `ralph/reports/audit/bed-fits-creature/FOLLOWUP-2026-09-01.md`. | Census 113 | ~~one scale value, then re-render~~ pose data fix, no scale touched |
| `BACKLOG-VISUAL-BED-REST-ROLL-FLIP` | `play_rest()`'s 90-degree roll flips trailpup and bramblebun onto their backs / into a tangle instead of settling them on their side (same root cause `BACKLOG-VISUAL-BED-FITS-CREATURE` found and fixed for terrapup) — contained within the bed rim, so not a fit defect, but a broken "does this creature look right resting" pose | Follow-up from `BACKLOG-VISUAL-BED-FITS-CREATURE` investigation, 2026-09-01 | needs a per-species `rest_roll_deg` survey across the roster, or a smarter roll grounded on bind-pose paw position — not bite-sized as a full pass; bite-sized per species |
