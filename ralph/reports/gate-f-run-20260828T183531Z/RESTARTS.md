...
| S07 | `bad057f6` | (unchanged — re-run only) | Not a segment failure. The session driving this attempt was reclaimed within the first second of play: telemetry shows only the entry-save handoff mechanics (wipe live saves, seed slot 4 from `S06/saves/S06-exit.json`, boot to title, focus Load Game, press accept) through the load-time bootstrap `flag_set`/`catch_result`/`faint`/`landmark_discover` quartet that always fires at `t=1.05` on restore — the same load-restore artefact already named in the S08-superseded-1 entry above, not a fight or gather that happened in S07. No `region_enter` past the title, no `INVENTORY.json`, no `notes/S07.md`. The partial directory is preserved as `S07-superseded-2/` rather than deleted, but it is **not evidence of anything** and no verdict in it may be read. Re-run from the same entry save, `S06/saves/S06-exit.json`, at the same rig SHA — no rig change needed, only the re-run. |
