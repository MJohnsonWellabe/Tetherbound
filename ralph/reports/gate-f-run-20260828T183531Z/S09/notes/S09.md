...
- predicted cost 637 s over 107648 frames at 0.0059 s/frame (re-priced after each boot)
...
- actual: booted title in 499 ms (30 settle frames); re-priced at boot:title: 0.0065 s/frame (was 0.0059), 107615 frames left + 0 s boot = 702 s against 14399 s of budget left
- events: t=0.88
- verdict: PASS
...
- actual: map opened the shell: context world -> menu_map, focus on '' (@Control@62920)
- events: t=181.77
- verdict: PASS
...
- actual: FAIL did not reach (64, 7400) in 49500 walking frames; stopped 6079.4 m short at (2.0, -3.0, 1321.0) (0 held)
- events: t=1007.20
- verdict: FAIL
...
- actual: FAIL did not reach (-68, 7140) in 14400 walking frames; stopped 5822.1 m short at (9.0, -3.0, 1318.0) (0 held)
- events: t=1251.50
- verdict: FAIL
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.
...
- actual: FAIL did not reach (45, 7440) in 15300 walking frames; stopped 6121.7 m short at (11.0, -3.0, 1318.0) (0 held)
- events: t=1538.05
- verdict: FAIL
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.
...
- actual: map opened the shell: context world -> menu_map, focus on '' (@Control@84196)
- events: t=1570.30
- verdict: PASS
...
- actual: FAIL did not reach (150, 7595) in 9000 walking frames; stopped 6278.1 m short at (11.0, -3.0, 1318.0) (0 held)
- events: t=1737.43
- verdict: FAIL
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.
...
- actual: 6278.1 m from (150, 7595), wanted within 12.0
- events: t=1737.43
- verdict: FAIL
...
- actual: walked 6031.0 m this segment (wanted >= 1800.0)
- events: t=1737.43
- verdict: PASS

### S09-59 — the 2 Hz trace ran throughout
- expected: Section C.2
- actual: route.csv has 3457 rows (wanted >= 3000)
- events: t=1737.43
- verdict: PASS
...
- actual: dead_travel peaked at 0.3 m this segment (wanted >= 150.0); 6031.0 m walked in total
- events: t=1737.43
- verdict: FAIL
...
- actual: FAIL map did not open the pause shell: context build_catalogue -> build_catalogue (owner=@CanvasLayer@84236)
- events: t=1737.73
- verdict: FAIL
- observation: Opened via the map shortcut rather than a bare game_menu press. The pause shell REOPENS ON THE LAST TAB USED, so a fixed press count from an assumed backpack start lands on the wrong tab in any segment that opens the menu more than once. Measured in S03: the shell reopened on `menu_map` and 4x menu_tab_right landed on `menu_settings` instead of `menu_build`, failing nine consecutive build steps. `map` is one of only two tabs with a shortcut in data/config/menu.json, so it is the only deterministic starting point available.
...
- actual: FAIL 4 x ui_down did not move focus off @Button@223176
- events: t=1738.28
- verdict: FAIL
...
- actual: slot 4 copied to saves/S09-exit.json (1415593 bytes)
- events: t=1740.37
- verdict: PASS
