...
- predicted cost 129 s over 21788 frames at 0.0059 s/frame (re-priced after each boot)
...
- actual: seeded slot 4 from ralph/reports/gate-f-run-20260902T103755Z-catchtest/S02/saves/S02-exit.json (1418492 bytes)
- events: t=0.23
- verdict: PASS
...
- actual: booted title in 487 ms (30 settle frames); re-priced at boot:title: 0.0065 s/frame (was 0.0059), 21755 frames left + 0 s boot = 142 s against 14399 s of budget left
- events: t=0.85
- verdict: PASS
...
- actual: walked 5.1 m to bramblebun (species_id, nearest of 64 (Wild_bramblebun_0_1, Wild_bramblebun_0_2, Wild_bramblebun_0_3, Wild_bramblebun_1003_1, Wild_bramblebun_1003_2, Wild_bramblebun_1006_1, ...)) in 58 walking frames (0 held)
- events: t=200.63
- verdict: PASS
...
- expected: the target is WEAKENED to roughly 30% of its own max HP or less -- not defeated, a defeated wild cannot be caught -- landing the throw in catching.json's steep reward band (hp_factor climbs from ~0.10 at full HP toward ~1.0 near empty) instead of its flat top.
- actual: 5 x combat_quick reached it: enemy hp fraction 0.291 (27.3/93.7)
- events: t=206.73
- verdict: PASS
- observation: RIG, 2026-08-31: `combat_quick` x20 KILLED a 106 HP target, so a fixed `times: 3` (S02's own proven opener) replaced it -- correctly avoiding a kill, but measured directly this run (gate-f-run-20260902T053310Z-s03enginefix) landing anywhere from 57% to 75% of max HP depending on the creature's own defence, the FLAT part of hp_factor's curve. Requested directly by the owner in this session, 2026-09-02 ('we need to chip them down further'): this now reads the LIVE enemy_hp_fraction (operator_harness.gd's new check, added for this) after every hit and stops at the threshold instead of a guessed count. Bounded against the same overkill this step's own history already hit once: quick attacks measured 8%-14% of max HP each in that same run, so a 30% floor leaves comfortable margin above zero, and `max_presses: 10` caps the loop if a tankier target does not converge -- FAILing loud with whatever fraction it actually reached rather than hanging.
...
- actual: reticle confirmed on body after 26 tracked frame(s): eligible
- events: t=207.57
- verdict: PASS
...
- expected: a note event carries this observation into events.jsonl
- actual: One catch attempt only, for fast iteration on the chip-down/catch mechanics. Check party_size (S03-11 confirmed 2 at entry) and the last catch_result/party grew observation in telemetry/events.jsonl for the outcome.
- events: t=213.73
- verdict: PASS
