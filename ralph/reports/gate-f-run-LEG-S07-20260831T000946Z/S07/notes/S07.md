# S07 — River & Relay / Band 3: river arrival -> relay pickets -> officer -> relay captain -> captive -> Old Mill Crossing restored

### preflight — capture not required
- segment declares no captures and no continuous record
- predicted cost 921 s over 155670 frames at 0.0059 s/frame (re-priced after each boot)

### S07-01 — how this segment is entered
- expected: a note event carries this observation into events.jsonl
- actual: Section B: this segment's entry save is `S06-exit`. The three steps below are the handoff's own mechanics: the live save directory is emptied first so the title's Load list has exactly one non-empty slot and auto-focuses it (`title_screen.gd` focuses the FIRST non-empty slot), then slot 4 is seeded from the run directory, then the load is driven through the production title-screen Load path by synthetic input. Section B names the seed and the Load path; the wipe is not named there and is recorded here as an added mechanical step, needed because an autosave left in slot 0 would take the auto-focus and the segment would load the wrong state.
- events: t=0.22
- verdict: PASS

### S07-02 — empty the live save directory
- expected: every slot file is gone, so the seeded slot 4 is the only thing the title's Load list can offer
- actual: wiped 2 files from /root/.local/share/godot/app_userdata/Tetherbound/saves (kept slots [])
- events: t=0.22
- verdict: PASS

### S07-03 — seed slot 4 from the previous segment's exit save
- expected: Section B: 'The next segment boots fresh, restores that file into `user://`, and loads it through the production title-screen Load path'
- actual: seeded slot 4 from ralph/reports/gate-f-run-LEG-S07-20260831T000946Z/saves/S06-exit.json (5489 bytes)
- events: t=0.22
- verdict: PASS

### S07-04 — boot the real title screen
- expected: the real title comes up with Start New Game focused and Load Game enabled, because a save now exists
- actual: booted title in 477 ms (30 settle frames); re-priced at boot:title: 0.0065 s/frame (was 0.0059), 155637 frames left + 0 s boot = 1016 s against 14399 s of budget left
- events: t=0.85
- verdict: PASS

### S07-05 — the title focuses something
- expected: the title focuses its first button; without focus a controller cannot reach Load Game at all
- actual: focus_owner=@Button@27 focus_text=Start New Game
- events: t=0.85
- verdict: PASS

### S07-06 — move focus to Load Game
- expected: d-pad down moves focus from Start New Game to Load Game
- actual: 1 x ui_down moved focus 'Start New Game' (@Button@27) -> 'Load Game' (@Button@28)
- events: t=0.88
- verdict: PASS

### S07-07 — open the slot list
- expected: Section B: 'driven with synthetic input -- this is itself save/load coverage'. Load Game opens the slot list and auto-focuses the first non-empty slot, which is the seeded slot 4
- actual: pressed ui_accept x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:0
- events: t=0.97
- verdict: PASS

### S07-08 — load the seeded slot
- expected: the slot loads and the title transitions into the Meadows; section I.4 records the load duration
- actual: pressed ui_accept x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:0
- events: t=1.55
- verdict: PASS

### S07-09 — let the loaded world stand up
- expected: a cold Meadows stand-up costs about 90 s of CPU on this container -- 466k scattered props and a terrain build -- and every assertion after this reads a live Player
- actual: waited 10800 physics frames
- events: t=181.55
- verdict: PASS

### S07-09a — deploy the active creature after load
- expected: RIG-11: a load restores the party and deploys nothing -- encounter_director.gd::_engageable() returns null on a null ally before any distance check, so no combat/catch/gate-flag assertion downstream of a load can mean anything without this. creature_recall (RB) summons the active creature so this segment's fights can actually start.
- actual: pressed creature_recall x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:10
- events: t=181.92
- verdict: PASS

### S07-10 — the team travelled
- expected: three entrants into band 3
- actual: party size 5 (wanted >= 3)
- events: t=181.92
- verdict: PASS
- observation: RIG-15: equals -> min. Catching is probabilistic; a script's single throw can miss on a real, non-buggy roll, and a team that ends up BIGGER than the milestone is still a team that reached it. `equals` failed both directions for no reason a real player's outcome should fail on.

### S07-11 — the relay captain is tracked
- expected: Section E.5 objective 18/27 `defeat_the_relay_captain`, 'Defeat the Relay Captain.'
- actual: tracked objective id=relay_captain_defeated text=Defeat the Relay Captain. (wanted defeat_the_relay_captain = flag_id relay_captain_defeated) [matched on entry id -> flag_id]
- events: t=181.92
- verdict: PASS

### S07-12 — Section H cadence for this segment
- expected: a note event carries this observation into events.jsonl
- actual: Section H: 0.1 Hz segment plus event-forced frames, with the band 2 -> band 3 handoff window inside it. No background-recorder action exists in the vocabulary; recorded as a gap.
- events: t=181.92
- verdict: PASS

### S07-12b — Section H band-handoff windows in this segment
- expected: a note event carries this observation into events.jsonl
- actual: Section H: 'Mandatory for the highest-risk segments: ... every band handoff +/-60 s (region transitions)'. The pairs in this segment raise the background recorder from the 0.1 Hz journey baseline to 0.5 Hz across each band boundary this segment crosses, and `record_stop` returns to the baseline rather than switching the recorder off, so the record stays continuous either side. AMBIGUITY RECORDED: +/-60 s cannot be sized exactly from a step script. A `move_to` leg's duration is not known before the run and a leg cannot be split, so each window is placed to contain the crossing plus as much of its approach and its settle as the surrounding steps allow -- erring long, because the schema's own measurement makes a 0.5 Hz window affordable as a window (1.05 ms/frame for its length) while a window that missed the transition would cost the evidence.
- events: t=181.92
- verdict: PASS

### S07-13 — open the Map tab
- expected: Section E.5 at a major objective
- actual: map opened the shell: context world -> menu_map, focus on '' (@Control@66421)
- events: t=182.10
- verdict: PASS

### S07-14 — zoom out to see band 3
- expected: Section E.5's map-usefulness record
- actual: pressed map_zoom_out x3 (tap, 1 frames each) on the default device, resolved to JoyAxis:4:1.0
- events: t=182.38
- verdict: PASS

### S07-15 — close the shell
- expected: B closes the shell
- actual: menu_cancel closed the shell: context menu_map -> world
- events: t=182.60
- verdict: PASS

### S07-16 — Section E.5 nav note -- defeat_the_relay_captain
- expected: a note event carries this observation into events.jsonl
- actual: Section E.5 navigation record, objective 18/27 `defeat_the_relay_captain`. Wording: 'Defeat the Relay Captain.' It names WHAT and no WHERE at all -- it names a person and a place-word ('Relay') that the player may or may not have on the map yet. Player-visible information: `the_tether_relay` is a `major` map landmark at (350,3760) with its own relay icon; the ground ahead falls toward a river. Route decision: continue north to the river and follow the near bank east to the relay pin, because the objective's only locational word is the relay's own name. Reasoned from the map frame taken above and from GF-09-WARRENS-01 (S06). Operator: record whether the relay was a pin before it was discovered.
- events: t=182.60
- verdict: PASS

### S07-17 — walk north toward the river
- expected: Section D RT-08: 'Warrens exit -> river -> Tether Relay | -> (350,3760)'. `band2_outrider_kest` stands at (0,2980) on this leg
- actual: walked 303.2 m to (0, 2980) in 3798 walking frames (0 held)
- events: t=245.92
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.

### S07-18 — challenge the band 2 outrider
- expected: an optional trainer standing on the route between bands; section D counts trainer encounters per route
- actual: pressed interact x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=246.12
- verdict: PASS

### S07-19 — hear him out
- expected: record what he offers and whether a player would take it
- actual: pressed interact x10 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=250.53
- verdict: PASS

### S07-20 — let the fight stage
- expected: the combat camera acquires both combatants
- actual: waited 180 physics frames
- events: t=253.53
- verdict: PASS

### S07-21 — fight the outrider
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x30 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=272.57
- verdict: PASS

### S07-22 — let the fight end
- expected: rewards and control return
- actual: waited 480 physics frames
- events: t=280.57
- verdict: PASS

### S07-22a — record_start: band 2 -> band 3, the river arrival
- expected: the window opens before the walk on to the bank, so the approach to the river is inside it -- GF-10-RELAY-01's whole point is whether the river reads as a regional barrier before it is reached
- actual: DELEGATED the §H record window "band 2 -> band 3, the river arrival" to capture lane S07C (this is the logic lane; it keeps no continuous record)
- events: t=280.57
- verdict: DELEGATED

### S07-19w — the world owns input before this walk
- expected: no narrative modal is holding locomotion when this walk starts
- actual: input_context=world (wanted world)
- events: t=280.57
- verdict: PASS
- observation: T2-GATEF-RUN6 modal-holds-locomotion audit. A walk that begins while a narrative modal still owns input does not fail as 'a modal is open' -- it fails as 'did not reach', many steps and sometimes many minutes later, which is how X04 lost its whole combat block to 3601 held frames. This assert is diagnostic and changes no behaviour: it turns that silent cascade into one named cause at the point of origin. RIG-25's rule, applied to narrative modals rather than to shop panels.

### S07-23 — walk on to the river bank
- expected: Section D RT-08's middle: the river is the regional barrier band 3 is built around
- actual: walked 539.6 m to (150, 3500) in 7017 walking frames (0 held)
- events: t=397.53
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.

### S07-24 — capture GF-10-RELAY-01
- expected: Section G: river first visible, RT-08 -- the river reads as a regional barrier
- actual: DELEGATED GF-10-RELAY-01 to capture lane S07C (this is the logic lane; it takes no frames)
- events: t=397.53
- verdict: DELEGATED

### S07-25 — capture GF-21-WEATHER-03
- expected: Section G: each weather preset first naturally encountered (clear/cloudy/fog/rain) -- weather identity; palette holds
- actual: DELEGATED GF-21-WEATHER-03 to capture lane S07C (this is the logic lane; it takes no frames)
- events: t=397.53
- verdict: DELEGATED

### S07-26 — the river region contains the arrival
- expected: map_state.gd's containment puts the player in `the_long_water`
- actual: region=corridor (wanted the_long_water)
- events: t=397.53
- verdict: FAIL

### S07-26a — record_stop: back to the 0.1 Hz journey baseline after band 2 -> band 3, the river arrival
- expected: the window closes after the region assert puts the player in `the_long_water`, so the region_enter event and its forced frame are both inside the raised window
- actual: DELEGATED the close of the §H record window to capture lane S07C (this is the logic lane; no window was opened here)
- events: t=397.53
- verdict: DELEGATED

### S07-27 — walk to the first relay picket
- expected: Section E.1 CB-04: 'S07: relay ladder + relay_captain'. relay_picket_hess stands at (241.3,3680)
- actual: walked 204.4 m to (241, 3680) in 2700 walking frames (0 held)
- events: t=442.55
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.

### S07-28 — challenge Hess
- expected: the first rung of the relay ladder
- actual: pressed interact x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=442.82
- verdict: PASS

### S07-29 — hear him out
- expected: Team Tether's occupation escalating
- actual: pressed interact x10 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=448.63
- verdict: PASS

### S07-30 — let the fight stage
- expected: the combat camera acquires both combatants
- actual: waited 180 physics frames
- events: t=451.63
- verdict: PASS

### S07-31 — capture GF-14-COMBAT-04c
- expected: Section G: CB-04: representative fight per band -- S07, relay ladder + relay_captain -- per-case: band 3's representative fight
- actual: DELEGATED GF-14-COMBAT-04c to capture lane S07C (this is the logic lane; it takes no frames)
- events: t=451.63
- verdict: DELEGATED

### S07-32 — fight Hess
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x32 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=479.78
- verdict: PASS

### S07-33 — let the fight end
- expected: rewards and control return
- actual: waited 480 physics frames
- events: t=487.78
- verdict: PASS

### S07-29w — the world owns input before this walk
- expected: no narrative modal is holding locomotion when this walk starts
- actual: input_context=world (wanted world)
- events: t=487.78
- verdict: PASS
- observation: T2-GATEF-RUN6 modal-holds-locomotion audit. A walk that begins while a narrative modal still owns input does not fail as 'a modal is open' -- it fails as 'did not reach', many steps and sometimes many minutes later, which is how X04 lost its whole combat block to 3601 held frames. This assert is diagnostic and changes no behaviour: it turns that silent cascade into one named cause at the point of origin. RIG-25's rule, applied to narrative modals rather than to shop panels.

### S07-34 — walk to the second relay picket
- expected: relay_picket_orrin at (284,3710.5), the ladder's second rung
- actual: walked 51.3 m to (284, 3710) in 664 walking frames (0 held)
- events: t=498.87
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.

### S07-34c — rotate to a fresh ally before Orrin
- expected: a level-headed five-creature roster does not run the same pilot through every fight on the ladder
- actual: pressed party_cycle x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:9
- events: t=499.27
- verdict: PASS
- observation: GATE-D3-STAMINA. Measured on this run's own telemetry (before this fix): Hess and Orrin alone wore the deployed ally (Moss, L13) from 180.6hp to 0 by the end of Dell's fight, three real trainer fights on one creature with nothing between them -- and the captain never got a healthy pilot at all, which is why relay_captain_defeated stayed unset even once his fight eventually engaged. Four real fights (Hess, Orrin, Dell, Vance) against a five-creature party is exactly the shape party_cycle exists for; a script that never presses it is not testing the game's own difficulty, it is testing one un-rotated creature's stamina.

### S07-35 — challenge Orrin
- expected: the second rung of the relay ladder
- actual: pressed interact x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=499.48
- verdict: PASS

### S07-36 — hear him out
- expected: the ladder escalates rung by rung
- actual: pressed interact x10 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=504.88
- verdict: PASS

### S07-37 — let the fight stage
- expected: the combat camera acquires both combatants
- actual: waited 180 physics frames
- events: t=507.88
- verdict: PASS

### S07-38 — fight Orrin
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x32 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=533.28
- verdict: PASS

### S07-39 — let the fight end
- expected: rewards and control return
- actual: waited 480 physics frames
- events: t=541.28
- verdict: PASS

### S07-40 — capture GF-10-RELAY-02
- expected: Section G: relay compound approach, pylons in frame, (350,3760) approach -- Team Tether occupation escalation
- actual: DELEGATED GF-10-RELAY-02 to capture lane S07C (this is the logic lane; it takes no frames)
- events: t=541.28
- verdict: DELEGATED

### S07-36w — the world owns input before this walk
- expected: no narrative modal is holding locomotion when this walk starts
- actual: input_context=world (wanted world)
- events: t=541.28
- verdict: PASS
- observation: T2-GATEF-RUN6 modal-holds-locomotion audit. A walk that begins while a narrative modal still owns input does not fail as 'a modal is open' -- it fails as 'did not reach', many steps and sometimes many minutes later, which is how X04 lost its whole combat block to 3601 held frames. This assert is diagnostic and changes no behaviour: it turns that silent cascade into one named cause at the point of origin. RIG-25's rule, applied to narrative modals rather than to shop panels.

### S07-40g — walk to the compound gate
- expected: GATE-D3-GEOM: the picket road's own approach bearing (the (230,3670)->(350,3760) leg, unit (0.8,0.6)) runs 92 degrees off tether_relay.json's own approach_bearing_deg (-34.4, unit (0.565,-0.826)) -- almost exactly parallel to the compound's front wall rather than toward it. A beeline from the road straight at Officer Dell's own coordinate clips the flank_west fence and the wall-following navigator cannot always work all the way around it inside one leg's budget (measured: tools/_probe_relay_gate_reach.gd, stuck-point replay, both legs succeed once routed through the gate first). Named here as its own waypoint rather than folded into S07-41's `at`, so a future re-siting of either the road or the gate shows up as one waypoint disagreeing with the map instead of a silent detour failure.
- actual: walked 82.3 m to (342, 3772) in 1424 walking frames (0 held)
- events: t=565.03
- verdict: PASS

### S07-41 — walk to Officer Dell
- expected: Section B: S07's span names the officer between the pickets and the captain. relay_officer_dell stands at (347.5,3763.5)
- actual: walked 10.7 m to (348, 3764) in 124 walking frames (0 held)
- events: t=567.12
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.

### S07-42 — the relay region contains the arrival
- expected: map_state.gd's containment puts the player in `the_tether_relay`
- actual: region=the_tether_relay (wanted the_tether_relay)
- events: t=567.12
- verdict: PASS

### S07-43 — rescan for points of interest
- expected: the compound's own content joins the POI set
- actual: 1089 points of interest in the tree
- events: t=567.12
- verdict: PASS

### S07-43c — rotate to a fresh ally before Dell
- expected: GATE-D3-STAMINA (S07-34c's own note): the third real fight on the ladder, still on a five-creature roster
- actual: pressed party_cycle x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:9
- events: t=567.58
- verdict: PASS

### S07-44 — challenge Officer Dell
- expected: the ladder's third rung, inside the compound
- actual: pressed interact x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=567.78
- verdict: PASS

### S07-45 — hear him out
- expected: record what the occupation says for itself
- actual: pressed interact x10 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=572.43
- verdict: PASS

### S07-46 — let the fight stage
- expected: the combat camera acquires both combatants
- actual: waited 180 physics frames
- events: t=575.43
- verdict: PASS

### S07-47 — fight Dell
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x34 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=598.55
- verdict: PASS

### S07-48 — let the fight end
- expected: rewards and control return
- actual: waited 480 physics frames
- events: t=606.55
- verdict: PASS

### S07-45w — the world owns input before this walk
- expected: no narrative modal is holding locomotion when this walk starts
- actual: input_context=combat (wanted world)
- events: t=606.55
- verdict: FAIL
- observation: T2-GATEF-RUN6 modal-holds-locomotion audit. A walk that begins while a narrative modal still owns input does not fail as 'a modal is open' -- it fails as 'did not reach', many steps and sometimes many minutes later, which is how X04 lost its whole combat block to 3601 held frames. This assert is diagnostic and changes no behaviour: it turns that silent cascade into one named cause at the point of origin. RIG-25's rule, applied to narrative modals rather than to shop panels.

### S07-49 — walk to Captain Vance
- expected: Section E.1 CB-04: relay_captain at (352,3757), the band's required fight
- actual: walked 13.0 m to (352, 3757) in 151 walking frames (1439 held)
- events: t=635.97
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.

### S07-49c — rotate to a fresh ally before the captain
- expected: GATE-D3-STAMINA (S07-34c's own note): the band's required fight, and the ladder's fourth real battle in a row -- without this the deployed ally arrives at the hardest of the four already worn from three others
- actual: pressed party_cycle x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:9
- events: t=636.27
- verdict: PASS

### S07-50 — challenge the Relay Captain
- expected: Section E.5 objective 18/27's own target
- actual: pressed interact x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=636.45
- verdict: PASS

### S07-51 — hear the captain out
- expected: the band's named antagonist
- actual: advanced 4 line(s) over 4 press(es) of interact; DialoguePanel closed, context 'narrative_modal' -> 'combat'
- events: t=637.80
- verdict: PASS
- observation: GATE-D3-DIALOGUE. relay_captain_challenge (data/dialogue/trainers.json) runs 4 lines against the pickets'/officer's own 2 -- '_authority_not_menace' in trainers.json says why: 'He is the most articulate of the four; Tether personnel believe the barriers are necessary work.' The fixed `times: 12` this step used before (Hess/Orrin/Dell's own working `times: 10` plus a guessed +2) closed the panel to `input_context=narrative_modal`, not `world` -- the very next step (S07-53) asserted `input_context == combat` and got `narrative_modal` instead, no engage was ever received (S07-53f), and every objective/flag downstream of the captain's own defeat (rescue_the_captive, disable_the_relay, restore_the_mill_crossing) FAILed as the honest, correct consequence of a fight that never started. Switched to the schema's own by-predicate primitive rather than guessing a higher press count a fifth line would break again.

### S07-52 — let the fight stage
- expected: Section L.6 T05: the combat camera acquires both combatants at fight start
- actual: waited 180 physics frames
- events: t=640.80
- verdict: PASS

### S07-53 — combat owns input
- expected: input_context is 'combat'
- actual: input_context=combat (wanted combat)
- events: t=640.80
- verdict: PASS

### S07-53f — a fight is actually running
- expected: CombatManager::is_fighting() is true -- the engage was received, not merely pressed
- actual: combat_running=true (wanted true)
- events: t=640.80
- verdict: PASS
- observation: T2-GATEF-RUN6 / RIG-26, and the fix T5-FEEL's played-path evidence independently recommended: assert that a fight is RUNNING, not merely that combat-shaped input is owned. `input_context == "combat"` was the strongest claim these segments made after an engage, and it is not the same claim -- it is why two instrumented segments produced 128 events with zero `combat_start` and it read as a candidate GAME blocker for six runs.

### S07-54 — fight the captain
- expected: Section E.1 per-fight record; the HP margin at the end is band 3's difficulty read
- actual: pressed combat_quick x10 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=647.97
- verdict: PASS
- observation: GATE-D3-SWAP. This fight and the two before it were split into shorter combat_quick blocks with a party_cycle between each (was one 38-tap block then one swap): the third run's own telemetry showed a fresh but solo ally (Pipwing, undamaged at 120hp) taking hits every ~0.65s and fainting before the ORIGINAL single mid-fight swap (38 taps in) ever fired, losing the fight to Vance's three-creature team with two more full party members never touched. Real-time directly-piloted combat against the band's hardest trainer needs a pilot who responds to a faint quickly, not a script that mashes one block and checks once.

### S07-54b — swap out if this pilot is under pressure
- expected: GATE-D3-SWAP: cycle_active() (party.gd) already skips a fainted/resting member on its own, so this is a no-op on a healthy pilot and a real handoff on a hurt one -- safe every time
- actual: pressed party_cycle x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:9
- events: t=648.22
- verdict: PASS

### S07-54c — keep fighting
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x10 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=655.15
- verdict: PASS

### S07-54d — swap out if this pilot is under pressure
- expected: GATE-D3-SWAP: see S07-54b
- actual: pressed party_cycle x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:9
- events: t=655.35
- verdict: PASS

### S07-54e — keep fighting
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x10 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=662.18
- verdict: PASS

### S07-55 — switch pilot under pressure
- expected: pilot handoff and camera handoff mid-fight
- actual: pressed party_cycle x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:9
- events: t=662.38
- verdict: PASS

### S07-56 — keep fighting
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x13 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=671.10
- verdict: PASS

### S07-56b — swap out if this pilot is under pressure
- expected: GATE-D3-SWAP: see S07-54b
- actual: pressed party_cycle x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:9
- events: t=671.33
- verdict: PASS

### S07-56c — keep fighting
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x13 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=679.53
- verdict: PASS

### S07-57 — a charged attack
- expected: the charged verb's hold edge
- actual: pressed combat_charged x1 (long, 60 frames each) on the default device, resolved to JoyAxis:4:1.0
- events: t=680.75
- verdict: PASS

### S07-57b — swap out if this pilot is under pressure
- expected: GATE-D3-SWAP: see S07-54b
- actual: pressed party_cycle x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:9
- events: t=680.97
- verdict: PASS

### S07-58 — finish the captain
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x10 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=687.75
- verdict: PASS

### S07-58b — swap out if this pilot is under pressure
- expected: GATE-D3-SWAP: see S07-54b
- actual: pressed party_cycle x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:9
- events: t=687.95
- verdict: PASS

### S07-58c — finish the captain
- expected: Section E.1 per-fight record
- actual: pressed combat_quick x10 (tap, 1 frames each) on the default device, resolved to JoyAxis:5:1.0
- events: t=694.47
- verdict: PASS

### S07-59 — let the captain fight end
- expected: rewards and control return with no stuck modal
- actual: waited 600 physics frames
- events: t=704.47
- verdict: PASS

### S07-60 — the Relay Captain is beaten
- expected: `relay_captain_defeated` closes section E.5 objective 18/27
- actual: flag relay_captain_defeated set
- events: t=704.47
- verdict: PASS

### S07-61 — the captive is tracked
- expected: Section E.5 objective 19/27 `rescue_the_captive`, 'Find who Team Tether is holding at the relay.'
- actual: tracked objective id=captive_rescued text=Find who Team Tether is holding at the relay. (wanted rescue_the_captive = flag_id captive_rescued) [matched on entry id -> flag_id]
- events: t=704.47
- verdict: PASS

### S07-62 — Section E.5 nav note -- rescue_the_captive
- expected: a note event carries this observation into events.jsonl
- actual: Section E.5 navigation record, objective 19/27 `rescue_the_captive`. Wording: 'Find who Team Tether is holding at the relay.' It names WHAT and a WHERE the player is already standing in. Player-visible information: the compound itself, now that its ladder is beaten. Route decision: search the compound; no travel decision exists. Reasoned from GF-10-RELAY-02.
- events: t=704.47
- verdict: PASS

### S07-51w — the world owns input before this walk
- expected: no narrative modal is holding locomotion when this walk starts
- actual: input_context=world (wanted world)
- events: t=704.47
- verdict: PASS
- observation: T2-GATEF-RUN6 modal-holds-locomotion audit. A walk that begins while a narrative modal still owns input does not fail as 'a modal is open' -- it fails as 'did not reach', many steps and sometimes many minutes later, which is how X04 lost its whole combat block to 3601 held frames. This assert is diagnostic and changes no behaviour: it turns that silent cascade into one named cause at the point of origin. RIG-25's rule, applied to narrative modals rather than to shop panels.

### S07-63 — walk to the captive
- expected: data/config/relay_site.json's `people[0]` is Sela at (356,3753)
- actual: walked 10.9 m to (356, 3753) in 137 walking frames (0 held)
- events: t=706.77
- verdict: PASS
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.

### S07-64 — speak to the captive
- expected: Section B: S07's span names the captive between the captain and the crossing
- actual: pressed interact x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=706.95
- verdict: PASS

### S07-65 — hear her out
- expected: the reason the crossing can be restored at all
- actual: pressed interact x14 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=712.57
- verdict: PASS

### S07-66 — capture GF-10-RELAY-03
- expected: Section G: captive rescue / crossing restored moment, relay & (-152,4203) -- world visibly changes
- actual: DELEGATED GF-10-RELAY-03 to capture lane S07C (this is the logic lane; it takes no frames)
- events: t=712.57
- verdict: DELEGATED

### S07-67 — the captive is free
- expected: `captive_rescued` closes section E.5 objective 19/27
- actual: flag captive_rescued set
- events: t=712.57
- verdict: PASS

### S07-68 — the relay shutdown is tracked
- expected: Section E.5 objective 20/27 `disable_the_relay`, 'Shut down the Tether Relay.'
- actual: tracked objective id=relay_disabled text=Shut down the Tether Relay. (wanted disable_the_relay = flag_id relay_disabled) [matched on entry id -> flag_id]
- events: t=712.57
- verdict: PASS

### S07-69 — Section E.5 nav note -- disable_the_relay
- expected: a note event carries this observation into events.jsonl
- actual: Section E.5 navigation record, objective 20/27 `disable_the_relay`. Wording: 'Shut down the Tether Relay.' It names WHAT and a WHERE the player is standing in. Player-visible information: the relay apparatus and its pylons, in frame in GF-10-RELAY-02. Route decision: no travel; find the apparatus in the compound. AMBIGUITY RECORDED: section B's S07 span does not name this rung at all, so the protocol gives no site for the interaction; it is transcribed at the compound centre, which is where the apparatus stands.
- events: t=712.57
- verdict: PASS

### S07-65w — the world owns input before this walk
- expected: no narrative modal is holding locomotion when this walk starts
- actual: input_context=world (wanted world)
- events: t=712.57
- verdict: PASS
- observation: T2-GATEF-RUN6 modal-holds-locomotion audit. A walk that begins while a narrative modal still owns input does not fail as 'a modal is open' -- it fails as 'did not reach', many steps and sometimes many minutes later, which is how X04 lost its whole combat block to 3601 held frames. This assert is diagnostic and changes no behaviour: it turns that silent cascade into one named cause at the point of origin. RIG-25's rule, applied to narrative modals rather than to shop panels.

### S07-69r — walk to the ramp foot
- expected: GATE-D3-DECK: tether_relay.json's `ramps[0]` ('yard_stair') foot, in the site's own (s,t) frame at (-12.5,-5.0) -- converted with the same site centre/bearing S07-40g's own note already derives. A tight close_enough (1.5m against the ramp's own 3.2m width) so this leg actually ends ON the ramp's own footprint, not merely near it -- the wider 3.0m tolerance a first attempt used left enough slack that the NEXT leg's straight line to the console (local (2.9,-9.0) from the foot's (-12.5,-5.0), a bearing nothing like the ramp's own foot-to-head run) missed the incline and walked at ground level toward the pad's base instead of climbing to it.
- actual: walked 19.8 m to (339, 3768) in 277 walking frames (0 held)
- events: t=717.20
- verdict: PASS

### S07-69s — climb the ramp to its head
- expected: GATE-D3-DECK: `ramps[0]`'s own head, local (-5.5,-11.5) -- the far end of the SAME sloped surface S07-69r's foot stands on, so a walk between the two follows the ramp's own run (about 20 degrees per tether_relay.json's own ramp comment) rather than cutting across open air beside it.
- actual: walked 8.2 m to (337, 3758) in 122 walking frames (0 held)
- events: t=719.25
- verdict: PASS

### S07-69t — cross the gantry onto the pad
- expected: GATE-D3-DECK: the pad, local (7.0,-9.0) -- flat deck at the same deck_y as the gantry between it and the ramp head, so this leg is level walking once the climb above is done.
- actual: walked 12.6 m to (346, 3749) in 890 walking frames (0 held)
- events: t=734.10
- verdict: PASS

### S07-70 — walk to the relay apparatus console
- expected: data/config/relay_site.json's `site.centre` (350,3760) is the SITE's own coordinate, not the console's -- tether_relay.json's `apparatus.console.at` is a further-offset [2.9,-9.0] in the site's own local frame, on the raised pad (`deck_y: 10.0`) reached by a ramp off the yard's west side, per that file's own `_comment_decks`. A flat move_to at the site centre (ground level, no `y`) put the walker on open ground roughly under the pad rather than on it -- interact pressed at nothing, relay_disabled never set. move_to_entity resolves the live Console node by name and arrives in 3D (CD-5), which is what an elevated target with a real ramp underneath it needs.
- actual: FAIL reached Console (node name) in plan view (2.45 m in x/z) but it is 5.49 m away in 3D -- 4.91 m of that is vertical. Walking cannot close it, and an `interact` from here would press through the floor.
- events: t=734.12
- verdict: FAIL
- observation: answer_prompts turned ON for this journey walk. TRADE RECORDED EXPLICITLY: the schema says this flag must stay off in any segment whose subject is whether something blocks travel, so these segments can no longer evidence 'a narrative modal blocked the player from travelling'. That finding is already captured in full, un-answered, in S02-superseded-2/3/4 and S02/BLOCKER.md -- three press counts, identical 7201-frame holds. What it buys is everything downstream: measured in S03 and S04, an unanswered modal at a walk's end left input_context='narrative_modal' where the next step expected combat, so no fight started, no catch resolved, the team never reached three, and every tournament entry check then failed on team size. Without this the journey produces no combat, catching, building or progression evidence at all.

### S07-71 — shut the relay down
- expected: the rung's own verb
- actual: pressed interact x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=734.45
- verdict: PASS

### S07-72 — see it through
- expected: record what the world does when the pylons go quiet
- actual: pressed interact x10 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=738.67
- verdict: PASS

### S07-73 — the relay is down
- expected: `relay_disabled` closes section E.5 objective 20/27
- actual: flag relay_disabled NOT set
- events: t=738.67
- verdict: FAIL

### S07-74 — the crossing is tracked
- expected: Section E.5 objective 21/27 `restore_the_mill_crossing`, 'Restore the Old Mill Crossing.'
- actual: tracked objective id=relay_disabled text=Shut down the Tether Relay. (wanted restore_the_mill_crossing = flag_id mill_crossing_restored)
- events: t=738.67
- verdict: FAIL

### S07-75 — Section E.5 nav note -- restore_the_mill_crossing
- expected: a note event carries this observation into events.jsonl
- actual: Section E.5 navigation record, objective 21/27 `restore_the_mill_crossing`. Wording: 'Restore the Old Mill Crossing.' It names WHAT and a named WHERE. Player-visible information: `old_mill_crossing` is a `major` map landmark at (-152,4203) with a bridge icon, upstream and west of the relay; the river is the barrier the whole band is built around. Route decision: follow the bank west and upstream to the crossing pin. Reasoned from GF-10-RELAY-01, GF-10-RELAY-02 and the map frame taken at the top of this segment.
- events: t=738.67
- verdict: PASS

### S07-75a — record_start: band 3 -> band 4, the Old Mill Crossing
- expected: the crossing is the band 3/4 boundary and restoring it is what opens it: the window opens before the walk upstream so the approach along the bank is recorded
- actual: DELEGATED the §H record window "band 3 -> band 4, the Old Mill Crossing" to capture lane S07C (this is the logic lane; it keeps no continuous record)
- events: t=738.67
- verdict: DELEGATED

### S07-76 — walk upstream to the Old Mill Crossing
- expected: Section D RT-09: 'Relay -> Old Mill Crossing -> Upper Meadows entry | (350,3760) -> (-152,4203)'
- actual: FAIL did not reach (-152, 4196) in 31500 walking frames; stopped 662.8 m short at (340.0, 6.0, 3751.0) (0 held)
- events: t=1263.68
- verdict: FAIL
- observation: GATE-D3-CHANNEL. Section D's own (-152,4203) is the crossing's `channel.centre` (terrain_playground.json) -- the middle of the UNBUILT GAP, not a place to stand before the Gear restores the span. The previous target sent the walker straight at the void: it closed 3232m in the first 31492 frames and then spent the rest of its whole budget lodged 8.2m short, repeatedly falling in and getting returned to the road by severed_spokes.gd's carve failsafe (the earlier run's own log: 20+ 'player went over the edge... back to the road' lines at (-152,-7,4198)), because `close_enough: 4.0` against the centre demands standing inside the channel itself (half_length 6.0, so the gap alone runs 4197-4209). Retargeted at `gated_crossing.gd::near_point(7.4)` -- `centre - across*7.4` with `across=(0,1)` (derived here from the same `axis_deg: 0` / `_road_start()` rule the script itself uses, not eyeballed) -- which is also where `mill_crossing.gd`'s own Interactable prompt actually stands (`gate_offset: 7.4` in the same crossing's `bridge` block): (-152, 4195.6). It is 0.6m from where the walker organically stalled, so the earlier run had already effectively arrived and was failing only because the target it was still chasing was unreachable by construction.

### S07-77 — rescan for points of interest
- expected: the crossing's own content joins the POI set
- actual: 1089 points of interest in the tree
- events: t=1263.68
- verdict: PASS

### S07-78 — restore the crossing
- expected: Section L.5's gate/crossing row: 'Old Mill Crossing ... each locked-probe then legitimately opened'. The locked probe is X06's; this is the legitimate opening
- actual: pressed interact x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=1264.07
- verdict: PASS

### S07-79 — see it through
- expected: record what communicates that the crossing is usable again
- actual: pressed interact x10 (tap, 1 frames each) on the default device, resolved to JoyBtn:2
- events: t=1268.60
- verdict: PASS

### S07-80 — the crossing is restored
- expected: `mill_crossing_restored` closes section E.5 objective 21/27 and section B's S07 span
- actual: flag mill_crossing_restored NOT set
- events: t=1268.60
- verdict: FAIL

### S07-80a — record_stop: back to the 0.1 Hz journey baseline after band 3 -> band 4, the Old Mill Crossing
- expected: the window closes once `mill_crossing_restored` is set. Section B puts the cut here, so section H's other half of this +/-60 s is S08's own opening window
- actual: DELEGATED the close of the §H record window to capture lane S07C (this is the logic lane; no window was opened here)
- events: t=1268.60
- verdict: DELEGATED

### S07-81 — band 3 was actually walked
- expected: Section D RT-08 plus RT-09 is well over 2 km; no pacing claim may come from a shortcut (section 0.6)
- actual: walked 2207.9 m this segment (wanted >= 2200.0)
- events: t=1268.60
- verdict: PASS

### S07-82 — the 2 Hz trace ran throughout
- expected: Section C.2
- actual: route.csv has 2421 rows (wanted >= 3000)
- events: t=1268.60
- verdict: FAIL

### S07-83 — band 3's longest empty stretch is on the record
- expected: Section D: '>= 250 m is a finding; 150-250 m is a watch item'. The peak is the half that proves accumulation; a PASS records a watch item and its size
- actual: dead_travel peaked at 1082.3 m this segment (wanted >= 150.0); 2207.9 m walked in total
- events: t=1268.60
- verdict: PASS

### S07-84 — Section B save handoff
- expected: a note event carries this observation into events.jsonl
- actual: Section B: 'each journey segment ends by saving to a dedicated slot through the production Save tab and copying the resulting `user://` slot file into the run directory (`saves/<segment>-exit.json`)'. The steps below are that save, driven as a player drives it -- the pause shell, five RB presses to the Save tab, focus down to slot 4, and the slot's own Save button. `save_out` afterwards only copies the artefact; it saves nothing itself. 
- events: t=1268.60
- verdict: PASS

### S07-85 — open the pause shell
- expected: the game_menu button, sent as a real physical event, opens the pause shell onto the Satchel tab
- actual: map opened the shell: context world -> menu_map, focus on '' (@Control@70958)
- events: t=1268.78
- verdict: PASS
- observation: RIG-14: opened via the map shortcut rather than a bare game_menu press. The pause shell REOPENS ON THE LAST TAB USED, so a fixed press count from an assumed backpack start lands on the wrong tab in any segment that opens the menu more than once -- this segment's own Section E.5 map-navigation notes leave the shell on `menu_map`, so the bare `open_menu` + `times: 5` this step used before landed on `menu_backpack` (5 presses from map wraps past save and back around to backpack), the Save button was never actually focused, and `save_out` then copied out whatever was already seeded in slot 4 under this segment's own name -- the exact mechanism RIG-10 named without yet fixing. `map` is one of only two tabs with a shortcut in data/config/menu.json, so it is the only deterministic starting point available. Matches S03/S06/S08's own working save steps.

### S07-86 — cycle right to the Save tab
- expected: five RB presses move from Satchel to Save, the sixth tab in data/config/menu.json
- actual: pressed menu_tab_right x3 (tap, 1 frames each) on the default device, resolved to JoyBtn:10
- events: t=1269.32
- verdict: PASS
- observation: RIG-14: times 5 -> 3: counted from the map tab (index 2), not from an assumed backpack start. Target tab unchanged: save.

### S07-87 — the Save tab is up
- expected: input_context names the live tab: menu_save
- actual: input_context=menu_save (wanted menu_save)
- events: t=1269.32
- verdict: PASS

### S07-88 — move focus to slot 4's Save button
- expected: focus starts on slot 0's Save button (tab_save.gd::first_focus) and four d-pad downs reach slot 4's
- actual: 4 x ui_down moved focus 'Save' (@Button@71008) -> 'Save' (@Button@71028)
- events: t=1269.57
- verdict: PASS

### S07-89 — press Save
- expected: the focused Save button activates and writes slot 4 -- the handoff slot; autosave is slot 0 and slots 1-3 stay free for natural play coverage
- actual: pressed ui_accept x1 (tap, 1 frames each) on the default device, resolved to JoyBtn:0
- events: t=1269.80
- verdict: PASS

### S07-90 — give the write a moment
- expected: the slot file is on disk and section I.4 has timed the save operation
- actual: waited 120 physics frames
- events: t=1271.80
- verdict: PASS

### S07-91 — copy slot 4 out into the run directory
- expected: Section B: the resulting `user://` slot file is copied into the run directory as `saves/S07-exit.json`
- actual: slot 4 copied to saves/S07-exit.json (1420801 bytes)
- events: t=1271.80
- verdict: PASS

### S07-92 — close the shell
- expected: B closes the shell and hands the world back
- actual: menu_cancel closed the shell: context menu_save -> world
- events: t=1271.98
- verdict: PASS

### S07-93 — close the segment
- expected: a note event carries this observation into events.jsonl
- actual: S07 exit save written and copied.
- events: t=1271.98
- verdict: PASS
