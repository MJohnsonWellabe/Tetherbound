...
func _shared_variant_material(source: Material, name: String, colour: Color,
	...
	var body_emission_path := str(_cfg.get("body_emission_override", "")) if body else ""
	# A body may explicitly reject an imported full-surface emission channel.
	# Absent means preserve the source material exactly, so this is not a cast-wide
	...
	if body_emission_path != "":
		var body_emission := load(body_emission_path) as Texture2D
		if body_emission != null:
			material.emission_texture = body_emission
		else:
	...
	if material.emission_enabled:
		...
		if body and emission_floor > 0.0:
			...
			# T1-CAST supplies the VALUE (per-rank `emission_floor` out of
			# npc_ranks.json, so a grunt and a captain can differ) and T1-WORLD
			# supplies the SCALE (`_emission_floor_scale`, driven by the clock, so
			# the floor moves with time of day -- see its comment for what a night
			# frame looked like when it did not). The two lanes changed these two
			# lines independently and both changes are kept: at the base scale of
			# 1.0 this is byte-identical to each side's own daylight numbers.
