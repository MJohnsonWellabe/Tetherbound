...
func _spawn_creatures() -> void:
	...
	for index in entries.size():
		...
		for n in count:
			...
			# WILD-ECOLOGY (prompt 60): "a handful of special encounters across
			# the chapter... a reason to win even if the player does not catch
			# it". The cluster's FIRST member is its alpha when the entry asks
			# for one -- one per cluster, never the whole group, so the rest of
			# the cluster stays the ordinary population the band's level band
			# describes.
			#
			...
			# a species the player already knows, which is the read the word
			# should carry anyway.
			if n == 0:
