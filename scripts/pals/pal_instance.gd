...
## Ids into data/moves/moves.json (scripts/pals/move_db.gd resolves them to
...
## `make_pal`, `pal_species.spawn`, every test that calls `from_species(id,
## definition)` with two arguments — keeps getting a level 1 pal whose stats
