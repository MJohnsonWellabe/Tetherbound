...
## A live, orbitable 3D view of one pal, for the TEAM screen's centre column
## (spec §8.2).
##
## Lifts `scripts/ui/starter_picker.gd::_build_preview`'s construction almost
## verbatim — own_world_3d SubViewport, a warm key + cool rim light, `pal.tscn`
...
## `smoke_menu.gd` must stay green either way. If `pal.tscn` fails to build a
...
func _try_build_body(species_id: String) -> Node3D:
	var instance: Node3D = PAL_SCENE.instantiate() as Node3D
	if instance == null:
		return null
	instance.name = "TeamPreview_%s" % species_id
	instance.set_script(PAL_BODY)
...
## colour. Reached when `pal.tscn` cannot be instanced at all (as could
## happen headless with a stripped-down renderer) — a coloured shape still
## says "this is the pal in slot N", where an empty viewport says nothing.
