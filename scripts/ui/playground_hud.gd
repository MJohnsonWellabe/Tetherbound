...
## HUD-SCALE: -460 -> -340, kept in sync with `Root/BottomDock`'s own
## `offset_top` as this constant's header requires. The dock's contents lost
## ~120px when the quick-bar slot went 180 -> 132 and the legend 112 -> 64;
## `alignment = 2` bottom-aligns them, so a dock that kept reserving 460px
## would simply hold the left column 120px higher than it needs to be.
const BOTTOM_DOCK_TOP_OFFSET := -340.0
