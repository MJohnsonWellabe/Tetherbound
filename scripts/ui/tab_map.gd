...
##
## Within that local map, this is the instance for whichever realm the CANVAS
## is currently drawing (`_display_realm()`) -- never `game.map` directly, so
## that field (the single alias every other system reads as THE active realm)
## stays untouched by this tab merely looking at a realm the player is not in.
