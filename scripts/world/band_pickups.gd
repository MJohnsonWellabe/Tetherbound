...
static func place_one(world: Node3D, vegetation: Node3D, pickup: Dictionary) -> Dictionary:
	...
	if bool(spot["nudged"]):
		out["nudged"] = 1
		# N14: report the long ones. A 2 m step off a trunk is housekeeping; a
		# 6.5 or 8 m walk is far enough that the site's authored `_why` deserves
		# a re-read, and silence about it is how three sites sat unclear on
		# `main` for a wave without anyone reconciling them.
		#
		# Ported into place_one() by the landing lane: N14 wrote this inline in
		# the caller, which the consolidation had already refactored into this
		# function. Keeping the refactor and moving the reporting in preserves
		# both lanes rather than choosing one.
