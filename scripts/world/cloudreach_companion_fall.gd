...
func tick(delta: float, trainer: CharacterBody3D, flying: bool, world: Node) -> bool:
	...
	if flying or bool(trainer.call("is_carried")) or not trainer.is_on_floor():
		return false
...
func recover(companion: CharacterBody3D, trainer: CharacterBody3D) -> bool:
	...
	if spot == Vector3.INF:
		# Nowhere clear this frame. Try again next frame; the trainer moves.
		return false
	...
	companion.call("place_on_ground", spot)
...
func find_spot(companion: CharacterBody3D, trainer: CharacterBody3D) -> Vector3:
	...
	for distance: float in [ring, ring + 1.5]:
		for i in ANGLE_STEPS:
			...
			var floor_y := _walkable_floor(space, candidate, base.y, companion, exclude)
			if is_nan(floor_y):
				continue
...
func _walkable_floor(space: PhysicsDirectSpaceState3D, at: Vector3, level: float,
		companion: CharacterBody3D, exclude: Array[RID]) -> float:
	...
	if hit.is_empty() or (hit["normal"] as Vector3).y < cos(companion.floor_max_angle):
		return NAN
