...
static func write_all(
	...
	var manifest_path := ProjectSettings.globalize_path(_manifest_path(world_name))
	var manifest_file := FileAccess.open(manifest_path, FileAccess.WRITE)
	assert(manifest_file != null, "Failed to open scatter manifest for writing: %s (error %s)" % [manifest_path, FileAccess.get_open_error()])
