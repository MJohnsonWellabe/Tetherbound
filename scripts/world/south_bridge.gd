...
##
## The deeper Meadows is visible across an old bridge the player can walk to at
## any level and cannot cross. Beating Oskar yields the South Bridge Key
## (`SC13`); this file is the mechanism the key operates, and it is a mechanism
## and not a message.
##
## HARD RULE, from the backlog item and the spec: *"never explains itself with
## UI text."* There is no dialogue here — `road_gate.gd` says a line because
## `SA7` is a tutorial for the idea that gated things have keys, and this is
## the real gate, hours later, where the world has already taught it. A locked
## attempt gets a physical answer: the leaf jars against its lock and settles
## back. The only words anywhere near the crossing are on the trailhead
## fingerpost by the square, and they name the destination, never its
## condition — the same rule the seven severed spokes carry.
##
## Geography is authored, not invented here. `terrain_playground.json`'s
## `crossings[]` entry owns the road, the gully's carve and the bridge's
## offsets together, so this script cannot drift from the ground the bake
## actually cut: the span is placed from the carve's own centre and axis, and
## sized to it (see `data/config/building_prefabs.json`'s `south_bridge`).
##
...
const CROSSING_ID := "south_bridge"
const KEY_ITEM_ID := "south_bridge_key"
...
func build(world: Node3D) -> void:
	...
	if _crossing.is_empty():
		push_error("no `crossings` entry '%s' in %s; the South Bridge has no gap to span" % [
			CROSSING_ID, TERRAIN_CONFIG])
		return
	...
	if is_nan(near_ground) or is_nan(far_ground):
		push_error("no ground at the South Bridge's abutments; the span has nothing to stand on")
		return
	...
	if not prefabs.call("load_recipes"):
		push_error("no building recipes; the South Bridge cannot build its span")
		return
	...
	if span == null:
		push_error("south bridge prefab missing: %s" % bridge.get("prefab", "south_bridge"))
		return
	span.name = "Span"
	add_child(span)
...
## file places settlement structures from `village.json` and this is not one.
func _add_prefab_colliders(prefabs: RefCounted, span: Node3D, prefab_name: String) -> void:
	...
	if boxes.is_empty():
		...
		push_warning("the south bridge prefab authored no colliders; the deck is not walkable")
		return
...
func _build_gate(prefabs: RefCounted, bridge: Dictionary) -> void:
	...
	if leaf == null:
		push_error("south bridge gate prefab missing: %s" % bridge.get("gate_prefab", "south_bridge_gate"))
		return
	...
	_prompt.call("configure", "Try the bridge gate", 4.0, true)
