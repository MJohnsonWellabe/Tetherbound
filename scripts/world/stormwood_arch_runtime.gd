...
## `stormwood_dark_arches`: the optional ancient pairs C (Rodline–Lantern Hollow)
## and D (Old Rodfolk Hall–Fallen Giant). Trying one of their dark arches is the
## inspection; the second step follows the ledger's own paid lit flags, so the
## chain never writes an arch flag itself.
