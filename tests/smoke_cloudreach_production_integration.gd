...
func _run() -> void:
	...
	check(flags.call("has","realm_key_stormwood"),"reward grants canonical next-realm key")
	...
	await frames(8)
	check(game.get("progression").call("has","realm_key_stormwood"),"reward entitlement survives disk reload")
