...
func _check_full_map_controller_ownership_and_recovery() -> void:
	...
	# Regression for the player-facing failure that forced the compass rollback:
	# the real Meadows map must survive a complete close/reopen cycle. The
	# restored menu intentionally builds a fresh tab, so prove the new canvas is
	# present, clipped, and drawing the same real map state rather than asserting
	# the failed same-canvas experiment.
	await _press_action("map")
	if not bool(_menu.call("is_open")) or str(_menu.call("current_tab_id")) != "map":
		_fail("physical Map button did not open the full map a second time")
		return
	...
	if reopened_map == null or reopened_canvas == null or not reopened_canvas.clip_contents:
		...
		return
	if reopened_map.call("_map_state") != _game.get("map"):
		_fail("second full-map open lost the active Meadows map state")
		return
	for i in 12:
		await process_frame
	await _press_action("menu_cancel")
	if bool(_menu.call("is_open")) or paused:
		_fail("physical Back did not close the second full-map open")
		return
	print("  ok    real Meadows full map opens, closes, opens again, and closes cleanly")
...
func _report() -> void:
	...
	if _failures.is_empty():
		print("Gate A map/cycle: OK -- real pad cycling, movement-up minimap, repeated full-map open/close, zoom/pan, recovery.")
		quit(0)
		return
