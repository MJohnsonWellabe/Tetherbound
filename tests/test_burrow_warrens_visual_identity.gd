...
func test_facade_is_one_continuous_mantle_not_applied_shoulder_plates() -> void:
	...
	assert_false(bank.has("facade_cut"),
		"The rejected independently authored facade-shoulder family returned")
	var mantle: Dictionary = bank.get("facade_mantle", {})
	assert_true(bool(mantle.get("enabled", false)),
		"The throat outer end lost its continuous production bank mantle")
	assert_true(not mantle.has("earth_shoulders") and not mantle.has("root_runs"),
		"The continuous mantle regressed to applied shoulder or root entries")
	assert_true(float(mantle.get("toe_run_m", 0.0)) >= 10.0 and
		float(mantle.get("merge_depth_m", 0.0)) >= 28.0,
		"The mantle no longer has a broad toe-to-bank merge interval")
	assert_true(float(mantle.get("rear_half_width_m", 0.0)) >= 20.0 and
		float(mantle.get("rear_half_width_m", 0.0)) >=
			float(mantle.get("front_half_width_m", 99.0)) * 2.0,
		"The mantle no longer broadens continuously into the buried bank")
	assert_true(float(mantle.get("west_width_bias_m", 0.0)) >= 4.0 and
		float(mantle.get("centre_drift_west_m", 0.0)) >= 4.0 and
		float(mantle.get("east_width_recess_m", 0.0)) >= 1.5,
		"The single mantle lost its gradual west-heavy asymmetry")
	assert_true(float(mantle.get("height_m", 0.0)) >= 7.0 and
		float(mantle.get("union_blend_m", 0.0)) >= 0.8 and
		float(mantle.get("erosion_amount", 0.0)) >= 0.15,
		"The mantle lost its bank-scale height, smooth union or erosion")
	var front_mounds := 0
	...
	assert_true(height_source.contains("var mantle := _bank_foreland_mantle_term(x, z)") and
		height_source.contains("_smax(h, mantle, blend)") and
		height_source.find("_bank_foreland_mantle_term") <
			height_source.find("h = lerp(h, 0.0, settled)"),
		"Continuous mantle is not smooth-unioned before route suppression")
...
func test_first_interior_uses_organic_earth_finish_with_only_the_den_boundary_colliding() -> void:
	...
	assert_true(int(finish.get("length_segments", 0)) >= 24 and
		float(finish.get("passage_overlap_m", 99.0)) <= 0.35 and
	...
	assert_true(float(finish.get("mouth_passage_crown_scale", 0.0)) >= 1.2,
		"The mouth-to-hall crown can leave the final pale cap sliver above the threshold")
	assert_true(float(finish.get("mouth_passage_front_overlap_m", 99.0)) <= 0.35,
		"The mouth-to-hall shell can overlap back into the mouth as a nested rib")
	assert_true(float(finish.get("mouth_passage_half_width_extra_m", 0.0)) >= 0.55,
		"The continuous mouth gallery can collapse into a narrow repeated tunnel")
	assert_true(float(finish.get("mouth_passage_start_inset_m", 0.0)) >= 2.5,
		"The complete gallery profile can return as a freestanding exterior ring")
	assert_true(float(finish.get("mouth_front_open_depth_frac", 1.0)) <= 0.12 and
		float(finish.get("mouth_front_open_width_scale", 0.0)) >= 0.85 and
		float(finish.get("mouth_front_open_height_scale", 0.0)) >= 1.2,
		"The mouth cavern front can close back into a visible portal arch")
	assert_true((_warrens_config().get("bank", {}).get("warren_holes", []) as Array).is_empty(),
		"Applied dark oval hole props returned to the production mound")
	...
	assert_false(finish.has("arc_segments") or finish.has("endcap_arc_segments") or
		...
		finish.has("mass_latitude_segments") or finish.has("mass_relief"),
	...
	assert_false(organic_source.contains("_organic_portal_hood_mesh") or
		...
		organic_source.contains("radii") or
		organic_source.contains("var theta := PI - PI"),
		"Rejected capsule, arch, hood, portal or half-dome geometry remains visible")
	...
	assert_true(wall_source.contains("hide_legacy_box_visuals") and
		wall_source.contains("hide_chamber_visual") and
		wall_source.contains("hide_box_visual := hide_chamber_visual") and
	...
	var chambers_start := source.find("func _build_chambers")
	var chambers_end := source.find("func _face_is_exterior", chambers_start)
	var chambers_source := source.substr(chambers_start, chambers_end - chambers_start) \
...
func test_capture_serializes_final_pose_and_keeps_threshold_step_judgeable() -> void:
	...
	assert_true(source.contains('"geometry_revision": "BURROW-WARRENS-IDENTITY-R62"') and
		...
		source.contains('"excavated_threshold_canopy_count"') and
