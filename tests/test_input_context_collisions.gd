	...
	for name: String in _contexts().keys():
		...
		for action: String in actions:
			...
			if alias_of.get(action, action) == alias_of.get(other, other):
				continue
			_fail(
				"context '%s': '%s' and '%s' are both live and both bound to %s. One press, two verbs. Held-button chords are banned (archive/owner/OWNER_DIRECTIVES_2026-08-22.md section 1), so the fix is to move or merge a verb, not to make one of them a hold." % [
