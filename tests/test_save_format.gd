...
## GAME-F4. `base_hp`/`base_attack`/`base_defence` drive every level-up
...
## creature's stats collapsed to roughly the level multiplier alone. Found
## driving Gate F's isolated S07 leg: a hand-seeded level 9-13 party loaded
## with the right stats, then the first trainer win in the segment silently
## gutted every one of them.
...
## The species-lookup fallback: a save written before this fix has no
## `base_hp`/`base_attack`/`base_defence` keys at all. It must not fall back
## to CreatureInstance's own bare class defaults (which is the exact bug) --
## it must reconstruct them from `species.json`, the same "trust nothing you
## cannot look up" rule `_migrate_v1` already applies to a pre-D30 save's
## moves.
