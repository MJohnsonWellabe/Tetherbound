...
const CORRIDOR_HALF_WIDTH_M := 3.75
...
func _steepest(field: RefCounted, points: Array) -> Dictionary:
	...
	for i in range(1, points.size()):
		...
		for n in steps + 1:
			...
			for offset: float in [-CORRIDOR_HALF_WIDTH_M, 0.0, CORRIDOR_HALF_WIDTH_M]:
...
func test_every_authored_road_corridor_is_walkable() -> void:
	...
	for route: Dictionary in world.routes:
		if str(route.kind) == "island":
			continue  # The Crown ring is an arch-only island (test_stormwood_closed_gates_sealed).
