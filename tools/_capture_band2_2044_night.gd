extends SceneTree
## T1-CAST correction. band2-2044 (duskhush) carries `"time": "night"` in
## spawns.json -- confirmed by reading encounter_director.gd's own
## `_sync_spawn_gates` (wild.visible = _gate_active(gate)), a night-gated
## wild is INVISIBLE whenever the world clock is pinned to day, which
## tools/_capture_band24_rock_priority.gd's own `apply_time("day")` call
## did for every point including this one. The first pass's "duskhush is
## invisible on the rock" finding was reading an empty scene, not a
## silhouette failure. Re-renders the same vantage with night pinned
## instead, so the actual question (does duskhush read against this rock
## face) gets asked of a scene where duskhush actually exists.
##
##   xvfb-run -a -s "-screen 0 1280x800x24" godot --path . \
##     --rendering-driver opengl3 --resolution 1280x800 \
##     --script tools/_capture_band2_2044_night.gd

const HEIGHTFIELD := preload("res://scripts/world/playground_heightfield.gd")
const SCENE := "res://scenes/world/meadows_playground.tscn"
const OUT := "res://shots/band2_2044_night"
const SETTLE_FRAMES := 60
const PER_SHOT_SETTLE := 12
const EYE_HEIGHT := 1.7
const CLEAR_ENOUGH := 6.0

const TAG := "band2-2044-duskhush-32deg-NIGHT"
const CENTRE := Vector2(-395.0, 2500.0)
const RADIUS := 15.0
...
func _init() -> void:
	...
	# encounter_director.gd's own _process re-syncs every gated wild's
	# visibility against WorldLook's is_dark() each tick; a few frames after
	# apply_time("night") is enough for that to have run at least once.
	for _i in 5:
		await process_frame
