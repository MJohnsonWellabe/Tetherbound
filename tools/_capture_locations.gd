	...
	},
	{
		...
		"shots": [
			...
			{"label": "fire", "mode": "detail", "at": [237.0, 3678.0], "look": [241.0, 3674.0],
			 "back": 3.0, "up": 1.8, "look_up": 0.24,
			 "_why": "the campfire and what is (or is not) arranged around it.\n\nFINDING (tools/_probe_detail_shots.gd), not a framing problem: this shot's OWN look coordinate is wrong. band3_the_river_lock/props.json's actual `Bonfire_Fire`/`campfire_stone_ring` sit at (241.4, 3667.3) -- 6.72m from this look point (241.0, 3674.0), outside any reasonable 'near' radius (the probe used 6m). No back/up/look_up correction moves WHERE the camera aims in x/z, only how high and how far back -- so this frame cannot be made truthfully 'about the fire' without moving `look`, which is out of this round's scope (only back/up/look_up). What DOES stand within reach of the current look point is the checkpoint's own watch-post furniture (`Bench`, `Bag`, `Rope_1`, the east barricade crate/barrel) 1-6m off -- legitimately 'what is arranged around' the post per this shot's own second clause, and 1.9m from the real fire besides. `look_up` 0.24 aims at `Bench`'s own centre height (-3.64 = -3.88 + 0.24, ground read confirmed real via a smooth 8-sample eye/back gradient, not a collider hit); `back` 3.0 is the same ruler floor used throughout this round, well past the 2.93m the bench alone needs. Geometrically the real fire IS inside the horizontal FOV even at this distance (it sits ~4.45m off the view axis at a camera-to-fire depth of ~11-14m, and half-width at that depth is 8-10m), so it may appear small and off-centre in the frame rather than absent -- but centring it properly is a `look` fix for a future round, not this one."}
		]
	},
	{
		"id": "06-relay",
