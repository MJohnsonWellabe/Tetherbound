...
func _run() -> void:
	...
	# Sample several stopping points along it rather than committing to one
	# distance blind -- 180 frames landed in a hole (-25.4,-18.8), on_floor
	# false before the jump was even pressed.
	for chunk in [15, 15, 15, 15, 15, 15]:
		...
		for i in 20:
			await physics_frame
		_mark("pos=(%.1f, %.1f, %.1f) on_floor=%s vy=%.2f" % [
			mount.global_position.x, mount.global_position.y, mount.global_position.z,
			str(mount.is_on_floor()), mount.velocity.y])
