...
var LEGACY_TEXTURE: Texture2D
var _species := ""
var _wild: Node3D
var _bindings: Array[Dictionary] = []
func _run() -> void:
	if DisplayServer.get_name() == "headless":
		push_error("creature world A/B requires a rendering display")
		quit(1)
		return
	_biome_id = "meadows"
	...
	for arg: String in OS.get_cmdline_user_args():
		...
		elif arg.begins_with("--before="):
			var img := Image.load_from_file(arg.trim_prefix("--before=").strip_edges())
			if img != null and not img.is_empty():
				img.generate_mipmaps()
				LEGACY_TEXTURE = ImageTexture.create_from_image(img)
		if arg.begins_with("--output="):
			_output_dir = arg.trim_prefix("--output=").strip_edges()
	if _species.is_empty() or LEGACY_TEXTURE == null:
		push_error("creature world A/B requires --species= and a readable --before=<png>")
		quit(1)
		return
	if _output_dir.is_empty():
		push_error("creature world A/B requires a unique --output=res://... directory")
		quit(1)
		return
	for time_name: String in _times:
		for treatment: String in ["current", "before"]:
			_planned.append({"frame_id": "%s__%s__%s" % [_species, time_name, treatment],
	...
	if not _output_is_fresh():
		...
		return
	_begin_manifest()
	_manifest["fixture_disclosure"] = "Production Meadows world, Terrain3D, grass, WorldLook and CameraRig/Camera3D. Audit-spawned production creature on open authored terrain; movement/animation frozen. HUD hidden. Day/night clock pinned. The before treatment changes only duplicated runtime material albedo/emission textures. No gameplay/progression claim."
	_manifest["stage_xz"] = [STAGE.x, STAGE.y]
	_manifest["before_texture"] = "runtime ImageTexture from --before"
	_write_manifest()
...
func _prepare_subject() -> bool:
	...
	if director == null or is_nan(ground):
		...
		return false
	_wild = director.call("spawn_wild", _species, Vector3(STAGE.x, ground, STAGE.y),
		{"name": "CreatureWorldAB", "wander_radius": 0.0}) as Node3D
	if _wild == null:
...
func _collect_materials() -> bool:
	...
	if model == null:
		_failures.append("production creature model is missing")
		return false
	...
	if _bindings.is_empty():
		_failures.append("no visible textured BaseMaterial3D was bound to the creature")
		return false
	return true
func _capture(row: Dictionary) -> void:
	...
	if observed_clock.is_empty():
		return
	var legacy := str(row.treatment) == "before"
	if legacy:
		for binding: Dictionary in _bindings:
			...
			if actual.emission_texture == actual.albedo_texture:
	...
	if image == null or image.is_empty() or image.save_png(path) != OK:
		...
		print("CREATURE WORLD A/B %s -> %s" % [str(row.frame_id), path])
	_restore_materials()
	_write_manifest()
