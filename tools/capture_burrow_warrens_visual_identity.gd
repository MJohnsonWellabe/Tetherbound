...
## arrival/threshold are captured in authored day/night; the R11 organic route
...
##     --output=res://ralph/reports/MEADOWS-0912/final-warrens-11
...
func _run() -> void:
	...
	if bool(geometry_receipt.facade_root_holder_present):
		failures.append("R11 exterior still instantiated a separate facade-root assembly")
	if not bool(geometry_receipt.continuous_mantle_present):
		failures.append("R11 exterior did not build the continuous bank mantle")
	if int(geometry_receipt.organic_chamber_skin_count) != 3 \
		...
		failures.append("R11 organic route geometry is incomplete")
	if int(geometry_receipt.rejected_portal_hood_count) != 0:
		failures.append("R11 retained rejected projecting portal hoods")
	if int(geometry_receipt.rejected_threshold_fan_count) != 0:
		failures.append("R11 retained the rejected separately triangulated threshold fan")
	if int(geometry_receipt.hidden_threshold_collision_visual_count) != 2:
		failures.append("R11 threshold collision carriers are missing")
	if int(geometry_receipt.hidden_organic_wall_visual_count) != 12 \
			or int(geometry_receipt.hidden_organic_passage_visual_count) != 6:
		failures.append("R11 did not isolate every acceptance-route box mesh from rendering")
	if int(geometry_receipt.visible_rejected_carrier_count) != 0:
		failures.append("R11 rendered a collision-only legacy mesh")
	var complete := failures.is_empty() and records.size() == PLANNED_FRAMES.size()
	var manifest := {
		"geometry_revision": "BURROW-WARRENS-IDENTITY-R11",
		...
		"fixture_disclosure": "Production Meadows scene with ordinary live Terrain3D, scatter, props, vegetation, player and encounters. Exterior uses authored clear day/night and resets living residents to their authored homes before each comparison frame through wild_creature.revive_at_home(), preventing the day frame's elapsed AI time from biasing the night frame. Night exterior frames use the retained bounded capture-only key/rim evidence lights around the facade so bank, threshold walls and traveled floor remain judgeable; R11 adds no light and production materials, art and world lighting remain unchanged. Frames 03/03a/03b are the retained sequential outside-to-inside threshold receipt at three player-height positions. The separately triangulated ThresholdFan no longer exists; disturbed-earth colour comes from the production Bank surface. The old Throat, BankCap, chamber-wall and passage-box MeshInstances are hidden collision carriers only: their separately created production StaticBody/CollisionShape geometry stays active and unchanged. Indexed non-colliding organic threshold, passage, chamber and end-wall surfaces are the only rendered enclosure. Earned staging applies the ordinary CreatureInstance.take_damage + wild_creature.notify_fainted/clear_faint lifecycle only to the mandatory mouth and hall residents a player must already have beaten to stand there; guardian and optional branch resident remain fully live. HUD and independent SubmersionOverlay hidden; no progression reward/clear flag injected.",
