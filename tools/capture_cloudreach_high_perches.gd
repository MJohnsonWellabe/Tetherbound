...
## Dedicated production evidence for The High Perches. Both poses stand on the
## real crown while a fixed camera shows the complete south portal and roost
## court. No landmark, route, encounter, progression or actor is moved.
...
func _pose(stand: Dictionary) -> bool:
	...
	_camera.look_at(Vector3(target.x, target_y + float(stand.get("aim_up_m", 8.0)), target.y), Vector3.UP)
