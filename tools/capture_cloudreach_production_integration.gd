...
func _run() -> void:
	...
	if "--round5" in OS.get_cmdline_user_args():
		...
		performance_path=output_dir+"/performance.json"
	if "--round6" in OS.get_cmdline_user_args():
		output_dir="res://ralph/reports/CLOUDREACH-ENV-CORRECTION-0904/round6/sustained"
		performance_path="res://ralph/reports/CLOUDREACH-ENV-CORRECTION-0904/round6/sustained-performance.json"
	...
	if not repair and ("--round4" in OS.get_cmdline_user_args() or "--round5" in OS.get_cmdline_user_args() or "--round6" in OS.get_cmdline_user_args()):
