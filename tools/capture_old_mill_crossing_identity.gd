...
## proving the contained source/headrace/wheel/natural-cascade path and the
## sturdy timber-bent load path at ordinary player height. Production wildlife
...
## the full R14 projection/hydraulic contract.
...
func _run() -> void:
	...
	if hydraulic_selection.has("view"):
		...
		failures.append("no stable south/southwest hydraulic stand passed production physics and R14 visual gates")
	for raw: Variant in views:
		...
		for time_name: String in ["day", "night"]:
			...
			if not wildlife_blocker.is_empty():
				...
				continue
			var r14_proof := _verify_r14_projection(world, camera, str(view.name))
			var proof_failures := r14_proof.get("failures", []) as Array
			...
			records.append({
				...
				"r14_visual_proof": r14_proof.get("metrics", {}),
			})
			print("wrote %s" % path)
	...
	for expected: String in R14_EXPECTED_FRAMES:
		if not captured_frames.has(expected):
			failures.append("required R14 production frame missing: %s" % expected)
	for captured: String in captured_frames:
		if not R14_EXPECTED_FRAMES.has(captured):
			failures.append("unexpected R14 production frame: %s" % captured)

	var manifest := {
		"capture_serial": CAPTURE_SERIAL,
		"production_scene": SCENE,
		"named_location": "Old Mill Crossing",
		"r14_required_nodes": R14_PROOF_NODES,
		"r14_expected_frames": R14_EXPECTED_FRAMES,
		...
		"complete": failures.is_empty() and hydraulic_selection.has("view") \
			and records.size() == R14_EXPECTED_FRAMES.size(),
		"frames": records,
		"failures": failures,
	}
...
func _select_hydraulic_stand(world: Node3D, player: Node3D,
	...
	for raw_candidate: Dictionary in HYDRAULIC_STAND_CANDIDATES:
		...
		var proof := _verify_r14_projection(world, camera,
...
func _verify_r14_projection(world: Node3D, camera: Camera3D, view_name: String) -> Dictionary:
	var failures: Array[String] = []
	var metrics := {}
	var nodes := {}
	for key: String in R14_PROOF_NODES:
		var node := world.get_node_or_null(NodePath(str(R14_PROOF_NODES[key]))) as Node3D
		if node == null:
			failures.append("R14 proof node missing: %s" % key)
		else:
			nodes[key] = node
	...
	match view_name:
		...
		"03-hydraulic-sequence-south-bank":
			required = {
				...
				"foundation_return_brace": Vector2(8.0, 32.0),
				"foundation_lower_tie": Vector2(16.0, 5.0),
	...
	for key: String in required:
		...
		if rect.size.x < minimum.x or rect.size.y < minimum.y:
			failures.append("R14 %s is not projected/readable (%.1fx%.1f px; need %.1fx%.1f)" % [
	...
	if view_name == "03-hydraulic-sequence-south-bank":
		...
		for key: String in sequence:
			...
			if camera.is_position_behind(centre):
				failures.append("R14 hydraulic beat is behind camera: %s" % key)
		...
		for i in screen_points.size() - 1:
			...
			if length < 6.0:
				failures.append("R14 hydraulic beats %s -> %s collapse together on screen (%.1f px)" % [
		...
		for run: Array in [["headpond", "source", "headrace_upper", "headrace_mid",
			...
			for i in run.size() - 1:
				...
				if gap > 8.0:
					failures.append("R14 visible water ribbon breaks at %s -> %s (%.1f px)" % [
						run[i], run[i + 1], gap])
		...
		if total_span < 220.0:
			failures.append("R14 source-to-outfall chain spans only %.1f px" % total_span)
		...
		if vertical_drop < 110.0:
			failures.append("R14 headpond-to-outfall drop reads as only %.1f px" % vertical_drop)
		...
		if outfall_to_toe_gap > 4.0:
			failures.append("R14 tailrace outfall is visibly detached from its river toe")
		...
		if wheel_roundness < 0.72 or broadside_alignment < 0.72:
			failures.append("R14 wheel is not broadside/readable (roundness %.2f, alignment %.2f)" % [
		...
		if foundation_overlap >= 0.35:
			failures.append("R14 wheel is %.0f%% enveloped by foundation projection" % [
		...
		if not (source_point.y < contact_point.y and contact_point.y < discharge_point.y):
			failures.append("R14 source/contact/discharge do not descend on screen")
		...
		for i in world_heights.size() - 1:
			if world_heights[i + 1] >= world_heights[i] - 0.04:
				failures.append("R14 hydraulic grade is not downhill at %s -> %s" % [
