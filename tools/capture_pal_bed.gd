...
## Capture R2.8's pal bed for the required blind-judge visual pass.
...
## check -- a "pal bed" that reads as human-furniture-sized would be a real
## defect, not just a taste question.
##
##   xvfb-run -a -s "-screen 0 1280x720x24" \
##     godot --path . --rendering-driver opengl3 --resolution 1280x720 \
##     --script tools/capture_pal_bed.gd
...
func _alone() -> void:
	...
	await _shoot("%s/pal_bed_alone.png" % OUT_DIR, camera)
	world.queue_free()
...
func _with_workbench() -> void:
	...
	await _shoot("%s/pal_bed_scale_check.png" % OUT_DIR, camera)
	world.queue_free()
