...
| `focus_row` | `prefix`, `max_presses` (default 6) | Presses `ui_down`, re-reading `focus_text` after each press, until it starts with `prefix` (e.g. `"3."` for a numbered picker row's third entry, `tab_backpack.gd`'s target-picker rows read `"N.  Name  HP x / y"`). FAILs naming the last row text seen. Same reason `focus_item` exists over `focus_move`, applied to a picker whose auto-focus is not simply "the next item down the list": `tab_backpack.gd::_open_target_picker()` auto-focuses the first ELIGIBLE row (not fainted, not already full) — not the first not-yet-fed one, so a real feeding pass can land the SAME row twice in a row rather than advancing, and a blind press count has no way to know that. |
