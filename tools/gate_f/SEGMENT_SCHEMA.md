...
| `equip_tool` | `tool`, `control`, `max_attempts` (default 3), `settle_frames` (default 60) | Presses `control`, then re-reads `equipped().item` off the live probe until it equals `tool` — retrying up to `max_attempts` times — instead of a single blind hotbar press. Written because `playground_hud.gd`'s hotbar toggles: pressing the SLOT THAT IS ALREADY EQUIPPED unequips it rather than re-selecting it, and a press landing while a tool swing (`tool_hold.gd`) is still in flight is dropped, not queued — either one silently leaves the wrong tool (or nothing) equipped, which then reads at the harvest node as a refused gather (`harvest_node.gd`'s "Needs a [tool]." toast, e.g. "Needs a Knife.") that this vocabulary previously had no way to see. PASSes immediately, reporting 0 presses, if the wanted tool is already equipped. FAILs at `max_attempts` naming what is actually held. |
