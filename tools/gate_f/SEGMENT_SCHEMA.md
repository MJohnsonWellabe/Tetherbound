...
| Action | Args | Does |
|---|---|---|
| `press` | `control`, `hold`, `times`, `settle_frames`, `device`, `skip_if` (an `assert` check dict) | One control, down-edge, held, up-edge. `times` repeats it with `settle_frames` idle frames between repeats — use it instead of N identical steps, which hide which press failed to land. `device` picks which binding to inject; see **Devices** below. `skip_if` is the same CD-4 escape most other actions have — names the state in which this press is MOOT and returns SKIP before pressing. |
| `select_healthy_party` | `budget_frames` (default600, capped600) | In world context, reads the earned roster and selects its eligible creature with greatest current HP through mapped LB taps only. Pins party identity/order; traverses at most once and verifies the actual companion is visible. Already-best needs no input. No HP, party-index or deployment writes. Maximum600 physics waits and8 process waits; refuses modal/build ownership and unavailable companions. |
| `recover_fainted_party` | `budget_frames` (default/cap280) | In world context, recover up to five fainted earned members through physical Satchel open, item-identity grid navigation, Revive target selection and confirmation. Pins actual party/member identities, verifies exactly one owned draught spent per recovered member and no other member changed, and returns to world input. A living team needs zero input; insufficient stock, wrong focus/target, rejected controls or missing readback fail. Full conservative cost280 physics and670 process frames; no health, inventory or party writes. |
...
| `chip_to_floor` | `control` (`combat_quick`), `hold` (`tap`), `settle_frames` (30), `max_presses` (15), `ready_budget_frames` (180, capped600), `safety_factor` (1.25), `floor_fraction` (0.01), `skip_if` | Reads live solo production stats and bounds the next quick hit with production damage math, variance, type/move multipliers and possible critical damage before EVERY tap, including the first. Waits for pending attacks/cooldown within its charged budget, then re-reads HP. Also retains observed-hit safety. Stops before crossing the authored floor; never writes HP. Missing live encounter, fainting, unresolved readiness or a non-quick/multi-hit request fails. A freshly satisfied authored `skip_if` returns explicit no-input condition satisfaction. |
...
| `throw_until_caught` | `max_throws` (4), `aim_budget_frames` (240), `resolve_seconds` (6.0), `throw_control` (`interact`), `survival_hp_fraction` (optional), `skip_if` | Arms and tracks physically, verifies each real orb launch, and observes production verdict/miss/exit signals. Breakouts and misses permit another attempt immediately; catch requires a caught exit plus the exact foe appended to the unchanged earned party. Exhausting every allotted verified throw is an explicit attempt-complete-without-catch outcome, as authored in S03; it is not a caught result and does not waive following world-context or roster assertions. Early loss, invalid targeting, missing launch/outcome, changed identity and unproven party growth remain FAIL. Optional survival uses physical LB only. |
...
| `force_aim` | `skip_if` (an `assert` check dict) | HARNESS-ONLY SHORTCUT, and deliberately not the honest one -- `track_aim` above stays the real player-input simulation and should be preferred in any segment actually verifying aim. `force_aim` assigns `camera_rig.gd`'s public `yaw`/`pitch` directly instead of nudging them through `_stick_right`, so there is no per-frame turn-rate limit for a full-speed wild creature to outrun. It reuses `track_aim`'s own camera-eye/`centre()` geometry (the ray is cast from the aim camera, not the trainer -- see that row for the parallax reason) and still goes through the live `aim_report()` eligibility check afterward, FAILing loudly if that does not confirm. It shortcuts the STEERING ONLY: the catch roll, the orb physics, and everything downstream of the throw are untouched, and like `track_aim` it never presses the throw itself. Added for ralph/GATE-F-S03-CATCH-LOOP, whose subject is the team-building/revive/economy logic downstream of a catch rather than aim responsiveness -- `track_aim`'s clamped turn rate against a full-speed target was a second, independent source of flakiness on top of what that lane was verifying (gate_f-run-20260901T220548Z-s03fix attempt 5 spent a whole 966s segment landing exactly one catch, every scripted throw missing on `reticle_outside_body`/`ground`). Requires `combat_aim` already entered, same as `track_aim`; `skip_if` is the same CD-4 escape. | 
...
| `focus_row` | `prefix`, `max_presses` (default 6), `optional` (default `false`), `skip_if` (an `assert` check dict) | Presses `ui_down`, re-reading `focus_text` after each press, until it starts with `prefix` (e.g. `"3."` for a numbered picker row's third entry, `tab_backpack.gd`'s target-picker rows read `"N.  Name  HP x / y"`). FAILs naming the last row text seen. Same reason `focus_item` exists over `focus_move`, applied to a picker whose auto-focus is not simply "the next item down the list": `tab_backpack.gd::_open_target_picker()` auto-focuses the first ELIGIBLE row (not fainted, not already full) — not the first not-yet-fed one, so a real feeding pass can land the SAME row twice in a row rather than advancing, and a blind press count has no way to know that. `optional: true` turns "never reached it" into a `SKIPPED` and backs out of the picker with one `menu_cancel` instead of FAILing: `_eligible()`'s gate pulls an ineligible row (e.g. already at full nourishment) out of the focus chain entirely (`focus_mode = FOCUS_NONE`), so `ui_down` genuinely cannot reach it no matter how many presses — a real, expected state for a script that names a specific entrant without knowing every creature's live satiety in advance, not a flaky press. `skip_if` is the same CD-4 escape most other actions have. |
...
| `fight_until_resolved` | `budget_frames` (default 9000), `switch_below` (default 0.35), `gap_frames` (default 18), `quiet_frames` (default 240), `until_flag`, `require_victory` (default false) | Drives a fight **by predicate, never by a press count** — the same reason `advance_dialogue_until_closed` exists. Presses `combat_quick` only while the action machine reads READY (so it never mashes into the commitment guard `can_switch()` respects), and presses `party_cycle` ONCE when the piloted creature drops to `switch_below` of its max HP and the manager says a switch is possible. Stops when `is_fighting()` AND `EncounterDirector::trainer_battle_active()` have both been false for `quiet_frames` — both, because a trainer battle goes quiet BETWEEN its creatures and a driver that stopped on the first gap would abandon a five-creature Warden after his first one fell — or when `budget_frames` runs out, or when the cost gate stops the run. FAILs if `until_flag` is named and not set at the end. With `require_victory:true`, also requires an observed production `won` exit, no combat still running, unchanged party identities and live XP/level progress (except an already fully capped roster). Loss, absent exit and unfinished combat fail. Opens no menu and uses no item: a fight it wins is won on levels, types and the belt. |
...
| `move_to_entity` | `entity`, `within` (default 2.5), `nearest` (default `true`), `rank` (default 0), `close_3d` (default `true`), `optional` (default `false`), plus every `move_to` arg | Walks to a **thing**, resolved by identity and re-read every frame so the walk tracks something that moves. Resolution order: exact node name, group membership, `label()`, `species_id`, then a name substring; ambiguity picks the nearest and says so in the result; **`rank` picks the Nth-nearest instead** (0 = nearest), clamping to what actually spawned and saying it clamped. `rank` is what makes a RETRY ladder retry something: `ralph/GATE-F-FULL` measured S03's ten-attempt engage ladder resolving the same creature ten times, walking 0.0 m each time because the player was already inside `within`, and losing the interaction arbiter to the same deadwood node on all ten. An entity that is not in the world is a FAIL naming the search — "I arrived and nothing was here" is a finding a coordinate walk cannot make — **unless `optional: true`**, which turns that same "not found" into a `SKIPPED`: written for an opportunistic pass that names a `rank` past however many actually spawned (e.g. a training loop asking for the 25th-nearest wild creature in a meadow that may hold fewer), where "not found" is an expected outcome, not a defect. Every other caller keeps the loud FAIL — a step naming a SPECIFIC entity it expects to exist not finding it is real information a SKIP would bury.  With `require_engage_prompt:true`, arrival also requires this exact living wild to be the director's engageable target and the director to own the actionable Engage winner. Up to four physical approach stances share the original walking/held budgets; reaching a stance alone is not success. The verified target is pinned for the next guarded interaction. |
| `interact_with` | `entity` (optional), `expect_prompt`, `check_provider` (default `true`), `expect_change` (default `true`), `optional` (default `false`), `control` (default `interact`), `hold`, `settle_frames` | Presses `control` (`interact` unless overridden) **only when `interaction_arbiter.gd` has a live prompt**, and refuses otherwise with what the arbiter could see instead — including how far the named entity is in 3D and how much of that is vertical. Also refuses when the prompt belongs to a different provider, or when `expect_prompt` does not appear in it: a prompt from the wrong provider is how a step meaning to talk to Grandpa opens a chest, and it reads as a successful interaction either way. FAILs if the press changed nothing — `expect_change: false` acknowledges a press the operator knows is legitimately inert, and must be written down before playing so a quiet no-op can never be read as an expected one. `optional: true` turns the three "not pressed" reasons above (arbiter disabled, no live prompt, wrong live prompt) into a `SKIPPED`, never a press: written for a harvest node's second swing, where the node's own prompt may legitimately already be gone, but ALSO where a wild creature can wander up and win the arbiter's priority-then-nearest contest over the node before the second tap lands — a plain unconditional press there cannot tell "Strip meadow grass" from "Engage Mudsnout", and pressed at the wrong moment starts a real unplanned wild fight the rest of a gather ladder has no script to resolve, FAILing every following step for as long as the world stays in combat. Measured directly: one run that reached this lost 6+ downstream steps to exactly that before `optional` existed. `control` exists because not every live prompt answers to `interact` — `encounter_director.gd`'s "Put &lt;creature&gt; away" is bound to `creature_recall`, not `interact` (CONTROLLER-MAP: "Fleeing is RB. Putting the creature away IS disengaging.") — so a step that means to press a specific verified prompt with a different button names it here rather than getting silently ignored by the correct-looking-but-wrong `interact` press underneath.  `require_selected_engage:true` rechecks the preceding verified wild's identity, actual winning provider and actionable offer after prompt settling and immediately before input. Missing, changed, dead or overshadowed targets fail without a press; the usual verified full-roster condition is evaluated first. |
...
| `select_menu_tab` | `tab`, `max_presses` (default 16, clamped 1–32) | From an open pause shell, presses physical controller RB until the live `menu_<tab>` context matches. Waits for each real transition and deferred focus. Handles conditional Skills and Players without fixed tab counts. FAILs on a stalled transition, repeated tab, lost menu context, or exhausted budget. Does not open the shell or mutate production menu state. |
...
| `flag_set` | `flag`, `equals` (default `true`) | Require that exact progression flag to be set, or absent with `equals:false`. Harvest routes bracket the first interaction with absent/present checks of the authored node's flag. |
| `companion_deployed` | — | The active party member is living and is the encounter director's actual companion, with a visible, in-tree body whose deletion is not queued. A healthy party slot or hidden pending spawn alone fails. |
| `objective_is` | `id` | The tracked objective's flag id matches. |
| `party_size` | `equals` | The live party holds that many. |
| `active_creature_alive` | — | The piloted/deployed creature (mid-fight: `combat_manager.gd`'s own active; else the party's own active index) has HP > 0. 2.11/S08: the party's active index does not move when its occupant faints, so `party_size` and even a non-null `active_creature()` both stay silent about a fainted lead. Place this immediately before a fight step, not after — it is a pre-flight gate, and it fails the segment honestly at the moment the party stopped being fight-ready instead of letting `encounter_director.gd::can_challenge()`'s silent refusal (or, worse, a scripted press landing on whatever menu happens to be focused instead) burn the rest of the segment's budget unexplained. |
| `inventory_count` | `item`, `min`/`max`/`equals` | Live-reads `item`'s count off the same satchel snapshot the diff detector uses. For verifying a spend/gain actually happened, rather than trusting that a press sequence did what it meant to. |
| `creature_fed` | `index`, `equals` (default `true`) | Live-reads `party_state()[index].fed` — `creature_condition.gd::is_fed()`'s own threshold (`nourishment_fraction >= fed_at`, default 0.55). Note this is a LOWER bar than `tab_backpack.gd::_eligible()`'s food gate (`nourishment_fraction < 1.0`, i.e. "not yet full") — a creature can be `fed:true` here and still a legitimate feeding target. Prefer `focus_row`'s own `optional: true` over this for "is this entrant still feedable" — it checks the real gate empirically instead of a proxy threshold. |
