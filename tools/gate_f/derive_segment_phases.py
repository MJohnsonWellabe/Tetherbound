...
def derive(source: dict, plan: dict, source_text: str) -> tuple[list[dict], dict]:
    ...
    if final_exports.count(outputs[-1]) != 1:
...
def outputs_for_plan(plan_path: Path, root: Path = ROOT) -> dict[Path, str]:
    ...
    phases, manifest = derive(json.loads(text), plan, text)
